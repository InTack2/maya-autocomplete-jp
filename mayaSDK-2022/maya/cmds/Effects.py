...
def dynExpression(creation: bool = False,runtime: bool = False,runtimeAfterDynamics: bool = False,runtimeBeforeDynamics: bool = False,string: str = "") -> None:
    """
...
def expression(alwaysEvaluate: int = 1,animated: int = 1,attribute: str = "",name: str = "",object: str = "",safe: bool = False,shortNames: bool = False,string: str = "",timeDependent: bool = False,unitConversion: str = "") -> None:
    ...
    timeDependent (boolean): 時間が経過してエクスプレッションが評価された場合は、trueを返します。次の理由がある場合は、エクスプレッションを時間依存にすることができます。-エクスプレッションが「time」または「frame」キーワードを参照している。-エクスプレッションに副作用がある(安全ではない)。-エクスプレッションノードの「time」アトリビュートが手動で接続されている。エクスプレッションが安全で、時間に依存しない場合は、alwaysEvaluateがオンになっているときでも、常に必要に応じて評価されます。
