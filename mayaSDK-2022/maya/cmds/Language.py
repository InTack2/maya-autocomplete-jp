...
def matrixUtil(inverse: bool = False,quaternion: Tuple[float, float, float, float] = tuple(1.0, 1.0, 1.0, 1.0),relative: bool = False,rotation: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),scale: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),shear: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),translation: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),transpose: bool = False) -> None:
    """
    ...
    inverse (boolean): 合成または照会を行うと、逆行列が返されます。

    -----------------------------------------

    quaternion ([float, float, float, float]): 指定したクォータニオン値を回転コンポーネントに使用して、行列を合成、編集、照会します。

    -----------------------------------------

    relative (boolean): 移動、回転、スケール、またはシアを絶対値で設定しないで、値を加算します。

    -----------------------------------------

    rotation ([float, float, float]): 指定した値を回転コンポーネントに使用して、行列を合成、編集、照会します。

    -----------------------------------------

    scale ([float, float, float]): 指定した値をスケールコンポーネントに使用して、行列を合成、編集、照会します。

    -----------------------------------------

    shear ([float, float, float]): 指定した値をシアコンポーネントに使用して、行列を合成、編集、照会します。

    -----------------------------------------

    translation ([float, float, float]): 指定した値を移動コンポーネントに使用して、行列を合成します。

    -----------------------------------------

    transpose (boolean): 合成または照会を行うと、転置行列が返されます。
...
def optionVar(arraySize: str = "",clearArray: str = "",clearStash: str = "",exists: str = "",floatValue: Tuple[str, float] = tuple("", 1.0),floatValueAppend: Tuple[str, float] = tuple("", 1.0),intValue: Tuple[str, int] = tuple("", 1),intValueAppend: Tuple[str, int] = tuple("", 1),list: bool = False,remove: str = "",removeFromArray: Tuple[str, int] = tuple("", 1),stash: str = "",stringValue: Tuple[str, str] = tuple("", ""),stringValueAppend: Tuple[str, str] = tuple("", ""),unstash: str = "",version: int = 1) -> None:
    """
    ...
    clearStash (string): 変数のバックアップコピーをクリアします。
    ...
    stash (string): 変数のバックアップコピーを作成します。
    ...
    unstash (string): バックアップコピーから変数を復元します。
...
def scriptJob(allChildren: bool = False,attributeAdded: Tuple[str, str] = tuple("", ""),attributeChange: Tuple[str, str] = tuple("", ""),attributeDeleted: Tuple[str, str] = tuple("", ""),compressUndo: bool = False,conditionChange: Tuple[str, str] = tuple("", ""),conditionFalse: Tuple[str, str] = tuple("", ""),conditionTrue: Tuple[str, str] = tuple("", ""),connectionChange: Tuple[str, str] = tuple("", ""),disregardIndex: bool = False,event: Tuple[str, str] = tuple("", ""),exists: int = 1,force: bool = False,idleEvent: str = "",kill: int = 1,killAll: bool = False,killWithScene: bool = False,listConditions: bool = False,listEvents: bool = False,listJobs: bool = False,nodeDeleted: Tuple[str, str] = tuple("", ""),nodeNameChanged: Tuple[str, str] = tuple("", ""),optionVarChanged: Tuple[str, str] = tuple("", ""),parent: str = "",permanent: bool = False,protected: bool = False,replacePrevious: bool = False,runOnce: bool = False,timeChange: str = "",uiDeleted: Tuple[str, str] = tuple("", "")) -> None:
    ...
    optionVarChanged ([string, script]): 指定したoptionVarの値が変わったときにスクリプトを実行します。optioNVarが削除された場合、(削除を元に戻すことができても)このジョブは中断されます。
