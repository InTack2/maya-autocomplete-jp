...
def file(absoluteName: bool = False,activate: bool = False,activeProxy: bool = False,add: bool = False,anyModified: bool = False,applyTo: str = "",buildLoadSettings: bool = False,channels: bool = False,cleanReference: str = "",command: Tuple[str, str] = tuple("", ""),compress: bool = False,constraints: bool = False,constructionHistory: bool = False,copyNumberList: bool = False,defaultExtensions: bool = False,defaultNamespace: bool = False,deferReference: bool = False,editCommand: str = "",errorStatus: bool = False,executeScriptNodes: bool = False,exists: bool = False,expandName: bool = False,exportAll: bool = False,exportAnim: bool = False,exportAnimFromReference: bool = False,exportAsReference: bool = False,exportAsSegment: bool = False,exportSelected: bool = False,exportSelectedAnim: bool = False,exportSelectedAnimFromReference: bool = False,exportSelectedNoReference: bool = False,exportSelectedStrict: bool = False,exportSnapshotCallback: Tuple[str, str] = tuple("", ""),exportUnloadedReferences: bool = False,expressions: bool = False,fileMetaData: bool = False,flushReference: str = "",force: bool = False,groupLocator: bool = False,groupName: str = "",groupReference: bool = False,ignoreVersion: bool = False,i: bool = False,importFrameRate: bool = False,importReference: bool = False,importTimeRange: str = "",lastFileOption: bool = False,lastTempFile: bool = False,list: bool = False,loadAllDeferred: bool = False,loadAllReferences: bool = False,loadNoReferences: bool = False,loadReference: str = "",loadReferenceDepth: str = "",loadReferencePreview: str = "",loadSettings: str = "",location: bool = False,lockContainerUnpublished: bool = False,lockFile: bool = False,lockReference: bool = False,mapPlaceHolderNamespace: Tuple[str, str] = tuple("", ""),mergeBaseAnimLayer: bool = False,mergeNamespaceWithParent: bool = False,mergeNamespaceWithRoot: bool = False,mergeNamespacesOnClash: bool = False,modified: bool = False,moveSelected: bool = False,namespace: str = "",newFile: bool = False,open: bool = False,options: str = "",parentNamespace: bool = False,postSaveScript: str = "",preSaveScript: str = "",preserveName: bool = False,preserveReferences: bool = False,preview: bool = False,prompt: bool = False,proxyManager: str = "",proxyTag: str = "",reference: bool = False,referenceDepthInfo: int = 1,referenceNode: str = "",relativeNamespace: str = "",removeDuplicateNetworks: bool = False,removeReference: bool = False,rename: str = "",renameAll: bool = False,renameToSave: bool = False,renamingPrefix: str = "",renamingPrefixList: bool = False,replaceName: Tuple[str, str] = tuple("", ""),reserveNamespaces: bool = False,resetError: bool = False,returnNewNodes: bool = False,save: bool = False,saveDiskCache: str = "",saveReference: bool = False,saveReferencesUnloaded: bool = False,saveTextures: str = "",sceneName: bool = False,segment: str = "",selectAll: bool = False,shader: bool = False,sharedNodes: str = "",sharedReferenceFile: bool = False,shortName: bool = False,strict: bool = False,swapNamespace: Tuple[str, str] = tuple("", ""),type: str = "",uiConfiguration: bool = False,uiLoadConfiguration: bool = False,unloadReference: str = "",unresolvedName: bool = False,usingNamespaces: bool = False,withoutCopyNumber: bool = False,writable: bool = False) -> None:
    ...
    exportSnapshotCallback ([script, string]): -ea/exportAll、es/exportSelected、および-ess/exportSelectedStrictと一緒にこのフラグを指定すると、Mayaは書き出しのターゲットを指定したネームスペースに一時的に複製してからコールバックを呼び出し、重複する書き出しターゲットを操作し、その後で重複する書き出しターゲットをディスクに書き込むことができます。ディスクへの書き込み後、重複する書き出しターゲット、コールバックによって作成された新しいノード、およびおよび一時的なネームスペースがシーンから削除されます。明示的に作成されたノード(パースビューノード、最上位ノードなど)は複製されません。コールバックの継続期間については、以下のようになります。1.指定したネームスペースが最新になります。2.コールバックによって追加されたすべてのノードは、一時的な書き出しターゲットとして追跡されます。3.コールバックの目的は重複する書き出しターゲットを操作することのみですが、コールバックによるメインシーンの変更は制限されません。したがって、コールバックを書き込む場合は慎重に行ってください。このフラグには2つの引数を指定できます。a.[文字列]ディスクに書き込む前に呼び出すコールバック。b.[文字列]重複する書き出しターゲットを格納するための一時的なネームスペース。このフラグは、-ea/exportAll、-es/exportSelected、および-ess/exportSelectedStrictと組み合わせた場合のみ使用できます。-pv/previewフラグを指定した場合、プレビューされるのはスナップショットの複製前の書き出しターゲットの内容のみです。コールバック後の最終的な出力はプレビューされません。リファレンスノードは、これらのノードをシーン内で手動で複製する場合と同じ方法で複製されます。同様に、シーンアセンブリノードも手動で複製する場合と同じように複製されます。ただし、シーンアセンブリノードを複製すると特殊な動作が発生するため、書き出しターゲットに重複があると予測される場合は、コールバックでこれらの動作の違いを認識する必要があります。
    ...
    ignoreVersion (boolean): 公式にサポートされていないバージョンのファイルを開く場合に使用します。成功は保証されません。その結果、データが失われる、データが破損する、またはデータを開けなくなることがあります。一緒に使用できるのは、-oフラグと-iフラグのみです。
    ...
    mergeBaseAnimLayer (boolean): 設定すると、ファイルから読み込まれたベースアニメーションレイヤと、シーン内の既存のベースレイヤがマージされます。
    ...
    reserveNamespaces (boolean): このフラグを有効にすると、ロードされていないリファレンスのネームスペースは、ファイルがロードされた後で作成されるようになります。これにより、後でリファレンスをロードまたは追加したときに、予期しないネームスペースの衝突が発生する可能性が低くなります。
    ...
    uiLoadConfiguration (boolean): シーンのUI設定をロードします。（たとえば、panes）。現在の既定はオンで、initialStartup.melに設定されます。
...
def fileInfo(referenceNode: str = "",remove: str = "") -> None:
    """
    ...
    referenceNode (string): 照会するシーンファイルを示すには、referenceNodeの名前を指定します。このフラグは照会中のみ有効です。照会モードでは、このフラグに値が必要になります。
...
def namespace(absoluteName: bool = False,addNamespace: str = "",collapseAncestors: str = "",deleteNamespaceContent: bool = False,exists: str = "",force: bool = False,isRootNamespace: str = "",mergeNamespaceWithOther: str = "",mergeNamespaceWithParent: bool = False,mergeNamespaceWithRoot: bool = False,moveNamespace: Tuple[str, str] = tuple("", ""),parent: str = "",recurse: bool = False,relativeNames: bool = False,removeNamespace: str = "",rename: Tuple[str, str] = tuple("", ""),setNamespace: str = "",validateName: str = "") -> None:
    """
    ...
    mergeNamespaceWithOther (string): 「rm/-removeNamespace」フラグと一緒に使用します。ネームスペースを除去する場合、残りのネームスペースの内容を指定したネームスペースに移動します。
...
def loadModule(allModules: bool = False,load: str = "",scan: bool = False) -> None:
    """
    Maya プラグインは、Maya の標準プラグイン フォルダのいずれかに個別にインストールしたり、「モジュール」内の他のリソースとともにパッケージ化することができます。各モジュールは独自のフォルダに存在し、モジュールが提供するプラグインを Maya に認識させるモジュール定義ファイルを提供します。
...
def pluginInfo(activeFile: bool = False,allEvaluators: bool = False,animCurveInterp: str = "",apiVersion: bool = False,autoload: bool = False,cacheFormat: bool = False,changedCommand: str = "",command: str = "",constraintCommand: bool = False,controlCommand: bool = False,data: Tuple[str, str] = tuple("", ""),dependNode: bool = False,dependNodeByType: str = "",dependNodeId: str = "",device: bool = False,dragAndDropBehavior: bool = False,evaluator: bool = False,iksolver: bool = False,listPlugins: bool = False,listPluginsPath: bool = False,loadPluginPrefs: bool = False,loaded: bool = False,modelEditorCommand: bool = False,name: str = "",path: str = "",pluginsInUse: bool = False,referenceTranslators: bool = False,registered: bool = False,remove: bool = False,renderer: bool = False,savePluginPrefs: bool = False,serviceDescriptions: bool = False,settings: bool = False,tool: str = "",translator: bool = False,unloadOk: bool = False,userNamed: bool = False,vendor: str = "",version: bool = False,writeRequires: bool = False) -> None:
    """
    ...
    allEvaluators (boolean): いずれかのプラグインで登録されている、すべてのエバリュエータタイプの名前を含む文字配列を返します。
    ...
    evaluator (boolean): このプラグインで登録されている、すべてのエバリュエータタイプの名前を含む文字配列を返します。
...
def dgValidateCurve(allCurves: bool = False,verbose: bool = False) -> None:
    """
    カーブの内部状態が実際の状態と一致しているかを確認するには、dgValidateCurve コマンドを使用します。スタティックのタグが付いていない可能性のあるカーブを(タグが付いていたとしても)強制的にチェックします。DG は、それらがアニメートされていると考慮しないことにより、スタティック カーブを追跡して評価を最適化します。カーブにキーが追加され、修正されたら、スタティックではなくなります。カーブ上の特定の操作により、アニメーションなしでフラットになる可能性がありますが、すぐに再修正されると想定するため、DG はそれをスタティックであるとはみなしません。このコマンドにより、アニメーション カーブのスタティック状態に対する明示的な要求チェックを行えます。
    ...
    allCurves (boolean): 選択された、または指定されたオブジェクトを無視し、すべてのカーブで動作します。

    -----------------------------------------

    verbose (boolean): スタティックに設定されているかどうかに関わらず、すべてのカーブを出力します。

    -----------------------------------------

    Return Value:
    None: intスタティック状態が変更されたカーブの数です。
    """
    pass
...
def ogs(deviceInformation: bool = False,disposeReleasableTextures: bool = False,dumpTexture: str = "",enableHardwareInstancing: bool = False,fragmentEditor: str = "",fragmentXML: str = "",gpuMemoryTotal: int = 1,gpuMemoryUsed: bool = False,isLegacyViewportEnabled: bool = False,isRemoteGLSessionEnabled: bool = False,isWinRemoteSession: bool = False,pause: bool = False,rebakeTextures: bool = False,regenerateUVTilePreview: str = "",reloadTextures: bool = False,reset: bool = False,shaderSource: str = "",toggleTexturePaging: bool = False,traceRenderPipeline: bool = False) -> None:
    ...
    gpuMemoryTotal (int): Mayaが使用できるGPUメモリの合計容量を取得または設定します(MB単位)。
    ...
    isRemoteGLSessionEnabled (boolean): リモートGLが許可されているかどうかを照会します。
...
def openCLInfo(minVertexBuffer: bool = False,valid: bool = False) -> None:
    """
    OpenCL 情報を照会します。
    ...
    minVertexBuffer (boolean): 照会モードのみで有効です。頂点バッファの最小サイズを返します。

    -----------------------------------------

    valid (boolean): 照会モードのみで有効です。OpenCLが初期化されているかどうかを確認します。

    -----------------------------------------

    Return Value:
    None: booleanOpenCL が初期化されているかどうかを示す状態(「valid」フラグを指定)照会モードでは、戻り値のタイプは照会されたフラグに基づきます。
    """
    pass
...
def profiler(addCategory: str = "",allCategories: bool = False,bufferSize: int = 1,categoryIndex: int = 1,categoryIndexToName: int = 1,categoryInfo: str = "",categoryName: str = "",categoryNameToIndex: str = "",categoryRecording: bool = False,clearAllMelInstrumentation: bool = False,colorIndex: int = 1,eventCPUId: bool = False,eventCategory: bool = False,eventColor: bool = False,eventCount: bool = False,eventDescription: bool = False,eventDuration: bool = False,eventIndex: int = 1,eventName: bool = False,eventStartTime: bool = False,eventThreadId: bool = False,instrumentMel: bool = False,load: str = "",output: str = "",procedureDescription: str = "",procedureName: str = "",removeCategory: str = "",reset: bool = False,sampling: bool = False,signalEvent: bool = False,signalMelEvent: bool = False) -> None:
    ...
    allCategories (boolean): すべてのカテゴリの名前を照会します。categoryInfoフラグが設定されている場合は、カテゴリの名前とカテゴリの説明を交互に使用します。
    ...
    categoryInfo (string): addCategoryフラグと一緒に使用する場合は、追加されたプロファイルカテゴリの説明を設定します。照会モードで、categoryIndexまたはcategoryNameフラグで参照されているカテゴリの説明を返します。照会モードでは、このフラグは値を受け入れることができます。
...
def profilerTool(categoryView: bool = False,collapseSelectedEvents: bool = False,collapseSelectedEventsRepetition: bool = False,cpuView: bool = False,destroy: bool = False,eventTypes: bool = False,exists: bool = False,expandSelectedEvents: bool = False,expandSelectedEventsRepetition: bool = False,findNext: bool = False,findPrevious: bool = False,frameAll: bool = False,frameSelected: bool = False,isolateSegment: int = 1,make: bool = False,matchWholeWord: bool = False,searchEvent: str = "",segmentCount: bool = False,showAllEvent: bool = False,showCriticalPath: bool = False,showHotspot: bool = False,showSelectedEvents: bool = False,showSelectedEventsRepetition: bool = False,threadView: bool = False,unisolateSegment: bool = False) -> None:
    """
    ...
    eventTypes (boolean): 既存のイベントのイベントタイプリストを含むJSONデータを返します。フラグの値がtrueの場合は、選択したイベントのイベントタイプのみを表示します。それ以外の場合は、すべてのイベントのイベントタイプを表示します。返されるJSON文字列には、{"eventSummary":[{"type":EVENT_TYPE_NAME,,"description":EVENT_TYPE_DESCRIPTION,,"color":[RED_AS_FLOAT,GREEN_AS_FLOAT,BLUE_AS_FLOAT],"category":CATEGORY_NAME,"count":EVENT_TYPE_COUNT}]}の形式のイベントタイプ情報が含まれます。"type"および"description"は省略でき、省略した場合は、結果が匿名イベントに対応していることを示します。
    ...
    showHotspot (boolean): 選択したフレームのホットスポットを表示する
