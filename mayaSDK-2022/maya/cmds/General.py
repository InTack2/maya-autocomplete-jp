...
def about(apiVersion: bool = False,application: bool = False,batch: bool = False,buildDirectory: bool = False,buildVariant: bool = False,codeset: bool = False,compositingManager: bool = False,connected: bool = False,ctime: bool = False,currentDate: bool = False,currentTime: bool = False,customVersion: bool = False,customVersionClient: bool = False,customVersionMajor: bool = False,customVersionMinor: bool = False,customVersionString: bool = False,cutIdentifier: bool = False,date: bool = False,environmentFile: bool = False,evalVersion: bool = False,file: bool = False,fontInfo: bool = False,helpDataDirectory: bool = False,installedVersion: bool = False,ioVersion: bool = False,irix: bool = False,is64: bool = False,languageResources: bool = False,linux: bool = False,linux64: bool = False,liveUpdate: bool = False,localizedResourceLocation: bool = False,ltVersion: bool = False,macOS: bool = False,macOSppc: bool = False,macOSx86: bool = False,majorVersion: bool = False,minorVersion: bool = False,ntOS: bool = False,operatingSystem: bool = False,operatingSystemVersion: bool = False,patchVersion: bool = False,preferences: bool = False,product: bool = False,qtVersion: bool = False,tablet: bool = False,tabletMode: bool = False,uiLanguage: bool = False,uiLanguageForStartup: bool = False,uiLanguageIsLocalized: bool = False,uiLocaleLanguage: bool = False,version: bool = False,win64: bool = False,windowManager: bool = False,windows: bool = False) -> None:
    """
    ...
    customVersion (boolean): これがカスタムバージョンのMayaである場合はtrueを返します。

    -----------------------------------------

    customVersionClient (boolean): Mayaのカスタムクライアントバージョンの文字列を返します。カスタムバージョンでない場合は空白の文字列を返します。

    -----------------------------------------

    customVersionMajor (boolean): Mayaのカスタムメジャーバージョンの文字列を返します。カスタムバージョンでない場合は文字列0(ゼロ)を返します。

    -----------------------------------------

    customVersionMinor (boolean): Mayaのカスタムマイナーバージョンの文字列を返します。カスタムバージョンでない場合は文字列0(ゼロ)を返します。

    -----------------------------------------

    customVersionString (boolean): Mayaのカスタムバージョンの文字列を返します。カスタムバージョンでない場合は空白の文字列を返します。
    ...
    liveUpdate (boolean): このフラグは非推奨になりました(リリース2019)。Mayaの将来のリリースで除去される可能性があります。オートデスクのフォーマットされた製品情報を返します。
    ...
    majorVersion (boolean): Mayaのメジャーバージョンを返します。

    -----------------------------------------

    minorVersion (boolean): Mayaのマイナーバージョンを返します。
    ...
    patchVersion (boolean): Mayaのパッチバージョンを返します。
...
def assembly(active: str = "",activeLabel: str = "",canCreate: str = "",createOptionBoxProc: str = "",createRepresentation: str = "",defaultType: str = "",deleteRepresentation: str = "",deregister: str = "",input: str = "",isAType: str = "",isTrackingMemberEdits: str = "",label: str = "",listRepTypes: bool = False,listRepTypesProc: str = "",listRepresentations: bool = False,listTypes: bool = False,name: str = "",newRepLabel: str = "",postCreateUIProc: str = "",proc: str = "",renameRepresentation: str = "",repLabel: str = "",repName: str = "",repNamespace: str = "",repPostCreateUIProc: str = "",repPreCreateUIProc: str = "",repType: str = "",repTypeLabel: str = "",repTypeLabelProc: str = "",type: str = "") -> None:
    """
    ...
    active (string): アクティブリプリゼンテーションを名前で設定するか、またはアクティブリプリゼンテーションの名前を照会します。編集モードは複数のアセンブリに適用することができます。照会モードでは、単一のアセンブリのみが指定されている場合は単一の文字列を返し、複数のアセンブリが指定されている場合は文字列の配列を返します。名前として空の文字列を使用することは、現在のアクティブリプリゼンテーションを非アクティブにすることを意味します。

    -----------------------------------------

    activeLabel (string): アクティブリプリゼンテーションをラベルで設定するか、またはアクティブリプリゼンテーションのラベルを照会します。編集モードは複数のアセンブリに適用することができます。照会モードでは、単一のアセンブリのみが指定されている場合は単一の文字列を返し、複数のアセンブリが指定されている場合は文字列の配列を返します。

    -----------------------------------------

    canCreate (string): 特定のアセンブリが作成できるリプリゼンテーションタイプを照会します。

    -----------------------------------------

    createOptionBoxProc (script): 特定のアセンブリタイプのオプションボックスメニュープロシージャを設定または照会します。このアセンブリタイプは、-typeフラグを使用してアセンブリタイプを明示的に指定しない限り、既定のタイプになります。

    -----------------------------------------

    createRepresentation (string): アセンブリに対して特定のリプリゼンテーションタイプを作成および追加します。リプリゼンテーションタイプが追加のパラメータを必要とする場合は、「input」フラグを使用して指定する必要があります。たとえば、Mayaシーンアセンブリリファレンス実装のキャッシュおよびシーンリプリゼンテーションは入力ファイルを必要とします。

    -----------------------------------------

    defaultType (string): 既定のアセンブリのタイプを設定または照会します。assemblyコマンドを使用してアセンブリオブジェクトではなくアセンブリタイプで操作を実行する場合、-typeフラグを使用してアセンブリタイプを明示的に指定しない限り、既定のタイプで実行されます。

    -----------------------------------------

    deleteRepresentation (string): 特定のリプリゼンテーションをアセンブリから削除します。

    -----------------------------------------

    deregister (string): 登録されているアセンブリタイプを登録解除します。登録解除したタイプが既定のタイプである場合、既定のタイプは空の文字列に設定されます。

    -----------------------------------------

    input (string): リプリゼンテーション作成時のリプリゼンテーション作成プロシージャの追加パラメータを指定します。このフラグはcreateRepresentationフラグと一緒に使用する必要があります。

    -----------------------------------------

    isAType (string): 指定したオブジェクトがアセンブリタイプかどうかを照会します。

    -----------------------------------------

    isTrackingMemberEdits (string): 指定したオブジェクトがメンバー編集をトラッキングするかどうかを照会します。

    -----------------------------------------

    label (string): アセンブリタイプのラベルを設定または照会します。アセンブリタイプはフラグ「type」で指定されます。タイプが指定されていない場合、既定のタイプが使用されます。

    -----------------------------------------

    listRepTypes (boolean): 指定したアセンブリタイプでサポートされているリプリゼンテーションタイプを照会します。このアセンブリタイプは、-typeフラグを使用してアセンブリタイプを明示的に指定しない限り、既定のタイプになります。

    -----------------------------------------

    listRepTypesProc (script): アセンブリタイプでサポートされるリプリゼンテーションタイプのリストを提供するプロシージャを設定または照会します。この手順では引数を使用せず、このアセンブリタイプで作成できるリプリゼンテーションタイプの完全なセットを表すリプリゼンテーションタイプの文字列配列を返します。このプロシージャが適用されるアセンブリタイプは、typeフラグを使用してアセンブリタイプを明示的に指定しない限り、既定のタイプになります。

    -----------------------------------------

    listRepresentations (boolean): 特定のアセンブリの作成したリプリゼンテーションリストを照会します。-repTypeフラグを使用すると、リストをフィルタし、1つのリプリゼンテーションタイプのリプリゼンテーションを返すことができます。-repTypeフラグを使用しない場合、作成したすべてリプリゼンテーションが返されます。

    -----------------------------------------

    listTypes (boolean): サポートされているアセンブリタイプを照会します。

    -----------------------------------------

    name (string): アセンブリの作成時の名前を指定します。

    -----------------------------------------

    newRepLabel (string): リプリゼンテーションラベルの編集時に設定するリプリゼンテーションラベルを指定します。

    -----------------------------------------

    postCreateUIProc (script): 指定したアセンブリタイプのUI作成後プロシージャを設定または照会します。このプロシージャは、指定したタイプのアセンブリがスクリプトではなくUIから作成されるとすぐにMayaによって起動されます。このプロシージャを使用してダイアログを起動し、新しく作成したアセンブリの初期パラメータを取得および設定することができます。このアセンブリタイプは、-typeフラグを使用してアセンブリタイプを明示的に指定しない限り、既定のタイプになります。

    -----------------------------------------

    proc (script): 指定したアセンブリタイプのリプリゼンテーションUI作成後または作成前プロシージャを設定するときのプロシージャを指定します。このアセンブリタイプは、-typeフラグを使用してアセンブリタイプを明示的に指定しない限り、既定のタイプになります。

    -----------------------------------------

    renameRepresentation (string): このフラグの引数であるリプリゼンテーションの名前を変更します。新しい名前を提供するにはrepNameフラグを使用しなければなりません。

    -----------------------------------------

    repLabel (string): 指定したアセンブリについて、このフラグの引数であるリプリゼンテーションのラベルを照会または編集します。照会モードと編集モードの両方で、-repLabelフラグはリプリゼンテーションの名前を指定します。編集モードでは、-newRepLabelフラグを使用して新しいリプリゼンテーションラベルを指定する必要があります。照会モードでは、このフラグに値が必要になります。

    -----------------------------------------

    repName (string): リプリゼンテーションの作成または名前変更時に設定するリプリゼンテーション名を指定します。このフラグはcreateRepresentationフラグの場合にはオプションです。省略すると、アセンブリはリプリゼンテーションに名前を付けます。これはrenameRepresentationフラグの場合には必須です。

    -----------------------------------------

    repNamespace (string): このアセンブリノードのリプリゼンテーションのネームスペースを照会します。返される値は、リプリゼンテーションのアクティブ化によって作成されたノードが追加されるネームスペースを作成するためにMayaで使用されます。ネームスペースが親ネームスペースに追加されたときに名前の衝突が発生した場合、Mayaは新しい名前でrepNamespaceを更新します。アセンブリノードを処理するときは、(このフラグが影響しない、または照会しない)アセンブリノード自体のネームスペースと、そのリプリゼンテーションのネームスペースの2つのネームスペースが関係します。リプリゼンテーションのネームスペースは、アセンブリノードの子のネームスペースです。アセンブリノードのネームスペースは、それに含まれるアセンブリによって設定されるか(ネストされている場合)、最上位のファイルによって設定されます。アセンブリノードのネームスペースとリプリゼンテーションのネームスペースのいずれかまたは両方は、空の文字列にすることができます。アセンブリノードがネストされている場合、アセンブリノードのネームスペースは(そのネストにより)それに含まれるアセンブリのリプリゼンテーションのネームスペースになります。

    -----------------------------------------

    repPostCreateUIProc (string): 特定のリプリゼンテーションタイプ、および特定のアセンブリタイプのUI作成後プロシージャを設定または照会します。このプロシージャは2つの引数を取ります。1つはアセンブリへのDAGパス、もう1つはリプリゼンテーションの名前です。値は返しません。指定したタイプのリプリゼンテーションがスクリプトからではなくUIから作成されるとすぐにMayaによって起動されます。このプロシージャを使用してダイアログを起動し、新しく作成したリプリゼンテーションの初期パラメータを取得および設定することができます。リプリゼンテーションタイプはこのフラグの引数です。プロシージャ名を指定するには、-procフラグを使用する必要があります。このアセンブリタイプは、-typeフラグを使用してアセンブリタイプを明示的に指定しない限り、既定のタイプになります。照会モードでは、このフラグに値が必要になります。

    -----------------------------------------

    repPreCreateUIProc (string): 特定のリプリゼンテーションタイプ、および特定のアセンブリタイプのUI作成前プロシージャを設定または照会します。このプロシージャは引数を取らず、Mayaが-createRepresentationフラグを使用してassemblyコマンドを起動したときに-inputフラグに引数として渡される文字列を返します。リプリゼンテーション作成前プロシージャは、指定したタイプのリプリゼンテーションがスクリプトからではなくUIから作成される直前にMayaによって起動されます。このプロシージャを使用してダイアログを起動し、新しいリプリゼンテーションの作成引数を取得することができます。リプリゼンテーションタイプはこのフラグの引数です。プロシージャ名を指定するには、-procフラグを使用する必要があります。このアセンブリタイプは、-typeフラグを使用してアセンブリタイプを明示的に指定しない限り、既定のタイプになります。照会モードでは、このフラグに値が必要になります。

    -----------------------------------------

    repType (string): -listRepresentations照会でフィルタとして使用するリプリゼンテーションタイプを指定します。リプリゼンテーションタイプはこのフラグの引数です。照会モードでは、このフラグに値が必要になります。

    -----------------------------------------

    repTypeLabel (string): 特定のリプリゼンテーションタイプのラベルを照会します。照会モードでは、このフラグに値が必要になります。

    -----------------------------------------

    repTypeLabelProc (script): 指定したアセンブリタイプで、リプリゼンテーションタイプのラベルを提供するプロシージャを設定または照会します。プロシージャはリプリゼンテーションタイプを唯一の引数として取り、ローカライズされたリプリゼンテーションタイプのラベルを返します。このプロシージャが適用されるアセンブリタイプは、-typeフラグを使用してアセンブリタイプを明示的に指定しない限り、既定のタイプになります。

    -----------------------------------------

    type (string): 指定した登録されているアセンブリタイプのプロパティを設定または照会します。照会モードでは、このフラグに値が必要になります。
...
def baseTemplate(exists: bool = False,fileName: str = "",force: bool = False,load: bool = False,matchFile: str = "",silent: bool = False,unload: bool = False,viewList: str = "") -> None:
    """
    ...
    exists (boolean): 指定したテンプレートが存在するかどうかによって、trueまたはfalseを返します。matchFile引数と一緒に使用する際、テンプレートが存在し、ロード元のファイル名が指定のファイル名と一致する場合、照会はtrueを返します。

    -----------------------------------------

    fileName (string): テンプレートに関連付けるファイル名を指定します。この引数は、ロード、保存、または照会モードと一緒に使用できます。テンプレートに関連付けられた名前がない場合は、テンプレート名に基づいた既定のファイル名が使用されます。ファイル名とテンプレート名を対応させることをお勧めしますが、必須ではありません。

    -----------------------------------------

    force (boolean): このフラグは他のアクションとともに使用して、上書きや破壊的な操作を実行できるようにします。ロードで使用すると、ファイルから既存のテンプレートを再ロードできるようになります。作成モードで使用すると、既存のテンプレートを再作成することができるようになります(fromContainer引数を使用してテンプレートを再生成する場合など)。

    -----------------------------------------

    load (boolean): ファイルから既存のテンプレートをロードします。テンプレートに対するファイル名を指定すると、ファイル全体(つまりファイル内のすべてのテンプレート)がロードされます。ファイル名を指定しないと、テンプレート名に基づいて、既定のファイル名が使用されます。

    -----------------------------------------

    matchFile (string): このフラグは、他のフラグと一緒に使用して照会モードで使用し、照会操作の一部として一致させるオプションのファイル名を指定します。照会モードでは、このフラグに値が必要になります。

    -----------------------------------------

    silent (boolean): サイレントモードは、通常はコマンドの実行でレポートされるすべてのエラーや警告メッセージを出力しないようにします。戻り値には影響しません。

    -----------------------------------------

    unload (boolean): 指定したテンプレートをアンロードします。このアクションは、関連するテンプレートファイルを削除するのではなく、単にテンプレート定義を現在のセッションから除去するだけです。

    -----------------------------------------

    viewList (string): 照会モードで使用するこのフラグは、テンプレートに定義されたすべてのビューのリストを返します。
...
def baseView(itemInfo: str = "",itemList: bool = False,viewDescription: bool = False,viewLabel: bool = False,viewList: bool = False,viewName: str = "") -> None:
    """
    ビューは、特定のノード タイプまたはコンテナのアトリビュートのレイアウト情報を定義します。ビューは、ビルトインのビューのセットから選択するか、関連付けられたコンテナ テンプレート上で定義することができます。このコマンドは、コンテナ ノードまたは指定されたテンプレートのビュー関連情報を照会します。このコマンドが返す情報は、照会時にコンテナ ノード上で実行されているビュー関連設定(コンテナのビュー モード、テンプレート名、ビュー名のアトリビュートなど)に基づきます。
    ...
    itemInfo (string): itemListフラグと組み合わせて照会モードで使用します。ビューの各項目の情報のリストを返すコマンドで、この引数値によって各項目について返す情報フィールドを定義します。情報フィールドは返される文字配列にリスト表示されます。キーワードを指定する順番によって、コマンドが返すデータの順番が決まります。コロン(:)で区切られる、次の1つまたは複数のキーワードを使用して引数の値を指定します。itemIndex:連続した項目番号(0から開始)itemName:項目名(文字列)itemLabel:項目の表示ラベル(文字列)itemDescription:項目の説明フィールド(文字列)itemLevel:項目の階層レベル(0～n)itemIsGroup:(ブーリアン0または1)はこの項目がグループかどうかを示します。itemIsAttribute:(ブーリアン0または1)はこの項目がアトリビュートかどうかを示します。itemNumChildren:この項目の直接の子(グループまたはアトリビュート)の数itemAttrType:項目のアトリビュートタイプ(文字列)itemCallback:項目のコールバックフィールド(文字列)照会モードでは、このフラグに値が必要になります。

    -----------------------------------------

    itemList (boolean): 照会モードで使用して、ビューの各項目に関する情報のリストを返します。照会するビューを選択するには、viewNameフラグを使用します。各項目について返す情報は、itemInfo引数値で定義します。効率性の点では、必要なすべての項目の情報を一度に照会するのが最良です(照会するごとにビュー情報が再計算されるのを避けるため)。

    -----------------------------------------

    viewDescription (boolean): 照会モードで使用され、選択されたビューに関連付けられた説明フィールドを返します。このビューに対して説明が定義されていない場合、値は空です。

    -----------------------------------------

    viewLabel (boolean): 照会モードで使用して、ビューと関連付ける表示ラベルを返します。選択されたビューに基づき、ユーザインタフェースに適切なラベルを返します。表示目的の場合はビュー名よりもビューラベルの使用が適しています。

    -----------------------------------------

    viewList (boolean): 照会モードで使用され、指定のターゲット(コンテナまたはテンプレート)に対して定義されたすべてのビューのリストを返します。

    -----------------------------------------

    viewName (string): 照会モードで使用し、テンプレートターゲットと併せて使用すると、照会されたビューの名前が指定されます。コンテナターゲットと併せて使用される場合は、文字列引数を必要とせず、コンテナに関連付けられた現在アクティブなビューの名前を返します。現在選択しているビューが有効なテンプレートビューではない場合、またはいずれかのビルトインビューモードで生成されている場合は、この値が空になる場合があります。このため、表示目的の場合はビューラベルがより適切です。照会モードでは、このフラグは値を受け入れることができます。
...
def colorManagementFileRules(addRule: str = "",colorSpace: str = "",colorSpaceDescription: str = "",colorSpaceFamilies: str = "",colorSpaceNames: bool = False,down: str = "",enabled: bool = False,evaluate: str = "",extension: str = "",listRules: bool = False,load: bool = False,moveUp: str = "",pattern: str = "",remove: str = "",restoreDefaults: bool = False,save: bool = False) -> None:
    """
    ...
    colorSpaceDescription (string): 特定のカラースペースの説明を返します。照会モードでは、このフラグに値が必要になります。

    -----------------------------------------

    colorSpaceFamilies (string): 特定のカラースペースのファミリのリストを返します。ルールのUIポップアップのカラースペースを設定する場合に、サブメニューを追加するために使用されます。照会モードでは、このフラグに値が必要になります。

    -----------------------------------------

    colorSpaceNames (boolean): 使用可能なカラースペースのリストを返します。ルールのUIポップアップのカラースペースを設定するために使用されます。
...
def colorManagementPrefs(cmConfigFileEnabled: bool = False,cmEnabled: bool = False,colorManageAllNodes: bool = False,colorManagePots: bool = False,colorManagedNodes: bool = False,colorManagementSDKVersion: str = "",configFilePath: str = "",configFileVersion: str = "",defaultInputSpaceName: str = "",displayName: str = "",displayNames: bool = False,equalsToPolicyFile: str = "",exportPolicy: str = "",inhibitEvents: bool = False,inputSpaceDescription: str = "",inputSpaceFamilies: str = "",inputSpaceNames: bool = False,loadPolicy: str = "",loadedDefaultInputSpaceName: str = "",loadedDisplayName: str = "",loadedOutputTransformName: str = "",loadedRenderingSpaceName: str = "",loadedViewName: str = "",loadedViewTransformName: str = "",missingColorSpaceNodes: bool = False,ocioRulesEnabled: bool = False,ociov2Enabled: bool = False,outputTarget: str = "",outputTransformEnabled: bool = False,outputTransformName: str = "",outputTransformNames: bool = False,outputTransformUseColorConversion: bool = False,outputUseViewTransform: bool = False,policyFileName: str = "",popupOnError: bool = False,refresh: bool = False,renderingSpaceName: str = "",renderingSpaceNames: bool = False,restoreDefaults: bool = False,viewName: str = "",viewNames: bool = False,viewTransformName: str = "",viewTransformNames: bool = False) -> None:
    """
    ...
    colorManageAllNodes (boolean): ファイルテクスチャノードなどのすべての入力ノードにカラー管理を追加します。
    ...
    configFileVersion (string): 設定バージョンのバージョンを取得します。

    -----------------------------------------

    defaultInputSpaceName (string): このフラグは現在サポートされていません。詳細については、colorManagementFileRulesコマンドを参照してください。

    -----------------------------------------

    displayName (string): カラー管理されたビューアとカラー管理されたUIコントロールによって適用される、(表示、ビュー)ペアの中の表示。

    -----------------------------------------

    displayNames (boolean): 使用可能な表示のリストを返します。カラー管理プリファレンスUIポップアップの入力に使用されます。
    ...
    inhibitEvents (boolean): カラー管理設定を変更しているときに発生するクライアント/サーバ間の通知およびイベントトリガを禁止します。

    -----------------------------------------

    inputSpaceDescription (string): 特定の入力カラースペースの説明を返します。照会モードでは、このフラグに値が必要になります。

    -----------------------------------------

    inputSpaceFamilies (string): 特定の入力カラースペースのファミリのリストを返します。入力カラースペースUIポップアップに入力するときにサブメニューを追加するために使用されます。照会モードでは、このフラグに値が必要になります。
    ...
    loadedDefaultInputSpaceName (string): このフラグは現在サポートされていません。

    -----------------------------------------

    loadedDisplayName (string): (表示、ビュー)ペアから、ロードされた表示を取得します。ファイルを開く、読み込み、参照で使用され、見つからないカラースペースやトランスフォームを確認します。
    ...
    loadedViewName (string): (表示、ビュー)ペアから、ロードされたビューを取得します。ファイルを開く、読み込み、参照で使用され、見つからないカラースペースやトランスフォームを確認します。
    ...
    ociov2Enabled (boolean): 既定では、OCIOv2カラー管理システムです。
    ...
    viewName (string): カラー管理されたビューアとカラー管理されたUIコントロールによって適用される、(表示、ビュー)ペアの中のビュー。

    -----------------------------------------

    viewNames (boolean): 選択した表示から使用可能なビューのリストを返します。カラー管理プリファレンスUIポップアップの入力に使用されます。
...
def commandLogging(historySize: int = 1,logCommands: bool = False,logFile: str = "",recordCommands: bool = False,resetLogFile: bool = False) -> None:
    """
    ...
    historySize (uint): メモリ内のコマンドヒストリにおけるエントリ数を設定します。

    -----------------------------------------

    logCommands (boolean): コマンドのディスク上でのロギングを有効または無効にします。

    -----------------------------------------

    logFile (string): ディスク上のログに使用するファイル名を設定します。ロギングがアクティブな場合、現在のファイルは、新しいファイルが開かれる前に閉じられます。

    -----------------------------------------

    recordCommands (boolean): コマンドのメモリ内でのロギングを有効または無効にします。

    -----------------------------------------

    resetLogFile (boolean): 既定のログファイル名(「Maya.env」および既定のプロジェクトフォルダとともにアプリケーションフォルダにある「mayaCommandLog.txt」)にリセットします。
...
def commandPort(bufferSize: int = 1,close: bool = False,echoOutput: bool = False,listPorts: bool = False,name: str = "",noreturn: bool = False,pickleOutput: bool = False,prefix: str = "",returnNumCommands: bool = False,securityWarning: bool = False,sourceType: str = "") -> None:
    """
    ...
    bufferSize (int): コマンドや結果は、それぞれサイズ制限に従います。このオプションを使用すると、Mayaとの通信に使用するバッファサイズを指定できます。指定がない場合の既定のバッファサイズは、4096文字です。コマンドがbufferSizeの文字より長いと、クライアント接続は閉じられます。bufferSize文字より長い結果は、エラーメッセージに置き換えられます。

    -----------------------------------------

    close (boolean): commandPortを閉じ、パイプを削除します。

    -----------------------------------------

    echoOutput (boolean): すべてのコマンド出力のコピーがコマンドポートに送信されます。一般的には、結果だけが転送されます。このオプションは、すべての出力のコピーを提供します。

    -----------------------------------------

    listPorts (boolean): 使用可能なポートを返します。

    -----------------------------------------

    name (string): このコマンドが作成するコマンドポートの名前を指定します。name形式のCommandPort名は、nameに対応するローカルホストのUNIXドメインソケットを作成します。nameが「/」で始まらない場合は、/tmp/nameが使用されます。nameが「/」で始まる場合、nameはソケットのフルパスを示します。:portnumber形式の名前は、指定したポートのローカルホスト上にINETドメインを作成します。

    -----------------------------------------

    noreturn (boolean): コマンドポートソケットには実行したコマンドからの結果を書き込みません。代わりに、実行したコマンドからの結果をスクリプトエディタ(ScriptEditor)エディタウィンドウに書き込みます。提出したコマンドの実行に関連する情報がコマンドポートクライアントに戻されないとき、コマンドバッファをオーバーフローさせないように注意する必要があります。オーバーフローさせると接続が閉じられます。

    -----------------------------------------

    pickleOutput (boolean): Pythonの出力はpickle化されます。

    -----------------------------------------

    prefix (string): 文字列引数は、1つの文字列引数を取るMayaコマンドの名前です。このコマンドは、時間データがコマンドポートに送信されるたびにコールされます。コマンドポートに書き込まれたデータは、prefixコマンドに引数として渡されます。コマンドポートからのデータは、enocodeStringと同様にエンコードされ、引用符で囲まれます。コマンドポートデータに改行文字が埋め込まれている場合は、その入力は個別のラインに分割されます。これらのラインは、コマンドポートへの別々の書き込みであるかのように取り扱われます。最後のprefixコマンドに対する結果だけが返されます。

    -----------------------------------------

    returnNumCommands (boolean): コマンドの結果を無視し、このコールで読み取って実行されたコマンドの数を返します。これは、バッファのオーバーフローを追跡する簡単な方法です。noreturnフラグを指定すると、このフラグは無視されます。

    -----------------------------------------

    securityWarning (boolean): コマンドポートの入力のセキュリティ警告を有効化します。

    -----------------------------------------

    sourceType (string): この文字列引数は、commandPortに渡される「mel」、「python」などのソースタイプを示すのに使用されます。既定のソースタイプは「mel」です。

    -----------------------------------------

    Return Value:
    None: boolean- 照会モードで照会モードでは、戻り値のタイプは照会されたフラグに基づきます。
    """
    pass

    
def container(addNode: Tuple[str] = tuple(""),asset: Tuple[str] = tuple(""),assetMember: str = "",bindAttr: Tuple[str, str] = tuple("", ""),connectionList: bool = False,current: bool = False,fileName: Tuple[str] = tuple(""),findContainer: Tuple[str] = tuple(""),force: bool = False,includeHierarchyAbove: bool = False,includeHierarchyBelow: bool = False,includeNetwork: bool = False,includeNetworkDetails: str = "",includeShaders: bool = False,includeShapes: bool = False,includeTransform: bool = False,isContainer: bool = False,name: str = "",nodeList: bool = False,nodeNamePrefix: bool = False,parentContainer: bool = False,preview: bool = False,publishAndBind: Tuple[str, str] = tuple("", ""),publishAsChild: Tuple[str, str] = tuple("", ""),publishAsParent: Tuple[str, str] = tuple("", ""),publishAsRoot: Tuple[str, bool] = tuple("", False),publishAttr: str = "",publishConnections: bool = False,publishName: str = "",removeContainer: bool = False,removeNode: Tuple[str] = tuple(""),type: str = "",unbindAndUnpublish: str = "",unbindAttr: Tuple[str, str] = tuple("", ""),unbindChild: str = "",unbindParent: str = "",unpublishChild: str = "",unpublishName: str = "",unpublishParent: str = "",unsortedOrder: bool = False) -> None:
    """
    ...
    addNode (string[]): コンテナに追加するノードのリストを指定します。

    -----------------------------------------

    asset (string[]): 照会すると、nodeListのすべてのノードが同じコンテナに属する場合は、コンテナの名前を返します。それ以外の場合、空の文字列を返します。このフラグは機能的にはfindContainerフラグと同等です。

    -----------------------------------------

    assetMember (string): 照会時にbindAttrフラグと同時に使用することで、指定したコンテナ内ノードに関連するパブリッシュ済みアトリビュートのみを照会できます。照会モードでは、このフラグに値が必要になります。

    -----------------------------------------

    bindAttr ([string, string]): 内蔵されたアトリビュートを、コンテナのインタフェース上にある、バインドされていないパブリッシュ済みの名前にバインドします。戻り値は、バインドされたパブリッシュ済みの名前です。1番目の文字列は、ノード名とアトリビュート名をバインドするように「node.attr」フォーマットで指定します。2番目の文字列は、バインドされていないパブリッシュ済みの名前を指定します。照会モードでは、パブリッシュ済みの名前の文字配列とそれに対応するアトリビュートを返します。また、このフラグは、-publishName、-publishAsParent、-publishAsChildフラグと同時に照会モードで使用することもできます。

    -----------------------------------------

    connectionList (boolean): コンテナノードに対する外部接続のリストを返します。

    -----------------------------------------

    current (boolean): 作成モードでは、新たに作成されたアセットは常に現在になるように指定します。編集モードでは、選択したアセットを現在に設定します。照会では、現在のアセットを返します。

    -----------------------------------------

    fileName (string[]): 指定したファイル名と関連付けられているアセットの照会に使用されます。照会モードでは、このフラグに値が必要になります。

    -----------------------------------------

    findContainer (string[]): 照会すると、nodeListのすべてのノードが同じコンテナに属する場合は、コンテナの名前を返します。それ以外の場合、空の文字列を返します。照会モードでは、このフラグに値が必要になります。

    -----------------------------------------

    force (boolean): このフラグは、-addNodeフラグおよび-removeNodeフラグとしか使用できません。-addNodeを指定すると、ノードは、新しいコンテナに追加される前に、現在のコンテナから接続解除されます。-removeNodeを指定すると、ノードは、ネストされたコンテナから除去する場合、親コンテナに残るのではなく、すべてのコンテナから除去されます。

    -----------------------------------------

    includeHierarchyAbove (boolean): このフラグを使用して、提供されたノードリストの親階層もコンテナに含むこと、あるいはコンテナから削除することを指定します。階層の算入は、他のコンテナのメンバーになっているノードで停止します。

    -----------------------------------------

    includeHierarchyBelow (boolean): このフラグを使用して、提供されたノードリストの下階層もコンテナに含むこと、あるいはコンテナから削除することを指定します。階層の算入は、他のコンテナのメンバーになっているノードで停止します。

    -----------------------------------------

    includeNetwork (boolean): 提供済みノードリストに接続されたノードネットワークもコンテナに含まれることを指定します。ネットワーク走査は、既定のノードと、他のコンテナのメンバーになっているノードで停止します。

    -----------------------------------------

    includeNetworkDetails (string): このフラグを使用して、含めるネットワークの特定部分を指定します。。このフラグで有効な引数は次の通りです。「channels」、「sdk」、「constraints」、「history」、「expressions」、「inputs」、「outputs」。このフラグとincludeNetworkフラグの違いは、タイプに関係なく接続されたすべてのノードが含まれることです。DAGコンテナにはその子が含まれるため、コンストレイントが入力として指定されていない場合でも、常に選択したオブジェクトにペアレント化されたコンストレイントノードが含まれます。

    -----------------------------------------

    includeShaders (boolean): 含まれている任意のシェイプに対して、そのシェーダもコンテナに含まれることを指定します。

    -----------------------------------------

    includeShapes (boolean): このフラグを使用して、選択した任意のトランスフォームの直下にある子シェイプもコンテナに含むこと、あるいはコンテナから削除することを指定します。このフラグはincludeHierarchyBelowが使用されている場合は必要ありません。この場合、子シェイプとそのすべての子が自動的に含まれるからです。

    -----------------------------------------

    includeTransform (boolean): このフラグを使用して、選択した任意のシェイプの親トランスフォームもコンテナに含むこと、あるいはコンテナから削除することを指定します。このフラグはincludeHierarchyAboveが使用されている場合は必要ありません。この場合、親トランスフォームとそのすべての親が自動的に含まれるからです。

    -----------------------------------------

    isContainer (boolean): 選択または指定されたノードがコンテナノードであれば、trueを返します。複数のコンテナが照会されれば、1番目の状態のみが返されます。

    -----------------------------------------

    name (string): 新しく作成するコンテナの名前を設定します。

    -----------------------------------------

    nodeList (boolean): 照会すると、コンテナ内のノードのリストを返します。リストはコンテナに追加された順序でソートされます。また、reorderContainerコマンドによって行われたすべての順序の変更も表示します。

    -----------------------------------------

    nodeNamePrefix (boolean): パブリッシュ済みアトリビュートの名前を形式「node_attr」にすることを指定します。-publishConnections/-pcフラグとともに使用します。

    -----------------------------------------

    parentContainer (boolean): 指定したコンテナの親コンテナを照会するためのフラグです。

    -----------------------------------------

    preview (boolean): このフラグは作成モードでのみ有効です。これはコンテナを作成せず、内容をプレビューすることを示します。このフラグを使用すると、コンテナを作成した場合にコンテナに配置されるノードが選択されます。たとえば、-includeNetworkでコンテナに含まれる内容を確認し、選択内容を希望どおりに修正してから、選択したオブジェクトに対してのみcreatecontainerを実行できます。

    -----------------------------------------

    publishAndBind ([string, string]): 指定した名前をパブリッシュし、指定した名前にアトリビュートをバインドします。1番目の文字列は、ノード名とアトリビュート名を「node.attr」フォーマットで指定します。2番目の文字列は、同時にパブリッシュする名前を指定します。

    -----------------------------------------

    publishAsChild ([string, string]): 内蔵されたノードをコンテナのインタフェースに対してパブリッシュすることで、このノードが外部ノードの子となれることを示します。2番目の文字列は、パブリッシュ済みのノード名です。照会モードでは、パブリッシュ済みの名前とそれに対応するノードの文字列を返します。-publishNameフラグを照会モードで使用すると、パブリッシュ済みの名前のみを返します。-bindAttrフラグを照会モードで使用すると、パブリッシュ済みのノード名のみを返します。

    -----------------------------------------

    publishAsParent ([string, string]): 内蔵されたノードをコンテナのインタフェースに対してパブリッシュすることで、このノードが外部ノードの親となれることを示します。2番目の文字列は、パブリッシュ済みのノード名です。照会モードでは、パブリッシュ済みの名前とそれに対応するノードの文字配列を返します。-publishNameフラグを照会モードで使用すると、パブリッシュ済みの名前のみを返します。-bindAttrフラグを照会モードで使用すると、パブリッシュ済みのノード名のみを返します。

    -----------------------------------------

    publishAsRoot ([string, boolean]): ノードをルートとしてパブリッシュまたはパブリッシュ解除します。ルートトランスフォームノードは二重の意味で重要です。コンテナ中心の選択が有効な場合、ルートトランスフォームの下位階層にあるコンテナノードがメインシーンビューで選択されると、そのルートトランスフォームが選択されます。また、コンテナプロキシの書き出し時、移動、回転、スケールなど、すべてのパブリッシュ済みルート変換アトリビュートは、スタンドインノード上のアトリビュートと接続されます。照会モードでは、ルートとしてパブリッシュされたノードを返します。

    -----------------------------------------

    publishAttr (string): 照会モードでは-publishName(-pn)フラグと同時の場合のみ使用でき、アトリビュートを引数として使用します。戻り値は、アトリビュートのパブリッシュ済みの名前になります(ある場合)。照会モードでは、このフラグに値が必要になります。

    -----------------------------------------

    publishConnections (boolean): コンテナ内のノードからコンテナ外のノードへのすべての接続をパブリッシュします。

    -----------------------------------------

    publishName (string): 名前をコンテナのインタフェースに対してパブリッシュし、インタフェースに対してパブリッシュされた実際の名前を返します。照会モードでは、コンテナのパブリッシュ済みの名前を返します。-bindAttrフラグが指定されている場合、バインドされた名前のみを返します。-unbindAttrフラグが指定されている場合、バインドされていない名前のみを返します。-publishAsParent/-publishAsChildフラグが指定されている場合、パブリッシュ済みの親/子の名前のみを返します。-publishAttrが、「node.attr」フォーマットのアトリビュート引数と同時に指定されている場合、そのアトリビュートにパブリッシュ済みの名前があるときにそれを返します。

    -----------------------------------------

    removeContainer (boolean): コンテナからすべてのノードを接続解除し、コンテナノードを削除します。

    -----------------------------------------

    removeNode (string[]): コンテナから除去するノードのリストを指定します。ノードがネストされたコンテナのメンバーである場合、親コンテナに追加されます。すべてのコンテナから完全に除去するには、-forceフラグを使用します。

    -----------------------------------------

    type (string): 既定では、コンテナノードが作成されます。または、typeフラグを使用して、異なるタイプのコンテナを作成する必要があることを示すこともできます。現在のところ、コンテナノードの有効な別のタイプは「dagContainer」のみです。

    -----------------------------------------

    unbindAndUnpublish (string): 指定したアトリビュートを(「node.attr」フォーマットで)バインド解除し、接続された名前のパブリッシュを解除します。複合(Compound)をバインド解除すると、その複合の親/子の複合解除がトリガされることがあります。従って、この1つのフラグを使用する利点は、自動バインド解除に接続された名前を自動パブリッシュ解除することです。

    -----------------------------------------

    unbindAttr ([string, string]): パブリッシュ済みアトリビュートのバインドを、コンテナのインタフェースのパブリッシュ済みの名前から解除し、バインドされていないパブリッシュ済みの名前をコンテナのインタフェース上に残します。戻り値は、バインドされていないパブリッシュ済みの名前です。1番目の文字列は、ノード名とアトリビュート名をバインド解除するように「node.attr」フォーマットで指定します。2番目の文字列は、バインドされたパブリッシュ済みの名前を指定します。照会モードでは、-publishName、-publishAsParent、-publishAsChildフラグと一緒にのみ使用可能です。

    -----------------------------------------

    unbindChild (string): 子としてパブリッシュされたノードのバインドを解除しますが、コンテナのインタフェースからパブリッシュされた名前は除去しません。

    -----------------------------------------

    unbindParent (string): 親としてパブリッシュされたノードのバインドを解除しますが、コンテナのインタフェースからパブリッシュされた名前は除去しません。

    -----------------------------------------

    unpublishChild (string): 子としてパブリッシュされたノードのパブリッシュを、コンテナのインタフェースから取り消します。

    -----------------------------------------

    unpublishName (string): バインドされていない名前のパブリッシュを、コンテナのインタフェースから取り消します。

    -----------------------------------------

    unpublishParent (string): 親としてパブリッシュされたノードのパブリッシュを、コンテナのインタフェースから取り消します。

    -----------------------------------------

    unsortedOrder (boolean): このフラグはコンテナコマンド(OBSOLETE)の動作には影響しません。

    -----------------------------------------

    Return Value:
    None: string作成したノードの名前。照会モードでは、戻り値のタイプは照会されたフラグに基づきます。
    """
    pass

    
def containerBind(allNames: bool = False,bindingSet: str = "",bindingSetConditions: bool = False,bindingSetList: bool = False,force: bool = False,preview: bool = False) -> None:
    """
    ...
    allNames (boolean): コンテナ上にあるすべてのパブリッシュ済みの名前をバインド操作時に考慮するように指定します。既定では、バインドされていないパブリッシュ済みの名前のみが操作の対象になります。「all」に「force」オプションを追加で指定することで、バインド操作実行前に既存のバインドされたパブリッシュ済みの名前がリセット(またはバインド解除)されます。パブリッシュ済みの名前に対し適切なバインドがない場合は、バインドされていない状態のままになります。

    -----------------------------------------

    bindingSet (string): バインド操作や照会操作で使用するテンプレートバインドセットの名前を指定します。このフラグは照会モードでは使用できません。照会モードでは、このフラグに値が必要になります。

    -----------------------------------------

    bindingSetConditions (boolean): 照会モードで使用すると、指定したテンプレートバインドセットにあるバインドセットの条件エントリのリストが返されます。このリストには、各バインドセットのエントリに対する、すべてのパブリッシュ済みの名前と条件文字列のペアが含まれます。このフラグは接続されたバインドセットの全エントリを返し、コンテナにあるパブリッシュ済みの名前のリスト、バインド状態の有無など、各エントリの有効性は考慮しません。

    -----------------------------------------

    bindingSetList (boolean): 照会モードで使用すると、接続されたコンテナテンプレートに定義されている使用可能なバインドセットのリストが返されます。

    -----------------------------------------

    force (boolean): このフラグを使用して、通常であれば実行されない操作を強制実行します。

    -----------------------------------------

    preview (boolean): このフラグは、バインド操作のプレビューを提供しますが、操作を実際には実行しません。バインド操作の影響を受けるパブリッシュ済みの名前ごとにpublishedName/boundNameのペアのリストが返されます。バインド操作の結果、パブリッシュ済みの名前が変わらない場合はリスト表示されません。バインドされたがバインド解除されてしまうパブリッシュ済みの名前も表示され、接続されたboundNameは空文字列で示されます。
...
def containerProxy(fromTemplate: str = "",type: str = "") -> None:
    ...
    fromTemplate (string): 新しいコンテナプロキシを作成するために使用されるテンプレートファイルの名前を指定します。スタンドインアトリビュートが作成され、プロキシのすべての数値アトリビュート用にパブリッシュされます。

    -----------------------------------------

    type (string): プロキシに使用するコンテナノードのタイプを指定します。このフラグはfromTemplateフラグと一緒に使用する場合のみ有効です。既存のコンテナのプロキシを作成する場合、作成するタイプは常にソースコンテナと同じになります。既定値は「container」です。
...
def containerPublish(bindNode: Tuple[str, str] = tuple("", ""),bindTemplateStandins: bool = False,inConnections: bool = False,mergeShared: bool = False,outConnections: bool = False,publishNode: Tuple[str, str] = tuple("", ""),unbindNode: str = "",unpublishNode: str = "") -> None:
    """
    ...
    bindNode ([string, string]): 指定したノードをパブリッシュ済みのノード名にバインドします。

    -----------------------------------------

    bindTemplateStandins (boolean): このフラグは、テンプレートに存在するがバインドされていない任意のアトリビュートに対し一時的なスタンドインアトリビュートを作成し、バインドされていないアトリビュートに対して値を設定できるようになります。

    -----------------------------------------

    inConnections (boolean): 外部ノードからコンテナ内のノードへパブリッシュ解除されている接続をパブリッシュするように指定します。

    -----------------------------------------

    mergeShared (boolean): inConnectionsフラグとともに使用します。外部アトリビュートがコンテナ内の複数の内部アトリビュートに接続している場合に、パブリッシュされた単一のアトリビュートがすべての内部アトリビュートに対応して使用されることを示します。

    -----------------------------------------

    outConnections (boolean): コンテナ内のノードから外部ノードへパブリッシュ解除されている接続をパブリッシュするように指定します。

    -----------------------------------------

    publishNode ([string, string]): 名前とタイプをパブリッシュします。パブリッシュ直後は、何もバインドされていません。パブリッシュ済みの名前にノードをバインドする場合は、bindNodeフラグを使用します。

    -----------------------------------------

    unbindNode (string): フラグが指定した名前でパブリッシュされたノードのバインドを解除します。

    -----------------------------------------

    unpublishNode (string): 指定したパブリッシュ済みのノード名をパブリッシュ解除します。
...
def containerTemplate(addBindingSet: str = "",addNames: bool = False,addView: str = "",allKeyable: bool = False,attribute: str = "",attributeList: str = "",baseName: str = "",bindingSetList: str = "",childAnchor: bool = False,delete: bool = False,expandCompounds: bool = False,fromContainer: str = "",fromSelection: bool = False,layoutMode: int = 1,matchName: str = "",parentAnchor: bool = False,publishedNodeList: str = "",removeBindingSet: str = "",removeView: str = "",rootTransform: bool = False,save: bool = False,searchPath: str = "",templateList: str = "",updateBindingSet: str = "",useHierarchy: bool = False,exists: bool = False,fileName: str = "",force: bool = False,load: bool = False,matchFile: str = "",silent: bool = False,unload: bool = False,viewList: str = "") -> None:
    """
    ...
    addBindingSet (string): この引数を使用して、指定した名前の新しいバインドセットをテンプレートに追加します。既定のバインドセットが作成されます。バインドセットがすでに存在する場合、既存のバインドセットを置き換えるにはforceフラグを使用する必要があります。fromContainerオプションと一緒に使用すると、指定したコンテナの既存のバインドをもとに既定のバインドが開始されます。リファレンスコンテナなしで使用すると、バインドセットはプレースホルダエントリが含まれた状態で作成されます。新規バインドセットをテンプレートファイルに永久保存するには、テンプレートを保存する必要があります。

    -----------------------------------------

    addNames (boolean): 編集モードで、fromContainerフラグと一緒に使用する場合、コンテナ上にあるパブリッシュされた名前のうち、テンプレートのアトリビュートとして存在しないものはテンプレートに追加されます。

    -----------------------------------------

    addView (string): この引数は、新しいビューを指定した名前でテンプレートに追加するのに使用します。既定では、すべてのテンプレートアトリビュートのフラットなリストを含むビューが作成されます。layoutModeフラグでは、より詳細なレイアウトオプションを指定できます。新規ビューをテンプレートファイルと格納する前に、テンプレートを保存する必要があります。

    -----------------------------------------

    allKeyable (boolean): fromSelectionフラグがtrueでfromContainerがfalseのとき使用されます。trueの場合、すべてのキー設定可能なアトリビュートを使用してテンプレートまたはビューを定義します。falseの場合、アトリビュートフラグに渡されるアトリビュートを使用します。

    -----------------------------------------

    attribute (string): fromSelectionがtrueでallKeyableがfalseの場合、このアトリビュート名を使用し、テンプレートファイルにアトリビュート項目が作成されます。

    -----------------------------------------

    attributeList (string): 照会モードで使用して、テンプレート定義に含まれるアトリビュートのリストを返します。

    -----------------------------------------

    baseName (string): 照会モードで使用して、テンプレートのベース名を返します。ベース名はすべての修飾語句を除いたテンプレート名です。

    -----------------------------------------

    bindingSetList (string): 照会モードで使用するこのフラグは、テンプレートに定義されたすべてのバインドセットのリストを返します。

    -----------------------------------------

    childAnchor (boolean): このフラグはpublishedNodeListを照会する場合に、オプションとして指定できます。結果のリストには、childAnchorタイプのパブリッシュされたノードのみが表示されます。

    -----------------------------------------

    delete (boolean): 指定したテンプレートとそのファイルを削除します。このテンプレートに接続されたすべてのオブジェクト、または同じテンプレートファイルに含まれるすべてのオブジェクトが削除されます。ファイルを完全に削除しないで単にテンプレートをアンロードするだけの場合は、このコマンドではなくアンロードを使用してください。

    -----------------------------------------

    expandCompounds (boolean): この引数は、複合の親アトリビュートとその子を両方ともコンテナにパブリッシュする場合に、生成されたビューにどのように追加するかを定義するのに使用します。trueの場合、コンテナにパブリッシュされた複合の親アトリビュートとすべての複合の子アトリビュートがビューに追加されます。falseの場合は、親アトリビュートのみがビューに追加されます。注:子アトリビュートのみがパブリッシュされて親はパブリッシュされない場合は、子がビューに追加されます。このフラグは親と子の両方のアトリビュートがコンテナにパブリッシュされる状況でのみ使用します。既定値はfalseです。

    -----------------------------------------

    fromContainer (string): この引数は作成または編集モードで使用し、テンプレート内容の作成に使用されるコンテナノードを指定します。テンプレートの作成モードでは、指定したコンテナ内のパブリッシュされたアトリビュートのリストに基づいて、テンプレート定義が作成されます。編集モードで、addNamesフラグと一緒に使用するか、他のフラグを使用しない場合、コンテナ上にあるパブリッシュされた名前のうち、テンプレートのアトリビュートとして存在しないものはテンプレートに追加されます。このフラグはaddViewなどのフラグとも一緒に使用します。

    -----------------------------------------

    fromSelection (boolean): trueの場合、アクティブなセレクションリストを使用してテンプレートまたはビューを作成します。allKeyableもtrueの場合、セレクション内のキー設定可能なアトリビュートからテンプレートを作成するか、アトリビュートフラグで指定したアトリビュートを使用し、テンプレートを作成します。

    -----------------------------------------

    layoutMode (int): この引数は、ビューの作成時にレイアウトモードを指定するのに使用します。値は次のとおりです。0:フラットリストでレイアウト(ビューをコンテナから作成しない場合の既定)1:ノードでグループ化したレイアウト(ビューをコンテナから作成する場合の既定)ノード情報を必要とするレイアウトモードには、リファレンスコンテナまたはセレクションを提供するためにfromContainerまたはfromSelection引数が必要です。ビューは定義済みのテンプレートアトリビュートしか参照できないことに注意してください。つまり、fromContainerフラグまたはfromSelectionフラグを使用してビューを既存のテンプレートに追加する場合、テンプレートとコンテナの両方で定義されたアトリビュートまたは現在のセレクションのみがビューに追加されます(つまり、テンプレートで定義されていないコンテナにパブリッシュされたアトリビュートは無視されます)。

    -----------------------------------------

    matchName (string): このフラグは、他のフラグと一緒に使用して照会モードで使用し、照会操作の一部として一致させるオプションのテンプレート名を指定します。ベーステンプレート名は一致に使用され、同じベース名を持つすべてのテンプレートは異なるパッケージを渡って一致されます。照会モードでは、このフラグに値が必要になります。

    -----------------------------------------

    parentAnchor (boolean): このフラグはpublishedNodeListを照会する場合に、オプションとして指定できます。結果のリストには、parentAnchorタイプのパブリッシュされたノードのみが表示されます。

    -----------------------------------------

    publishedNodeList (string): 照会モードで使用するこのフラグは、テンプレート定義に含まれるパブリッシュ済みノードのリストを返します。既定では、テンプレートにあるすべてのパブリッシュされたノードを返します。パブリッシュされたノードのリストは、childAnchor、parentAnchor、rootTransformフラグを使って特定のタイプのパブリッシュされたノードに限定することができます。オプションのフラグを指定すると、指定したタイプのノードのみが返されます。

    -----------------------------------------

    removeBindingSet (string): この引数を使用して、指定したバインドセットをテンプレートから除去します。バインドセットをテンプレートファイルから永久に除去するには、テンプレートを保存する必要があります。

    -----------------------------------------

    removeView (string): この引数を使用して、指定したビューをテンプレートから除去します。ビューをテンプレートファイルから永久に除去する前に、テンプレートを保存する必要があります。

    -----------------------------------------

    rootTransform (boolean): このフラグはpublishedNodeListを照会する場合に、オプションとして指定できます。結果のリストには、rootTransformタイプのパブリッシュされたノードのみが表示されます。

    -----------------------------------------

    save (boolean): ファイルに指定したテンプレートを保存します。テンプレートに対するファイル名を指定すると、ファイル全体(つまりファイルに関連付けられたすべてのテンプレート)が保存されます。ファイル名を指定しないと、テンプレート名に基づいて、既定のファイル名が使用されます。

    -----------------------------------------

    searchPath (string): テンプレートの検索パスは、テンプレートファイルが検索されるすべての場所を、最初に検索される場所から最後に検索される場所までの順番に列挙するリストです。テンプレートの検索パス設定は現在のワークスペースに格納され、「テンプレート」のファイル規則のエントリとして設定、照会することができます(詳細については、workspaceコマンドを参照してください)。編集モードでは、このフラグで検索パス設定をカスタマイズすることができます。検索パスの値を設定する場合、リストは現在のプラットフォームが要求するパスリストのフォーマットに準拠している必要があります。つまりWindowsの場合はパスをセミコロン「;」で分け、LinuxとMacOSXの場合はコロン「:」で分けます。環境変数を使用することもできます。Mayaがカスタマイズした設定にビルトインパスを自動的に追加することがあります。照会モードでは、このフラグは検索パスの現在のコンテンツを返します。カスタマイズ、ビルトイン双方のすべてのパスが、照会の戻り値に含まれます。

    -----------------------------------------

    templateList (string): 照会モードで使用して、ロードされたすべてのテンプレートのリストを返します。この照会はオプションのmatchFileフラグおよびmatchNameフラグと一緒に使用できます。matchFileフラグと一緒に使用すると、テンプレートのリストは、指定したファイルに接続されたものに限定されます。matchNameフラグと一緒に使用すると、テンプレートのリストは、指定したテンプレート名に一致するものに限定されます。

    -----------------------------------------

    updateBindingSet (string): この引数を使用して、既存のバインドセットを新規のバインドに更新します。fromContainer引数と一緒に使用すると、指定コンテナのバインドをもとに、バインドセットエントリはバインドセット内で置き換えまたはマージされます。forceフラグを使用すると、バインドセットの既存エントリの値が置き換えられます。使用しないと、新規エントリのみがバインドセットにマージされ、既存エントリは変更されません。リファレンスコンテナなしで使用すると、バインドセットはプレースホルダエントリが含まれた状態で更新されます。新規バインドセットをテンプレートファイルに永久保存するには、テンプレートを保存する必要があります。

    -----------------------------------------

    useHierarchy (boolean): trueでありfromSelectionフラグが設定されている場合、セレクションリストはその階層も含むよう拡張されます。
...
def containerView(itemInfo: str = "",itemList: bool = False,viewDescription: bool = False,viewLabel: bool = False,viewList: bool = False,viewName: str = "") -> None:
    """
...
def filterInstances(shapes: bool = False) -> None:
    """
    このコマンドで選択リストをフィルタすることにより、同じオブジェクトやコンポーネントを参照する重複インスタンスを除去できます。このようなインスタンスが見つかった場合は、最初に選択したインスタンスと結合されます。
    ...
    shapes (boolean): trueの場合、このコマンドは選択したトランスフォームの下にあるインスタンス化されたシェイプをチェックし、親トランスフォームがインスタンスであるとみなされるかどうかを決定するためにそのシェイプを使用します。既定はfalseです。
...
def ls(absoluteName: bool = False,allPaths: bool = False,assemblies: bool = False,cameras: bool = False,containerType: str = "",containers: bool = False,dagObjects: bool = False,defaultNodes: bool = False,dependencyNodes: bool = False,exactType: str = "",excludeType: str = "",flatten: bool = False,geometry: bool = False,ghost: bool = False,head: int = 1,hilite: bool = False,intermediateObjects: bool = False,invisible: bool = False,leaf: bool = False,lights: bool = False,live: bool = False,lockedNodes: bool = False,long: bool = False,materials: bool = False,modified: bool = False,noIntermediate: bool = False,nodeTypes: bool = False,objectsOnly: bool = False,orderedSelection: bool = False,partitions: bool = False,persistentNodes: bool = False,planes: bool = False,preSelectHilite: bool = False,readOnly: bool = False,recursive: bool = False,referencedNodes: bool = False,references: bool = False,renderGlobals: bool = False,renderQualities: bool = False,renderResolutions: bool = False,renderSetups: bool = False,selection: bool = False,sets: bool = False,shapes: bool = False,shortNames: bool = False,showNamespace: bool = False,showType: bool = False,tail: int = 1,templated: bool = False,textures: bool = False,transforms: bool = False,type: str = "",ufeObjects: bool = False,undeletable: bool = False,untemplated: bool = False,uuid: bool = False,visible: bool = False) -> None:
    ...
    ufeObjects (boolean): -sl/selectionフラグと一緒に使用すると、ネイティブのMayaオブジェクトだけでなく、UFEインタフェースによって定義されているオブジェクトも返されます。
...
def matchTransform(pivots: bool = False,position: bool = False,positionX: bool = False,positionY: bool = False,positionZ: bool = False,rotatePivot: bool = False,rotation: bool = False,rotationX: bool = False,rotationY: bool = False,rotationZ: bool = False,scale: bool = False,scaleBox: bool = False,scalePivot: bool = False,scaleX: bool = False,scaleY: bool = False,scaleZ: bool = False) -> None:
    """
    ...
    positionX (boolean): ソースオブジェクトのX位置をターゲットオブジェクトに一致させます。

    -----------------------------------------

    positionY (boolean): ソースオブジェクトのY位置をターゲットオブジェクトに一致させます。

    -----------------------------------------

    positionZ (boolean): ソースオブジェクトのZ位置をターゲットオブジェクトに一致させます。

    -----------------------------------------

    rotatePivot (boolean): ソースオブジェクトの回転のピボット位置を、ターゲットトランスフォームのピボットに一致させます。
    ...
    rotationX (boolean): ソースオブジェクトのX回転をターゲットオブジェクトに一致させます。

    -----------------------------------------

    rotationY (boolean): ソースオブジェクトのY回転をターゲットオブジェクトに一致させます。

    -----------------------------------------

    rotationZ (boolean): ソースオブジェクトのZ回転をターゲットオブジェクトに一致させます。
    ...
    scaleBox (boolean): スケーリングを一致させる場合は、ソース/ターゲットオブジェクトの子バウンディングボックスのサイズを使用します。

    -----------------------------------------

    scalePivot (boolean): ソースオブジェクトのスケールピボットの位置を、ターゲットトランスフォームのピボットに一致させます。

    -----------------------------------------

    scaleX (boolean): ソースオブジェクトのXスケールをターゲットオブジェクトに一致させます。

    -----------------------------------------

    scaleY (boolean): ソースオブジェクトのYスケールをターゲットオブジェクトに一致させます。

    -----------------------------------------

    scaleZ (boolean): ソースオブジェクトのZスケールをターゲットオブジェクトに一致させます。
...
def performanceOptions(clusterResolution: float = 1.0,disableStitch: str = "",disableTrimBoundaryDisplay: str = "",disableTrimDisplay: str = "",latticeResolution: float = 1.0,passThroughBindSkinAndFlexors: str = "",passThroughBlendShape: str = "",passThroughCluster: str = "",passThroughDeltaMush: str = "",passThroughFlexors: str = "",passThroughLattice: str = "",passThroughPaintEffects: str = "",passThroughSculpt: str = "",passThroughWire: str = "",skipHierarchyTraversal: bool = False,useClusterResolution: str = "",useLatticeResolution: str = "") -> None:
    ...
    passThroughBindSkinAndFlexors (string): スキンのバインドおよびすべてのフレクサ通過の状態を設定します。有効な値は「on」、「off」、「interactive」です。
    ...
    passThroughDeltaMush (string): デルタMushデフォーマ通過の状態を設定します。有効な値は「on」、「off」、「interactive」です。

    -----------------------------------------

    passThroughFlexors (string): フレクサ通過の状態を設定します。有効な値は「on」、「off」、「interactive」です。
    ...
    passThroughPaintEffects (string): ペイントエフェクト通過の状態を設定します。有効な値は「on」、「off」、「interactive」です。

    -----------------------------------------

    passThroughSculpt (string): スカルプトデフォーマ通過の状態を設定します。有効な値は「on」、「off」、「interactive」です。

    -----------------------------------------

    passThroughWire (string): ワイヤデフォーマ通過の状態を設定します。有効な値は「on」、「off」、「interactive」です。
...
def reorderContainer(back: bool = False,front: bool = False,relative: int = 1) -> None:
    """
    ...
    back (boolean): オブジェクトをコンテナの内容リストの後面に移動します

    -----------------------------------------

    front (boolean): オブジェクトをコンテナの内容リストの前面に移動します

    -----------------------------------------

    relative (int): オブジェクトを他のコンテナの内容に対して相対的に移動します
...
def sceneLint(issueType: str = "",verbose: bool = False) -> None:
    """
    ...
    issueType (string): チェックする問題タイプのセットを指定します。省略した場合は、すべての既知の問題タイプがチェックされます。照会モードでは、特定の問題タイプでチェックする内容の説明を返します。照会モードでは、このフラグは値を受け入れることができます。

    -----------------------------------------

    verbose (boolean): 設定されている場合は、使用可能な問題タイプのリストを照会するときに、名前および説明を含めます。

    -----------------------------------------

    Return Value:
    None: stringシーン内で問題を引き起こす可能性のある問題を示す、JSON 形式の結果です。string[]照会した場合、issueType は指定したシーンの問題の説明、および役に立つ値を示します。string[]照会した場合、すべての issueTypes の名前別リストを返します。照会モードでは、戻り値のタイプは照会されたフラグに基づきます。
    """
    pass

    
def sets(addElement: str = "",afterFilters: bool = False,anyMember: str = "",clear: str = "",color: int = 1,copy: str = "",edges: bool = False,editPoints: bool = False,empty: bool = False,facets: bool = False,flatten: str = "",forceElement: str = "",include: str = "",intersection: str = "",isIntersecting: str = "",isMember: str = "",layer: bool = False,name: str = "",noIntermediate: bool = False,noSurfaceShader: bool = False,noWarnings: bool = False,nodesOnly: bool = False,remove: str = "",renderable: bool = False,size: bool = False,split: str = "",subtract: str = "",text: str = "",union: str = "",vertices: bool = False) -> None:
    """
...
def suitePrefs(applyToSuite: str = "",installedAsSuite: bool = False,isCompleteSuite: bool = False) -> None:
    """
    ...
    applyToSuite (string): 指定されたアプリケーションのマウスとキーボードのインタラクション設定をSuiteのすべてのアプリケーションに適用します(MayaがSuitesインストールの一部の場合)。有効な値は、「Maya」、「3dsMax」、または各アプリケーション専用の設定が使用されることを示す「undefined」です。

    -----------------------------------------

    installedAsSuite (boolean): MayaがSuitesインストールの一部の場合はtrue、それ以外の場合はfalseを返します。

    -----------------------------------------

    isCompleteSuite (boolean): SuitesインストールにEntertainmentCreationSuite製品が含まれている場合はtrue、それ以外の場合はfalseを返します。
...
def addAttr(attributeType: str = "",binaryTag: str = "",cachedInternally: bool = False,category: str = "",dataType: str = "",defaultValue: float = 1.0,disconnectBehaviour: int = 1,enumName: str = "",exists: bool = False,fromPlugin: bool = False,hasMaxValue: bool = False,hasMinValue: bool = False,hasSoftMaxValue: bool = False,hasSoftMinValue: bool = False,hidden: bool = False,indexMatters: bool = False,internalSet: bool = False,keyable: bool = False,longName: str = "",maxValue: float = 1.0,minValue: float = 1.0,multi: bool = False,niceName: str = "",numberOfChildren: int = 1,parent: str = "",proxy: str = "",readable: bool = False,shortName: str = "",softMaxValue: float = 1.0,softMinValue: float = 1.0,storable: bool = False,usedAsColor: bool = False,usedAsFilename: bool = False,usedAsProxy: bool = False,worldSpace: bool = False,writable: bool = False) -> None:
    ...
    softMinValue (float): ソフト最小値、数値アトリビュートにのみ有効です。このアトリビュート用のスライダで使用する既定の下限値を指定します。
    ...
    usedAsColor (boolean): アトリビュートをカラー定義として使用します。このフラグを使用するには、3DOUBLEまたは3FLOATの子が必要です。アトリビュートタイプ「-at」は必要に応じて「double3」または「float3」にする必要があります。また、このフラグを「double3」または「float3」としたデータ型「-dt」とともに使用してエフェクトを小さくすることもできますが、コードの部分によってはこの代替をサポートしないことがあります。特別なアトリビュートタイプ/データである「スペクトル」と「反射率」はカラーフラグもサポートしており、この上では既定でカラーフラグが設定されます。
    ...
    worldSpace (boolean): このアトリビュートをワールド空間として扱うかどうかを設定します。ワールド空間として扱う場合、アトリビュートはこのノードのワールド空間トランスフォームに依存し、ワールド空間トランスフォームに影響を与える階層内のアトリビュート変更によってダーティとマークされます。インスタンス化中のノードには複数のワールド空間パスがあり、Mayaはワールド空間アトリビュートのパスごとに配列要素が1つ必要になるため、アトリビュートに配列を指定する必要があります。備考:1.配列アトリビュートでのみ使用できます。2.非DAGノードでは、このプロパティは無視されます。3.アトリビュートは別のアトリビュートの影響を受けるか、接続されている必要があります。それ以外の場合、アトリビュートは計算されず、ダーティに戻りません。
...
def attributeQuery(affectsAppearance: bool = False,affectsWorldspace: bool = False,attributeType: bool = False,cachedInternally: bool = False,categories: bool = False,channelBox: bool = False,computeModifies: bool = False,connectable: bool = False,enum: bool = False,exists: bool = False,hidden: bool = False,indeterminant: bool = False,indexMatters: bool = False,internal: bool = False,internalGet: bool = False,internalSet: bool = False,keyable: bool = False,listChildren: bool = False,listDefault: bool = False,listEnum: bool = False,listParent: bool = False,listSiblings: bool = False,localizedListEnum: bool = False,longName: bool = False,maxExists: bool = False,maximum: bool = False,message: bool = False,minExists: bool = False,minimum: bool = False,multi: bool = False,niceName: bool = False,node: str = "",numberOfChildren: bool = False,range: bool = False,rangeExists: bool = False,readable: bool = False,renderSource: bool = False,shortName: bool = False,softMax: bool = False,softMaxExists: bool = False,softMin: bool = False,softMinExists: bool = False,softRange: bool = False,softRangeExists: bool = False,storable: bool = False,type: str = "",typeExact: str = "",usedAsColor: bool = False,usedAsFilename: bool = False,usesMultiBuilder: bool = False,worldspace: bool = False,writable: bool = False) -> None:
    ...
    computeModifies (boolean): このアトリビュートのcomputeModifies値を返します。
    ...
    localizedListEnum (boolean): 指定したアトリビュートのローカライズされたenum文字列のリストを返します。
...
def deleteAttr(attribute: str = "") -> None:
    """
...
def geometryAttrInfo(boundingBox: bool = False,castToEdges: bool = False,castToFaces: bool = False,castToVerts: bool = False,componentTagCategory: bool = False,componentTagExpression: str = "",componentTagHash: bool = False,componentTagHistory: bool = False,componentTagHistoryHash: bool = False,componentTagNames: bool = False,components: bool = False,deformerChain: bool = False,elementCount: bool = False,groupId: int = 1,matrix: bool = False,nodeChain: bool = False,originalGeometry: bool = False,outputPlugChain: bool = False,plugChain: bool = False,pointCount: bool = False,pointIndices: bool = False,points: bool = False,subsetState: bool = False) -> None:
    """
    ...
    boundingBox (boolean): ジオメトリのバウンディングボックスを返します。

    -----------------------------------------

    castToEdges (boolean): componentTagエクスプレッションが必ずエッジコンポーネントに解決されるようにします。

    -----------------------------------------

    castToFaces (boolean): componentTagエクスプレッションが必ずフェースコンポーネントに解決されるようにします。

    -----------------------------------------

    castToVerts (boolean): componentTagエクスプレッションが必ず頂点コンポーネントに解決されるようにします。

    -----------------------------------------

    componentTagCategory (boolean): このフラグは、結果として得られるコンポーネントのコンポーネントタグカテゴリを返します。頂点は「v」、エッジは「e」、面は「f」です。カテゴリを特定できない場合は、「不明」が返されます。

    -----------------------------------------

    componentTagExpression (string): 照会するcomponentTagExpressionを指定します。指定した場合、情報リクエストに対する回答として返されるのは、これらのcomponentTagsの組み合わせに含まれるジオメトリのサブセットに限定されます。

    -----------------------------------------

    componentTagHash (boolean): このフラグは、ジオメトリに含まれるすべてのcomponentTagsの状態に対して一意のハッシュ値を返します。ハッシュが以前のハッシュと異なる場合は、何らかの変更が加えられています(タグが追加/削除/名前変更されたか、コンポーネントの内容が変更されています)。

    -----------------------------------------

    componentTagHistory (boolean): このフラグは、componentTagsの説明と、ジオメトリに追加されたチェーン内のノードを返します。

    -----------------------------------------

    componentTagHistoryHash (boolean): このフラグは、プラグ内のジオメトリのcomponentTagのヒストリに対して一意のハッシュ値を返します。ハッシュが以前のハッシュと異なる場合は、何らかの変更が加えられています(さまざまなノードにタグが作成されたか、タグの内容が変更されています)。

    -----------------------------------------

    componentTagNames (boolean): ジオメトリのcomponentTagsの名前を返します。

    -----------------------------------------

    components (boolean): ジオメトリのコンポーネントを返します。

    -----------------------------------------

    deformerChain (boolean): このフラグは、ジオメトリが指定したプラグに到達するために通過するデフォーマノードのリストを返します。

    -----------------------------------------

    elementCount (boolean): コンポーネントの要素数を返します。

    -----------------------------------------

    groupId (int): 照会するグループIDを指定します。指定した場合、情報リクエストに対する回答として返されるのは、このグループIDに含まれるジオメトリのサブセットに限定されます。

    -----------------------------------------

    matrix (boolean): ジオメトリに関連付けられた行列を返します。

    -----------------------------------------

    nodeChain (boolean): このフラグは、ジオメトリが指定したプラグに到達するために通過するノードのリストを返します。

    -----------------------------------------

    originalGeometry (boolean): このフラグは、originalGeometryとして使用するのに最適な上流ノード(通常はフロントエンドのノード)のプラグ名を返します。何も存在しない場合は、空のプラグを返します。

    -----------------------------------------

    outputPlugChain (boolean): このフラグは、指定したプラグの上流にある一連のプラグを返します(出力プラグのみが含まれます)。

    -----------------------------------------

    plugChain (boolean): このフラグは、指定したプラグの上流にある一連のプラグを返します(入力プラグと出力プラグが両方含まれます)。

    -----------------------------------------

    pointCount (boolean): ジオメトリのポイント数を返します。

    -----------------------------------------

    pointIndices (boolean): ジオメトリのインデックスを返します。

    -----------------------------------------

    points (boolean): ジオメトリのポイントのリストを返します。

    -----------------------------------------

    subsetState (boolean): 指定したサブセットの状態を返します。-1の場合は、サブセットが無効です。0の場合は、サブセットにジオメトリのポイントが含まれていません。1の場合は、サブセットにジオメトリのポイントの一部(すべてではない)が含まれています。2の場合は、サブセットにジオメトリのすべてのポイントが含まれています。

    -----------------------------------------

    Return Value:
    None: Anyアトリビュート内のジオメトリに関する情報です。返される値の数および型は、情報リクエストによって決まります。
    """
    pass
...
def listHistory(allConnections: bool = False,allFuture: bool = False,allGraphs: bool = False,breadthFirst: bool = False,fastIteration: bool = False,future: bool = False,futureLocalAttr: bool = False,futureWorldAttr: bool = False,groupLevels: bool = False,historyAttr: bool = False,interestLevel: int = 1,leaf: bool = False,levels: int = 1,pruneDagObjects: bool = False) -> None:
    """
    ...
    fastIteration (boolean): このフラグを指定すると、さらにスケーラブルなパフォーマンスをする高速な反復モードが使用可能になります。特に、多数の接続を持つノードが移動している場合に有効です。ただし、アトリビュート間の依存関係が推移している場合などでは、若干異なる結果になることがあります(アトリビュートAがBの影響を受け、BがCの影響を受けている場合でも、AがCの影響を直接受けているわけではありません)。
...
def nodeType(apiType: bool = False,derived: bool = False,inherited: bool = False,isTypeName: bool = False,ufeRuntimeName: bool = False) -> None:
    """
    ...
    ufeRuntimeName (boolean): 指定したノードに対応するUFEランタイム名を返します。これは、ネイティブのMayaオブジェクトと、UFEインタフェースを使用してMayaに公開される非ネイティブオブジェクトとの間でフィルタする場合に、特に便利です。
...
def relationship(b: bool = False,relationshipData: str = "") -> None:
    """
    これはおもにファイル IO で使用します。リレーションシップの維持に必要な特定のアトリビュート/接続を書き出す代わりに、関連ノード/プラグの説明を書き出します。リレーションシップはオーナー ノードを持ち、特定のタイプである必要があります。ファイルの読み込み中に、ディペンデンシー グラフでリレーションシップを表すのに必要な接続の実行やデータの設定が行われます。
    ...
    b (boolean): 指定のリレーションシップを作成するのではなく切断します。

    -----------------------------------------

    relationshipData (string): リレーションシップの作成時に使用されるリレーションシップデータを提供します。
...
def displayPref(activeObjectPivots: bool = False,displayAffected: bool = False,displayGradient: bool = False,ghostFrames: Tuple[int, int, int] = tuple(1, 1, 1),materialLoadingMode: str = "",maxHardwareTextureResolution: bool = False,maxTextureResolution: int = 1,purgeExistingTextures: bool = False,regionOfEffect: bool = False,shadeTemplates: bool = False,textureDrawPixel: bool = False,wireframeOnShadedActive: str = "") -> None:
    ...
    ghostFrames ([int, int, int]): 廃止:これらの値を設定するには、「ゴースト化」コマンドを使用してください。
...
def weightsColor(colorRamp: str = "",deformer: str = "",falseColor: bool = False,outOfRangeColor: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),rampMaxColor: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),rampMinColor: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),useColorRamp: bool = False,useMaxMinColor: bool = False) -> None:
    """
    デフォーマ ウェイトのカラーをコントロールします。
    ...
    deformer (string): 視覚化する必要があるデフォーマを指定します。

    -----------------------------------------

    falseColor (boolean): ジオメトリでの疑似カラー表示を有効/無効にします。

    -----------------------------------------

    outOfRangeColor ([float, float, float]): デフォーマサブセットの外側の領域に使用する特殊なカラーを定義します。
    ...
    Return Value:
    None: string[]照会操作の場合照会モードでは、戻り値のタイプは照会されたフラグに基づきます。
    """
    pass
...
def selectPref(affectsActive: bool = False,allowHiliteSelection: bool = False,autoSelectContainer: bool = False,autoSelectOutlinerSetMembers: bool = False,autoUseDepth: bool = False,clickBoxSize: int = 1,clickDrag: bool = False,containerCentricSelection: bool = False,disableComponentPopups: bool = False,expandPopupList: bool = False,ignoreSelectionPriority: bool = False,manipClickBoxSize: int = 1,paintSelect: bool = False,paintSelectWithDepth: bool = False,popupMenuSelection: bool = False,preSelectBackfacing: bool = False,preSelectClosest: bool = False,preSelectDeadSpace: int = 1,preSelectHilite: bool = False,preSelectHiliteSize: float = 1.0,preSelectTweakDeadSpace: int = 1,selectTypeChangeAffectsActive: bool = False,selectionChildHighlightMode: int = 1,singleBoxSelection: bool = False,straightLineDistance: bool = False,trackSelectionOrder: bool = False,useDepth: bool = False,xformNoSelect: bool = False) -> None:
    ...
    autoSelectOutlinerSetMembers (boolean): 有効にすると、アウトライナ(Outliner)でセットを選択したときに、代わりにセットメンバーが自動的に選択されます。
...
def art3dPaintCtx(accopacity: bool = False,afterStrokeCmd: str = "",alphablendmode: str = "",assigntxt: bool = False,attrnames: str = "",beforeStrokeCmd: str = "",brushalignment: bool = False,brushdepth: float = 1.0,brushfeedback: bool = False,brushtype: str = "",clear: bool = False,commonattr: str = "",dragSlider: str = "",dynclonemode: bool = False,exists: bool = False,expandfilename: bool = False,extendFillColor: bool = False,fileformat: str = "",filetxtaspectratio: float = 1.0,filetxtsizex: int = 1,filetxtsizey: int = 1,floodOpacity: float = 1.0,floodall: bool = False,floodselect: bool = False,history: bool = False,image1: str = "",image2: str = "",image3: str = "",keepaspectratio: bool = False,lastRecorderCmd: str = "",lastStampName: str = "",lowerradius: float = 1.0,makeStroke: int = 1,mappressure: str = "",name: str = "",opacity: float = 1.0,outline: bool = False,outwhilepaint: bool = False,paintmode: str = "",paintoperationtype: str = "",painttxtattr: str = "",painttxtattrname: str = "",pfxScale: float = 1.0,pfxWidth: float = 1.0,pickColor: bool = False,pickValue: bool = False,playbackCursor: Tuple[float, float] = tuple(1.0, 1.0),playbackPressure: float = 1.0,preserveclonesource: bool = False,pressureMapping1: int = 1,pressureMapping2: int = 1,pressureMapping3: int = 1,pressureMax1: float = 1.0,pressureMax2: float = 1.0,pressureMax3: float = 1.0,pressureMin1: float = 1.0,pressureMin2: float = 1.0,pressureMin3: float = 1.0,profileShapeFile: str = "",projective: bool = False,radius: float = 1.0,record: bool = False,reflection: bool = False,reflectionaboutorigin: bool = False,reflectionaxis: str = "",reloadtexfile: bool = False,resizeratio: float = 1.0,resizetxt: bool = False,rgbcolor: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),rgbflood: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),saveTextureOnStroke: bool = False,saveonstroke: bool = False,savetexture: bool = False,screenRadius: float = 1.0,selectclonesource: bool = False,shadernames: str = "",shapeattr: bool = False,shapenames: str = "",showactive: bool = False,soloAsDiffuse: bool = False,stampDepth: float = 1.0,stampProfile: str = "",stampSpacing: float = 1.0,strokesmooth: str = "",surfaceConformedBrushVertices: bool = False,tablet: bool = False,tangentOutline: bool = False,textureFilenames: bool = False,updateEraseTex: bool = False,usepressure: bool = False,worldRadius: float = 1.0) -> None:
    ...
    mappressure (string): タブレットを使用するときのタブレット圧力のマッピングを設定します。次の3つの選択肢があります。「opacity」-圧力が不透明度にマップされる、「radius」-圧力がブラシ半径を修正するようマップされる、「both」-圧力が不透明度と半径の両方を修正する。C:既定は「opacity」です。Q:照会するとstringを返します。
    ...
    stampProfile (string): 現在のスタンプのブラシプロファイルを設定します。現在、次のプロファイルがサポートされています。「gaussian」、「poly」、「solid」、「square」。C:既定は「gaussian」です。Q:照会するとstringを返します。
...
def artFluidAttrCtx(accopacity: bool = False,activeListChangedProc: str = "",afterStrokeCmd: str = "",alphaclamp: str = "",alphaclamplower: float = 1.0,alphaclampupper: float = 1.0,attrSelected: str = "",autoSave: str = "",beforeStrokeCmd: str = "",brushalignment: bool = False,brushfeedback: bool = False,clamp: str = "",clamplower: float = 1.0,clampupper: float = 1.0,clear: bool = False,colorAlphaValue: float = 1.0,colorRGBAValue: Tuple[float, float, float, float] = tuple(1.0, 1.0, 1.0, 1.0),colorRGBValue: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),colorRamp: str = "",colorfeedback: bool = False,colorfeedbackOverride: bool = False,colorrangelower: float = 1.0,colorrangeupper: float = 1.0,currentPaintableFluid: str = "",dataTypeIndex: int = 1,delaySelectionChanged: bool = False,disablelighting: bool = False,displayAsRender: bool = False,displayVelocity: bool = False,doAutoSave: bool = False,dragSlider: str = "",duringStrokeCmd: str = "",dynclonemode: bool = False,exists: bool = False,expandfilename: bool = False,exportaspectratio: float = 1.0,exportfilemode: str = "",exportfilesave: str = "",exportfilesizex: int = 1,exportfilesizey: int = 1,exportfiletype: str = "",filterNodes: bool = False,history: bool = False,image1: str = "",image2: str = "",image3: str = "",importfileload: str = "",importfilemode: str = "",importreassign: bool = False,interactiveUpdate: bool = False,lastRecorderCmd: str = "",lastStampName: str = "",lowerradius: float = 1.0,makeStroke: int = 1,mappressure: str = "",maxvalue: float = 1.0,minvalue: float = 1.0,name: str = "",objattrArray: str = "",opacity: float = 1.0,outline: bool = False,outwhilepaint: bool = False,paintNodeArray: str = "",paintattrselected: str = "",paintmode: str = "",paintoperationtype: str = "",pickColor: bool = False,pickValue: bool = False,playbackCursor: Tuple[float, float] = tuple(1.0, 1.0),playbackPressure: float = 1.0,preserveclonesource: bool = False,profileShapeFile: str = "",projective: bool = False,property: str = "",radius: float = 1.0,rampMaxColor: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),rampMinColor: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),record: bool = False,reflection: bool = False,reflectionaboutorigin: bool = False,reflectionaxis: str = "",rgbValue: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),screenRadius: float = 1.0,selectclonesource: bool = False,selectedattroper: str = "",showactive: bool = False,stampDepth: float = 1.0,stampProfile: str = "",stampSpacing: float = 1.0,strokesmooth: str = "",surfaceConformedBrushVertices: bool = False,tablet: bool = False,tangentOutline: bool = False,toolOffProc: str = "",toolOnProc: str = "",useColorRamp: bool = False,useMaxMinColor: bool = False,useStrokeDirection: bool = False,usepressure: bool = False,value: float = 1.0,velocity: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),whichTool: str = "",worldRadius: float = 1.0) -> None:
    """
    これは、アトリビュート ペイント操作のベース コンテキストである artAttrContext にフラグを設定するためのコンテキスト コマンドです。すべてのコマンドは、最後の引数としてコンテキスト名を必要とします。これは作成、編集、または照会されるコンテキストの名前を表します。
    ...
    autoSave (string): 流体の状態を保存するMELコマンドです。ペイントされた流体プロパティの未保存の値を上書きするようなイベントが発生する前に呼び出されます。このようなイベントには、「現在のタイムの変更」、「現在ペイント可能なプロパティの変更」、「ペイントツールの終了」などが含まれます(auto-saveをオフにするには、引数に「」などの空文字列を渡してください)。
    ...
    currentPaintableFluid (string): このコンテキストで現在ペイントしている流体の名前を照会します。文字列を返します。
    ...
    delaySelectionChanged (boolean): 内部でのみ使用されます。通常、ツールはセレクションリストの変更に反応して、ペイント可能なジオメトリのリストを更新します。-dsltrueを使用した場合、ツールは対応する-dslfalseが呼ばれるまでペイント可能リストを更新しません。
    ...
    displayAsRender (boolean): trueの場合、流体の「シェーディングモード(ShadedDisplay)」アトリビュートを「AsRender」に設定します。これにより、すべての流体プロパティがハードウェアレンダリングで表示されます。falseの場合は、現在選択されているペイント可能な流体のアトリビュートのみが表示されます。

    -----------------------------------------

    displayVelocity (boolean): 上の「dar/displayAsRender」の設定とは関係なく、速度表示のオン/オフを切り替えます。たとえば、密度を表示している間だけ速度表示を有効にする場合などに使用します。

    -----------------------------------------

    doAutoSave (boolean): 未保存のペイントされた流体プロパティがある場合に、-autoSaveコマンドを実行します。
    ...
    property (string): ペイントする流体のプロパティを指定します。有効な値は、「color」、「density」、「densityAndColor」、「densityAndFuel」、「temperature」、「fuel」、「velocity」です。
    ...
    rgbValue ([float, float, float]): 「color」プロパティをペイントする際に使用するカラーの赤、緑と青成分を指定します。
    ...
    useStrokeDirection (boolean): 「velocity」をペイントする場合のみ適用できます。ペイントした速度の方向をブラシストロークの方向と同一にするかどうかを指定します。これは、-v/-velocityフラグで指定した値を上書きします。
    ...
    velocity ([float, float, float]): 「velocity」プロパティをペイントする際に使用する速度のx、yとz成分を指定します。
...
def clipEditorCurrentTimeCtx(exists: bool = False,history: bool = False,image1: str = "",image2: str = "",image3: str = "",name: str = "") -> None:
    """
    このコマンドはクリップ エディタ内のトラック領域の現在のタイムを変更するために使用できるコンテキストを作成します。
...
def dynParticleCtx(conserve: float = 1.0,cursorPlacement: bool = False,exists: bool = False,grid: bool = False,gridSpacing: float = 1.0,history: bool = False,image1: str = "",image2: str = "",image3: str = "",jitterRadius: float = 1.0,lowerLeftX: float = 1.0,lowerLeftY: float = 1.0,lowerLeftZ: float = 1.0,name: str = "",nucleus: bool = False,numJitters: int = 1,particleName: str = "",sketch: bool = False,sketchInterval: int = 1,textPlacement: bool = False,upperRightX: float = 1.0,upperRightY: float = 1.0,upperZ: float = 1.0) -> None:
    """
    ...
    conserve (float): 運動量保存のコントロール(0～1の間)。値を小さくすると、オブジェクトの既存速度がフィールドで消去されます。つまり、フレーム間で運動量が維持されません。値を1にすると(既定)、本来の物理的法則に従って運動量が維持されます。

    -----------------------------------------

    cursorPlacement (boolean): カーソルを使用して、グリッドの左下と右上を配置します。
    ...
    grid (boolean): パーティクルグリッドが作成されます。

    -----------------------------------------

    gridSpacing (float): グリッド内のパーティクル間の間隔。
    ...
    jitterRadius (float): パーティクルインスタンスを配置する、中心からの最大半径。

    -----------------------------------------

    lowerLeftX (float): パーティクルグリッドの左下のX位置。

    -----------------------------------------

    lowerLeftY (float): パーティクルグリッドの左下のY位置。

    -----------------------------------------

    lowerLeftZ (float): パーティクルグリッドの左下のZ位置。
    ...
    nucleus (boolean): trueに設定すると、nParticleが、nucleusノード接続と共に作成されます。そうでない場合、標準パーティクルが作成されます。

    -----------------------------------------

    numJitters (int): パーティクルごとのジッタ(インスタンス)の数。
    ...
    sketch (boolean): スケッチモードでパーティクルが作成されます。

    -----------------------------------------

    sketchInterval (int): スケッチモードの場合のパーティクル間の間隔。

    -----------------------------------------

    textPlacement (boolean): テキストフィールドを使用して、グリッドの左下と右上を指定します。

    -----------------------------------------

    upperRightX (float): パーティクルグリッドの右上のX位置。

    -----------------------------------------

    upperRightY (float): パーティクルグリッドの右上のY位置。

    -----------------------------------------

    upperZ (float): パーティクルグリッドの右上のZ位置。
...
def filterButterworthCtx(apply: bool = False,cutoffFrequency: float = 1.0,endTime: float = 1.0,exists: bool = False,history: bool = False,image1: str = "",image2: str = "",image3: str = "",keepKeysOnFrame: bool = False,name: str = "",samplingRate: float = 1.0,selectedKeys: bool = False,startTime: float = 1.0) -> None:
    """
    Butterworth フィルタのコンテキストを作成/編集します。このコンテキストを使用すると、アニメーション カーブのセットに関する Butterworth フィルタをインタラクティブにプレビュー/編集できます。
    ...
    apply (boolean): 指定した場合は、現在のコンテキストの状態を確定して、この操作のコマンドを記録します。これは、現在のツールのコンテキストを終了しないで、ツールのアクションを完了する操作と同等です。

    -----------------------------------------

    cutoffFrequency (float): Butterworthフィルタのカットオフ周波数の設定を指定します。既定値は7.0です。

    -----------------------------------------

    endTime (time): このフィルタの時間範囲の終了時間を指定します。この時間範囲は、selectedKeysがfalseの場合に使用されます。
    ...
    keepKeysOnFrame (boolean): trueの場合、Butterworthフィルタは、指定したサンプリングレートで出力キーをフレーム全体に再配置します。
    ...
    samplingRate (float): Butterworthフィルタのサンプリングレートの設定を指定します。既定値は30.0です。

    -----------------------------------------

    selectedKeys (boolean): trueの場合は、選択したキーに適用するフィルタを設定します。それ以外の場合、フィルタは指定した時間範囲に適用されます。既定はオンです。

    -----------------------------------------

    startTime (time): このフィルタの時間範囲の開始時間を指定します。この時間範囲は、selectedKeysがfalseの場合に使用されます。
...
def filterGaussianCtx(apply: bool = False,endTime: float = 1.0,exists: bool = False,history: bool = False,image1: str = "",image2: str = "",image3: str = "",name: str = "",sampleCount: int = 1,selectedKeys: bool = False,startTime: float = 1.0,useQuaternion: bool = False,width: float = 1.0) -> None:
    """
    スムーズ(ガウス)フィルタのコンテキストを作成します。このコンテキストを使用すると、アニメーション カーブのセットに関するスムーズ(ガウス)フィルタをインタラクティブにプレビュー/編集できます。
    ...
    sampleCount (int): このパラメータは、サンプリングされる隣接オブジェクトの数を指定します。
    ...
    useQuaternion (boolean): 有効にすると、このフィルタはまずカーブ(回転チャネルカーブと3つの兄弟が同時に必要)をオイラー空間からクォータニオンに変換します。次に、ガウスフィルタを適用します。最後にクォータニオンからオイラー空間に変換し直します。

    -----------------------------------------

    width (time): このパラメータは、ガウスカーネルシェイプの幅を指定します。幅が広いほど、次のようになります。
...
def filterKeyReducerCtx(apply: bool = False,endTime: float = 1.0,exists: bool = False,history: bool = False,image1: str = "",image2: str = "",image3: str = "",keySync: bool = False,name: str = "",precision: float = 1.0,precisionMode: int = 1,preserveKeyTangent: str = "",selectedKeys: bool = False,startTime: float = 1.0) -> None:
    """
    KeyReducer フィルタの コンテキストを作成/編集します。このコンテキストを使用すると、アニメーション カーブのセットに関する KeyReducer フィルタをインタラクティブにプレビュー/編集できます。
    ...
    keySync (boolean): trueの場合は、セカンダリフィルタパスが適用され、検出されたキーごとに、兄弟カーブ(X,Y,Z)にキーが追加されます。
    ...
    precisionMode (int): KeyReducerフィルタの精度モードを指定します。使用可能なモードは、0:絶対値、1:パーセンテージです。既定は1(パーセンテージモード)です。

    -----------------------------------------

    preserveKeyTangent (string): 指定した場合、イン接線タイプまたはアウト接線タイプが指定したタイプと一致するキーが保持されます。サポートされている接線タイプは、fixed、linear、flat、smooth、step、clamped、plateau、stepnext、autoです。
...
def graphTrackCtx(exists: bool = False,history: bool = False,image1: str = "",image2: str = "",image3: str = "",name: str = "") -> None:
    """
    このコマンドを使用して、グラフ エディタ(Graph Editor)のトラック コンテクストを作成します。
...
def greasePencilCtx(autoCreateFrames: bool = False,canDraw: bool = False,createOrEditFrame: int = 1,exists: bool = False,exportArchive: Tuple[str, str] = tuple("", ""),fileTextureSize: int = 1,greasePencilType: int = 1,image1: str = "",image2: str = "",image3: str = "",importArchive: str = "",makeStroke: int = 1,removeFrame: int = 1,resetBrushes: bool = False,rgbcolor: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),sequenceNodeName: str = "") -> None:
    """
    これは、グリース ペンシル ツール用のツール コンテキスト コマンドです。
    ...
    autoCreateFrames (boolean): 作図するときに自動的にフレームを作成する必要がありますか?

    -----------------------------------------

    canDraw (boolean): 作図が許可されているか確認します。

    -----------------------------------------

    createOrEditFrame (int): 作成または編集するフレーム番号です。
    ...
    exportArchive ([string, string]): 書き出しアーカイブの名前を変更します。

    -----------------------------------------

    fileTextureSize (int): ファイルテクスチャの幅と高さの両方の寸法です(それらは正方形になります)。

    -----------------------------------------

    greasePencilType (int): グリースペンシルのタイプです。1=ペンシル、2=マーカー、3=ソフトペンシル、4=消しゴム
    ...
    importArchive (string): 読み込みアーカイブの名前を変更します。
    ...
    removeFrame (int): 指定されたフレームを削除します。

    -----------------------------------------

    resetBrushes (boolean): ブラシをリセットする必要がありますか?

    -----------------------------------------

    rgbcolor ([float, float, float]): グリースペンシルのカラーです。

    -----------------------------------------

    sequenceNodeName (string): シーケンスノードの名前を照会します。
...
def insertKeyCtx(breakdown: bool = False,exists: bool = False,history: bool = False,image1: str = "",image2: str = "",image3: str = "",name: str = "",preserveTangent: bool = False) -> None:
    """
    ...
    preserveTangent (boolean): 接線を保持するかどうかを指定します。
...
def keyframeRegionTrackCtx(exists: bool = False,history: bool = False,image1: str = "",image2: str = "",image3: str = "",name: str = "") -> None:
    """
    このコマンドを使用して、ドープシート エディタのトラック コンテキストを作成できます。
...
def manipMoveContext(activeHandle: int = 1,activeHandleNormal: int = 1,alignAlong: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),bakePivotOri: bool = False,constrainAlongNormal: bool = False,currentActiveHandle: int = 1,editPivotMode: bool = False,editPivotPosition: bool = False,exists: bool = False,image1: str = "",image2: str = "",image3: str = "",interactiveUpdate: bool = False,lastMode: int = 1,manipVisible: bool = False,mode: int = 1,orientAxes: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),orientJoint: str = "",orientJointEnabled: bool = False,orientObject: str = "",orientTowards: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),pinPivot: bool = False,pivotOriHandle: bool = False,position: bool = False,postCommand: str = "",postDragCommand: Tuple[str, str] = tuple("", ""),preCommand: str = "",preDragCommand: Tuple[str, str] = tuple("", ""),preserveChildPosition: bool = False,preserveUV: bool = False,reflection: bool = False,reflectionAbout: int = 1,reflectionAxis: int = 1,reflectionTolerance: float = 1.0,secondaryAxisOrient: str = "",snap: bool = False,snapComponentsRelative: bool = False,snapLiveFaceCenter: bool = False,snapLivePoint: bool = False,snapPivotOri: bool = False,snapPivotPos: bool = False,snapRelative: bool = False,snapValue: float = 1.0,translate: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),tweakMode: bool = False,xformConstraint: str = "") -> None:
    ...
    bakePivotOri (boolean): ピボット方向をベイク処理します。ピボット方向の変更をトランスフォーム階層/ジオメトリに自動的にベイク処理します。
...
def propModCtx(animCurve: str = "",animCurveFalloff: Tuple[float, float] = tuple(1.0, 1.0),animCurveParam: str = "",direction: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),exists: bool = False,image1: str = "",image2: str = "",image3: str = "",linear: float = 1.0,linearParam: Tuple[float, float] = tuple(1.0, 1.0),nurbsCurve: str = "",powerCutoff: float = 1.0,powerCutoffParam: Tuple[float, float] = tuple(1.0, 1.0),powerDegree: float = 1.0,powerDegreeParam: float = 1.0,script: str = "",scriptParam: str = "",type: int = 1,worldspace: bool = False) -> None:
    ...
    script (string): ドロップオフを計算するために使用するスクリプトの名前です。スクリプトは、入力として6つの浮動小数点の値を取ります。最初の3つは移動ロケータの位置を、残りの3つは操作するポイントの位置を表します。スクリプトは、負の値またはゼロのドロップオフ係数を返すはずです。

    -----------------------------------------

    scriptParam (string): ドロップオフを計算するために使用するスクリプトの名前です。スクリプトは、入力として4つの浮動小数点の値を取ります。最初の2つは移動ロケータのパラメトリック位置を、残りの2つは操作するポイントのパラメトリック位置を表します。スクリプトは、負の値またはゼロのドロップオフ係数を返すはずです。

    -----------------------------------------

    type (int): ドロップオフ機能のタイプを選択します。使用できる値は、1(linear)、2(power)、3(script)、4(animcurve)です。既定は1です。
...
def setKeyCtx(breakdown: bool = False,exists: bool = False,history: bool = False,image1: str = "",image2: str = "",image3: str = "",name: str = "",preserveTangent: bool = False) -> None:
    """
...
def targetWeldCtx(exists: bool = False,image1: str = "",image2: str = "",image3: str = "",mergeToCenter: bool = False,preserveUV: bool = False) -> None:
    """
    ...
    preserveUV (boolean): falseの場合、コンポーネントを連結したときにUVは変更されません。trueの場合、コンポーネントを連結したときにテクスチャがずれないようにUVが修正されます。既定はtrueです。
...
def wireContext(crossingEffect: float = 1.0,deformationOrder: str = "",dropoffDistance: float = 1.0,envelope: float = 1.0,exclusive: bool = False,exclusivePartition: str = "",exists: bool = False,groupWithBase: bool = False,history: bool = False,holder: bool = False,image1: str = "",image2: str = "",image3: str = "",localInfluence: float = 1.0,name: str = "") -> None:
    """
    このコマンドは wire デフォーマの作成に使用できるツールを生成します。
    ...
    crossingEffect (linear): 複合フィルタエフェクトの量を設定します。値は0から1で、0でフルに複合され、1で単純化されたエフェクト(フィルタをオフにした場合と同様)になります。既定は0です。

    -----------------------------------------

    deformationOrder (string): デフォメーションの優先順位を決める適切なフラグを設定します。

    -----------------------------------------

    dropoffDistance (linear): ワイヤのドロップオフ距離を設定します。

    -----------------------------------------

    envelope (linear): デフォーマのエンベロープ値を設定します。既定は1.0です。

    -----------------------------------------

    exclusive (boolean): 排他モードをオンまたはオフに設定します。

    -----------------------------------------

    exclusivePartition (string): 排他パーティションの名前を設定します。
    ...
    holder (boolean): Wireコンテキストからワイヤのホルダを指定できるかどうかを制御します。ホルダは、ワイヤのデフォメーション領域の制限に使用できるカーブです。既定はfalseです。
    ...
    localInfluence (linear): ワイヤが他のワイヤに対して持つ、ローカルな影響力を設定します。既定は0です。
...
def wrinkleContext(branchCount: int = 1,branchDepth: int = 1,exists: bool = False,history: bool = False,image1: str = "",image2: str = "",image3: str = "",name: str = "",randomness: float = 1.0,style: str = "",thickness: float = 1.0,wrinkleCount: int = 1,wrinkleIntensity: float = 1.0) -> None:
    """
    ...
    branchCount (uint): radialwrinkleのしわから出る枝の数を設定します。既定は2です。

    -----------------------------------------

    branchDepth (uint): radialwrinkleの枝分かれの深さを設定します。既定は0です。
    ...
    randomness (linear): リンクル構造の直線とその他の要素からのリンクルの折り目の偏差を設定します。既定は0.2です。

    -----------------------------------------

    style (string): リンクルシェイプを設定します。有効値は「lines」、「radial」、「custom」です。既定は「radial」です。

    -----------------------------------------

    thickness (linear): リンクルの折り目の太さを、基礎となるワイヤからのドロップオフ距離という形で設定します。

    -----------------------------------------

    wrinkleCount (uint): リンクルのしわの数を設定します。既定は3です。

    -----------------------------------------

    wrinkleIntensity (linear): リンクルのしわの深さを設定します。既定は0.5です。
