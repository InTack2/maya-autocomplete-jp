...
def animCurveEditor(areCurvesSelected: bool = False,autoFit: str = "",autoFitTime: str = "",classicMode: bool = False,clipTime: str = "",constrainDrag: int = 1,control: bool = False,curvesShown: bool = False,curvesShownForceUpdate: bool = False,defineTemplate: str = "",denormalizeCurvesCommand: str = "",displayActiveKeyTangents: str = "",displayActiveKeys: str = "",displayInfinities: str = "",displayKeys: str = "",displayNormalized: bool = False,displayTangents: str = "",displayValues: str = "",docTag: str = "",exists: bool = False,filter: str = "",forceMainConnection: str = "",highlightAffectedCurves: bool = False,highlightConnection: str = "",keyMinScale: float = 1.0,keyScale: float = 1.0,keyingTime: str = "",lockMainConnection: bool = False,lockPlayRangeShades: str = "",lookAt: str = "",mainListConnection: str = "",menu: str = "",normalizeCurvesCommand: str = "",outliner: str = "",panel: str = "",parent: str = "",preSelectionHighlight: bool = False,renormalizeCurves: bool = False,resultSamples: float = 1.0,resultScreenSamples: int = 1,resultUpdate: str = "",selectionConnection: str = "",showActiveCurveNames: bool = False,showBufferCurves: str = "",showCurveNames: bool = False,showPlayRangeShades: str = "",showResults: str = "",showUpstreamCurves: bool = False,simpleKeyView: bool = False,smoothness: str = "",snapTime: str = "",snapValue: str = "",stackedCurves: bool = False,stackedCurvesMax: float = 1.0,stackedCurvesMin: float = 1.0,stackedCurvesSpace: float = 1.0,stateString: bool = False,timelinePositionTop: bool = False,unParent: bool = False,unlockMainConnection: bool = False,updateMainConnection: bool = False,useTemplate: str = "",valueLinesToggle: str = "") -> None:
    """
    グラフ エディタの特性を編集します。
    ...
    highlightAffectedCurves (boolean): オンにすると、選択したキー/接線の影響を受けるカーブセグメントがハイライトされます。
    ...
    keyMinScale (float): グラフエディタ内の最小のキー選択サイズです。0.0の値は、最小サイズがないことを意味します。1.0の値は、keyScaleが1.0に設定されているキーのサイズが最小サイズになることを意味します。

    -----------------------------------------

    keyScale (float): グラフエディタでキーサイズを調整します。
    ...
    lockPlayRangeShades (string): 有効な値:「on」、「off」、「tgl」再生範囲のシェーディングをロックします。
    ...
    showPlayRangeShades (string): 有効な値:「on」、「off」、「tgl」再生範囲のシェーディングを表示します。
    ...
    simpleKeyView (boolean): on|offは、より単純で小さなキーの表示を切り替えます。
    ...
    timelinePositionTop (boolean): on|off|tglは、上部または下部のタイムライン表示を切り替えます。
...
def autoKeyframe(addAttr: str = "",characterOption: str = "",listAttr: bool = False,noReset: bool = False,state: bool = False) -> None:
    """
    ...
    characterOption (string): 有効なオプションは、「standard」、「all」です。キャラクタのキーを自動設定する場合に、AutoKeyが標準設定どおりに動作するか(「standard」)、キャラクタアトリビュートすべてにキーを設定するか(「all」)を指定します。既定は「standard」です。
...
def backgroundEvaluationManager(interrupt: bool = False,mode: str = "",pause: bool = False,resume: bool = False) -> None:
    """
    ...
    interrupt (boolean): インタラクティブなワークフロー中にバックグラウンド実行の高速割り込みを有効または無効にします。

    -----------------------------------------

    mode (string): 評価マネージャの現在の評価モードを変更します。サポートされる値は、「serial」と「parallel」です。

    -----------------------------------------

    pause (boolean): バックグラウンド評価を一時停止します。バックグラウンド評価が完全に停止するまでブロックします。現在の状態を取得するために照会することができます。

    -----------------------------------------

    resume (boolean): バックグラウンド評価を再開します。誰かが要求しなくても、一時停止した評価を再開します。
...
def cacheEvaluator(cacheFillMode: str = "",cacheFillOrder: str = "",cacheInvalidate: Tuple[float, float] = tuple(1.0, 1.0),cacheName: str = "",cachedFrames: bool = False,cachingPoints: bool = False,creationParameters: bool = False,delegateEvaluation: bool = False,dynamicsAsyncRefresh: bool = False,dynamicsSupportActive: bool = False,dynamicsSupportEnabled: bool = False,flushCache: str = "",flushCacheRange: Tuple[Tuple[float, float], bool] = tuple(tuple(1.0, 1.0), False),flushCacheSync: bool = False,flushCacheWait: bool = False,hybridCacheMode: str = "",layeredEvaluationActive: bool = False,layeredEvaluationCachingPoints: bool = False,layeredEvaluationEnabled: bool = False,listCacheNames: bool = False,listCachedNodes: bool = False,listValueNames: bool = False,newAction: str = "",newActionParam: str = "",newFilter: str = "",newFilterParam: str = "",newRule: str = "",newRuleParam: str = "",pauseInvalidation: bool = False,preventFrameSkip: bool = False,resetRules: bool = False,resourceUsage: bool = False,resumeInvalidation: bool = False,safeMode: bool = False,safeModeMessages: bool = False,safeModeTriggered: bool = False,valueName: str = "",waitForCache: float = 1.0) -> None:
    """
    ...
    cacheFillMode (string): キャッシュの充填モードを指定します。有効な値は、再生中にキャッシュを充填する「syncOnly」、再生中に、およびバックグラウンドでキャッシュする「syncAsync」、およびバックグラウンドでのみキャッシュを充填する「asyncOnly」です。照会により、現在のモードが返されます。

    -----------------------------------------

    cacheFillOrder (string): キャッシュがタイムラインを充填する順序を指定します。有効な値は、順方向にキャッシュを充填する「forward」、逆方向にキャッシュを充填する「backward」、順方向と逆方向に同時にキャッシュを充填する「bidirectional」、アニメーションの先頭から順方向にキャッシュを充填する「forwardFromBegin」です。照会により、現在のキャッシュ充填モードが返されます。

    -----------------------------------------

    cacheInvalidate (timerange): キャッシュを無効化する必要があるフレームレンジを指定します。範囲は、正の整数のペアとして指定する必要があります。使用例:-ci"10:20"{Pythonequivalent:('10','20')}は、10～20の範囲(現在の時間単位、両方の値を含む)のすべてのフレームを意味します。範囲の一端を省略すると、次の例のように、アニメーション範囲のいずれかの端(または両端)が使用されます。-ci"10:"は、時間10(現在の時間単位)から(タイムライン上の)アニメーション範囲内の最大時間までのすべてのフレームを意味します。-ci":10"は、(タイムライン上の)アニメーション範囲の最大時間から、時間10(現在の時間単位、両方の値を含む)までのすべてのフレームを意味します。-ci":"は、現在のアニメーション範囲の、最小時間から最大時間までのすべてのフレームを指定する短縮形です。

    -----------------------------------------

    cacheName (string): 値の照会元のキャッシュの名前を指定します。照会モードでは、このフラグに値が必要になります。

    -----------------------------------------

    cachedFrames (boolean): 有効なキャッシュデータを持つフレームのリストを取得します。結果は、(cache-status,begin-frame,end-frame)の複数のトリプレットを含む整数配列です。たとえば9個の整数の配列[(0b01,1,3),(0b10,7,10),(0b11,13,15)]のようになります。MELでは、結果は「int[9]」と入力されます。Pythonでは、結果は「Tuple[int,int,int][3]」と入力されます。結果は、フレーム1:3(1,2,3)、7:10(7,8,9,19)、および13:15(13,14,15)がキャッシュされていることを示しています。他のフレームには有効なキャッシュデータは含まれていません。「layeredEvaluationActive」がfalseの場合、cache-status番号は常に1です。「layeredEvaluationActive」がtrueの場合、cache-statusは{1,2,3}のいずれかになります。フレームがアニメーションキャッシュまたはダイナミクスキャッシュのどちらで有効かを表します。エンコーディングは次のとおりです。1(0b01):アニメーションキャッシュのみが有効です2(0b10):ダイナミクスキャッシュのみが有効です3(0b11):アニメーションキャッシュとダイナミクスキャッシュの両方が有効です上記の例は、フレーム1:3はアニメーションキャッシュでのみ有効であることを示しています。フレーム7:10は、ダイナミクスキャッシュでのみ有効です。フレーム13:15は両方のキャッシュで有効で、「完全キャッシュ」と見なされます。

    -----------------------------------------

    cachingPoints (boolean): キャッシングポイントとしてマークされたノード(少なくとも1つのタイプのキャッシュがアクティブなノード)のリストを取得します。

    -----------------------------------------

    creationParameters (boolean): 現在のモードの作成パラメータを取得します。結果は、各ルールの要素を含む配列を表すJSON文字列です。各要素は、ルール作成時のパラメータ名とその値の間の関連付けを表します。

    -----------------------------------------

    delegateEvaluation (boolean): 指定されたノードが評価をデリゲートされているかどうかを返します。

    -----------------------------------------

    dynamicsAsyncRefresh (boolean): ダイナミクスサポートモードで非同期更新を有効/無効にします。これまでは、シミュレーションシステム関連の編集を行うには、ユーザがシーンを再生し直して、結果を確認する必要がありました。非同期更新をアクティブにすると、Mayaはシミュレーションをバックグラウンドで処理し、結果の準備ができた後にビューポートを更新します。フレームに一時的な編集内容が含まれている場合は、自動更新が行われないことに注意してください。たとえば、オブジェクトはキーフレームの設定なしで移動されます。

    -----------------------------------------

    dynamicsSupportActive (boolean): ダイナミクスサポートモードがアクティブかどうかを照会します。ダイナミクスサポートモードは、ヘアや流体などの物理シミュレーションをサポートする場合に使用します。このようなノードがシーン内に検出された場合は、このモードがアクティブになり、「enableDynamicsSupport」がtrueに設定されます。ダイナミクスサポートモードがアクティブな場合は、次の動作に注目してください。キャッシュされていないフレームでは、ダイナミクスノードがフリーズしますタイムスライダに別のダイナミクスキャッシュラインが表示されますダイナミクスキャッシュはアニメーションキャッシュがフィルされた後で開始しますダイナミクスキャッシュはバックグラウンドでのみフィルされますダイナミクスキャッシュは常に最初から前方にフィルされますダイナミクスキャッシュの評価ではフォアグラウンドのダイナミクスノードが更新される場合があります(「dynamicsAsyncRefresh」フラグを参照)

    -----------------------------------------

    dynamicsSupportEnabled (boolean): ダイナミクスノードがキャッシュされた再生に参加できるかどうかを指定します。オフにすると、「セーフモード」がトリガされて、キャッシングが禁止されます。オンにすると、ダイナミクスノードはキャッシングに参加するようになり、「ダイナミクスサポートモード」がトリガされます。詳細については、フラグ「dynamicssupportactive」を確認してください。

    -----------------------------------------

    flushCache (string): 現在のキャッシュをフラッシュするよう指定します。有効な値は、既存のキャッシュをバックアップとして保存する「keep」、および現在のキャッシュを削除する「destroy」です。

    -----------------------------------------

    flushCacheRange ([timerange, boolean]): キャッシュをフラッシュする必要があるフレームレンジを指定します。既定では、これによってキャッシュが破壊されます。「flushCache」も設定されている場合は、フラッシュされるキャッシュ範囲で何を行うべきかを定義します。範囲は、正の整数とブール値のペアとして指定する必要があります。使用例:-flushCacheRange"10:20"on{Pythonequivalent:('10','20',True)}は、10～20の範囲(現在の時間単位、両方の値を含む)のすべてのフレームを意味します。-flushCacheRange"12:18"off{Pythonequivalent:('12','18',False)}は、12より前と18より後(現在の時間単位、両方の値を含まない)のすべてのフレームを意味します。範囲の一端を省略すると、次の例のように、アニメーション範囲のいずれかの端(または両端)が使用されます。-flushCacheRange"10:"onは、時間10(現在の時間単位)から(タイムライン上の)アニメーション範囲内の最大時間までのすべてのフレームを意味します。-flushCacheRange":10"onは、(タイムライン上の)アニメーション範囲の最大時間から、時間10(現在の時間単位、両方の値を含む)までのすべてのフレームを意味します。-flushCacheRange":"onは、現在のアニメーション範囲の、最小時間から最大時間までのすべてのフレームを指定する短縮形です。

    -----------------------------------------

    flushCacheSync (boolean): キャッシュのフラッシュ方法(同期または非同期)を指定します。同期の場合はtrue、非同期の場合はfalseです。

    -----------------------------------------

    flushCacheWait (boolean): キャッシュの破棄が完了するまで待機します。

    -----------------------------------------

    hybridCacheMode (string): ハイブリッドキャッシュモードを指定します。有効な値は、ハイブリッドキャッシュを使用しない「disabled」、(キャッシュするのではなく)スムーズメッシュプレビューを使用する場合にGPUでサポートされるデフォメーションスタックを持つメッシュをGPUで評価する「smp」、GPUでサポートされるデフォメーションスタックを持つすべてのメッシュをGPUで評価する「all」です。照会により、現在のモードが返されます。

    -----------------------------------------

    layeredEvaluationActive (boolean): レイヤ評価モードがアクティブかどうかを照会します。レイヤ評価がアクティブな場合は、バックグラウンドキャッシュフィルプロセスが複数のパスに分割されて、さまざまなコンテンツ(評価ノード)に対応できるようになります。これらのコンテンツは「評価レイヤ」と呼ばれ、アニメーション評価中にそれぞれ異なるLOD(詳細レベル)を表します。例:1番目のレイヤには、キャラクタモーションをなどの通常のアニメーションが含まれています。2番目のレイヤには、キャラクタのヘアやクロスなどのダイナミクスシミュレーションが含まれています。Mayaはレイヤごとに独立したキャッシュおよびキャッシュフィルパスを作成します。これらのレイヤを表す追加のキャッシュバーがタイムスライダのUIに追加されます。各レイヤのバックグラウンドキャッシュフィルパスは順番に開始されます。上の例では、バックグラウンドキャッシュフィルの2つのパスが観察されます。バックグラウンドキャッシュフィル(再生フィル)の最初のパスでは、キャラクタモーションのみが評価およびフィルされ、ヘアおよびクロスはフリーズします。すべてのフレームで2番目のレイヤのキャッシュがフィルされた後に、キャッシュフィルの1番目のパスによってヘアおよびクロスの物理シミュレーションが開始し、2番目のレイヤのキャッシュがフィルされます。1つのフレームで2番目のレイヤのキャッシュがフィルされたら、ユーザはタイムラインをスクラブして、完全に更新されたエフェクトを表示することができます。レイヤ評価がアクティブな場合に、フォアグラウンドで再生または操作を行うと、最初の評価レイヤのみが評価されます。すべてのFXコンテンツは、バックグラウンドシミュレーションが完了するまでビューポート内でフリーズします。たとえば、キャラクタの頭部を回転しても、ヘアはリアルタイムで追従しません。フラグ「dynamicsAsyncRefresh」が有効な場合は、シミュレーションがキャッシュされた後、FXコンテンツは自動的に更新されます。詳細については、フラグを参照してください。

    -----------------------------------------

    layeredEvaluationCachingPoints (boolean): レイヤ評価のためにキャッシングポイントとしてマークされたノードリストを取得します。

    -----------------------------------------

    layeredEvaluationEnabled (boolean): ダイナミクスサポートモードでレイヤ評価を有効/無効にします。レイヤ化された評価が有効な動作の詳細については、フラグ「dynamicsSupportActive」および「layeredEvaluationActive」を参照してください。このフラグは、テストの目的でプラグイン開発者をサポートするために提供されています。本番環境でこのオプションを無効にすることはお勧めしません。無効にすると、ダイナミクスノードは通常のアニメーションと同じキャッシュを共有します。ダイナミクスノードを評価し、フォアグラウンドでキャッシュに保存できます。バックグラウンドの「cacheFillOrder」オプションは「forwardFromBegin」にロックされます。cacheFillModeを「syncOnly」に設定して使用すると、バックグラウンドで評価できないレガシーダイナミクスノードをサポートするために使用することもできます。

    -----------------------------------------

    listCacheNames (boolean): 既存のキャッシュ名のリストを返します。

    -----------------------------------------

    listCachedNodes (boolean): キャッシュされたノードのリストを返します。

    -----------------------------------------

    listValueNames (boolean): 指定したキャッシュに対して照会できる値名のリストを返します。

    -----------------------------------------

    newAction (string): 新しいフィルタ/アクションルール内に作成する新しいアクションの名前を指定します。

    -----------------------------------------

    newActionParam (string): 新しいフィルタ/アクションルール内に作成する新しいアクションに渡すパラメータ文字列を指定します。

    -----------------------------------------

    newFilter (string): 新しいフィルタ/アクションルール内に作成する新しいフィルタの名前を指定します。

    -----------------------------------------

    newFilterParam (string): 新しいフィルタ/アクションルール内に作成する新しいフィルタに渡すパラメータ文字列を指定します。

    -----------------------------------------

    newRule (string): 新規作成するルールの名前を指定します。

    -----------------------------------------

    newRuleParam (string): 新規作成するルールに渡すパラメータ文字列を指定します。

    -----------------------------------------

    pauseInvalidation (boolean): キャッシュのすべての受信無効化を一時停止します。resumeInvalidationフラグを指定して、対称的に操作します。PauseInvalidationは複数回呼び出すことができ、ネストされた状況で役立ちます。無効化を再開するには、再開を同じ回数だけ呼び出す必要があります。照会した場合、キャッシングが一時停止される期間が返されます。0を指定すると、キャッシングが再開されます。

    -----------------------------------------

    preventFrameSkip (boolean): フレームのスキップを有効にするかどうかを指定します。フレームのスキップが有効な場合は次の動作が発生し、再生がリアルタイムで行われるように設定されます。キャッシュをリアルタイムのフレームレートで充填できない場合、フレームはスキップされません。すべてのフレームがループされた(そのため、すべてのフレームがキャッシュされた)後も、キャッシュからの再生をリアルタイムフレームレートで実行できない場合、フレームはスキップされます。すべてのフレームがキャッシュされる前にメモリ制限に到達した場合、フレームはスキップされます。再生時にキャッシュが無効化されている場合は(キャッシュのフラッシュなど)、(キャッシュが再びいっぱいになるまで)フレームはスキップされません。

    -----------------------------------------

    resetRules (boolean): キャッシュ設定ルールをリセットして、ルールセットを空にします。

    -----------------------------------------

    resourceUsage (boolean): リソース使用率の現在の状態を文字列として返します。「unlimited」の場合は、リソース制限が無視されています。「out」の場合は、メモリ制限に達しています。「low」の場合は、メモリ使用率が指定された制限の90%に達しています。「okay」の場合は、メモリ使用率が指定された制限の90%未満です。

    -----------------------------------------

    resumeInvalidation (boolean): キャッシュのすべての受信無効化を再開します。pauseInvalidationフラグを指定して、対称的に操作します。PauseInvalidationは複数回呼び出すことができ、ネストされた状況で役立ちます。無効化を再開するには、再開を同じ回数だけ呼び出す必要があります。照会した場合、キャッシュが再開されている場合はtrue、それ以外の場合はfalseを返します。

    -----------------------------------------

    safeMode (boolean): セーフモードをオンまたはオフにします。照会モードでは、キャッシュエバリュエータのセーフモードのステータスを返します。

    -----------------------------------------

    safeModeMessages (boolean): セーフモードのメッセージをコンソールに出力します。

    -----------------------------------------

    safeModeTriggered (boolean): キャッシュエバリュエータに対してセーフモードがトリガされたかどうかを返します。

    -----------------------------------------

    valueName (string): 値を照会するパラメータの名前を指定します。照会モードでは、このフラグに値が必要になります。

    -----------------------------------------

    waitForCache (float): キャッシュがバックグラウンドで充填されるまで待機するよう指定します。[待機時間(秒)]でタイムアウトします。

    -----------------------------------------

    Return Value:
    None: stringメモリ制限に達したかどうかを示す状態(「resourceUsage」フラグで「out」、「okay」、「low」、または「unlimited」を指定)booleanセーフ モードを有効にするかどうかを示す状態(「safeMode」フラグを指定)booleanセーフ モードがトリガされたかどうかを示す状態(「safeModeTriggered」フラグを指定)booleanフレームのスキップ禁止が有効かどうかを示す状態(「preventFrameSkip」フラグを指定)booleanバックグラウンドでキャッシュが計算されたかどうかを示す状態(「waitForCache」フラグを指定)string[]使用可能なキャッシュ名(「listCacheNames」フラグを指定)。stringキャッシュ エバリュエータによって現在キャッシュされているノードのリスト(「listCachedNodes」フラグを指定)string[]使用可能な値の名前(「listValueNames」フラグを指定)string[]要求されたノードのパラメータ値(「cacheName」フラグを指定)string[]要求されたノードでデリゲート評価が有効かどうかの状態(「delegateEvaluation」フラグを指定)stringJSON 配列として表された、現在モードの作成パラメータ(「creationParameters」フラグを指定)string[]キャッシング ポイントとしてマークされたノードのリスト(「cachingPoints」フラグを指定)string[]レイヤ評価のためにキャッシング ポイントとして強制されるノードのリスト(「layeredEvaluationCachingPoints」フラグを指定)int[]キャッシュされるフレームのリスト(「cachedFrames」フラグを指定)string現在のキャッシュ充填モード(「cacheFillMode」フラグを指定)string現在のキャッシュ充填順序(「cacheFillOrder」フラグを指定)stringすべてのセーフ モード メッセージのリスト(「safeModeMessages」フラグを指定)string現在のハイブリッド キャッシュ モード(「hybridCacheMode」フラグを指定)照会モードでは、戻り値のタイプは照会されたフラグに基づきます。
    """
    pass

    
def character(addElement: str = "",addOffsetObject: str = "",anyMember: str = "",characterPlug: bool = False,clear: str = "",empty: bool = False,excludeDynamic: bool = False,excludeRotate: bool = False,excludeScale: bool = False,excludeTranslate: bool = False,excludeVisibility: bool = False,flatten: str = "",forceElement: str = "",include: str = "",intersection: str = "",isIntersecting: str = "",isMember: str = "",library: bool = False,memberIndex: int = 1,name: str = "",noWarnings: bool = False,nodesOnly: bool = False,offsetNode: bool = False,remove: str = "",removeOffsetObject: str = "",root: str = "",scheduler: bool = False,split: str = "",subtract: str = "",text: str = "",union: str = "",userAlias: str = "") -> None:
    """
    ...
    addElement (name): 指定したキャラクタに項目リストを追加します。別のキャラクタにあるためキャラクタに追加できない項目がある場合、このコマンドは失敗します。別のキャラクタは-addElementに渡されると、サブキャラクタとして追加されます。ノードが渡されるとキー設定可能なアトリビュートに展開されて、キャラクタに追加されます。

    -----------------------------------------

    addOffsetObject (string): オフセットを計算および適用するときに、選択したキャラクタメンバーオブジェクトを使用する必要があることを示します。キャラクタの指定には、フラグ引数を使用します。

    -----------------------------------------

    anyMember (name): 指定した項目のいずれかが指定したセットのメンバーであるかどうかをテストします。

    -----------------------------------------

    characterPlug (boolean): 指定したキャラクタメンバーに対応するキャラクタのプラグを返します。

    -----------------------------------------

    clear (name): 指定したキャラクタからすべての項目を除去する操作です。

    -----------------------------------------

    empty (boolean): 作成するキャラクタを空にする必要があることを示します。つまり、キャラクタに追加するオブジェクトを識別する引数がすべて無視されます。

    -----------------------------------------

    excludeDynamic (boolean): キャラクタを作成するときに、ダイナミックアトリビュートを除外します。

    -----------------------------------------

    excludeRotate (boolean): キャラクタを作成するときに、Transform-typeノードからRotateアトリビュートを除外します。

    -----------------------------------------

    excludeScale (boolean): キャラクタを作成するときに、Transform-typeノードからScaleアトリビュートを除外します。

    -----------------------------------------

    excludeTranslate (boolean): キャラクタを作成するときに、Transform-typeノードからTranslateアトリビュートを除外します。たとえば、キャラクタにジョイントだけが含まれている場合に、そのキャラクタに回転だけを含めたいことがあります。

    -----------------------------------------

    excludeVisibility (boolean): キャラクタを作成するときに、Transform-typeノードからVisibilityアトリビュートを除外します。

    -----------------------------------------

    flatten (name): 指定したキャラクタの構造を平坦化する操作です。つまり、指定したキャラクタに含まれるキャラクタはすべてそのメンバーで置き換えられ、そのキャラクタはほかのキャラクタではなく、キャラクタのメンバーを含むようになります。

    -----------------------------------------

    forceElement (name): 編集モードのみで使用してください。項目を強制的にキャラクタに追加します。項目がcharacterPartitionにある別のキャラクタにあれば、別のキャラクタから除去され、characterPartitionのキャラクタは通常メンバーシップに対して相互排他的になります。

    -----------------------------------------

    include (name): 指定したキャラクタに項目リストを追加します。キャラクタに追加できない項目がある場合は、警告メッセージが発生します。これは、-add/addElement操作よりも厳密性に欠けます。

    -----------------------------------------

    intersection (name): リストのキャラクタすべてのメンバーである項目のリストを返す操作です。通常、キャラクタは相互排他的にする必要があります。

    -----------------------------------------

    isIntersecting (name): リストのキャラクタが共通メンバーを持つかどうかをテストする操作です。通常、キャラクタは、常にfalseを返すように相互排他的にする必要があります。

    -----------------------------------------

    isMember (name): 指定した項目がすべて指定したキャラクタのメンバーであるかどうかをテストする操作です。

    -----------------------------------------

    library (boolean): このキャラクタに関連付けられたクリップライブラリがあれば返します。クリップライブラリがあるのは、キャラクタにクリップを作成した場合だけです。

    -----------------------------------------

    memberIndex (uint): Queryフラグの後に使用すると、指定したキャラクタメンバーのmemberIndexを返します。または、照会フラグの前に使用すると、指定したインデックスに対応するメンバーを返します。

    -----------------------------------------

    name (string): 新しいキャラクタの名前として文字列を割り当てます。新しいキャラクタを作成する操作に対して有効です。

    -----------------------------------------

    noWarnings (boolean): 無効な項目をキャラクタに追加する場合などに、警告メッセージが発生してはならないことを示します(UIで使用)。

    -----------------------------------------

    nodesOnly (boolean): このフラグは、メンバーシップが照会するキャラクタの結果を修正します。キャラクタに含まれたsphere1.txなどのアトリビュートをリストするとき、ノードだけをリストします。キャラクタにノードのアトリビュートまたはコンポーネントが複数存在する場合でも、各ノードは一度だけリストされます。

    -----------------------------------------

    offsetNode (boolean): キャラクタのルートにオフセットを追加するために使用するcharacterOffsetノードの名前を返します。

    -----------------------------------------

    remove (name): 指定したキャラクタの項目リストを除去します。

    -----------------------------------------

    removeOffsetObject (string): 選択したキャラクタオフセットオブジェクトをオフセットとして除去する必要があることを示します。キャラクタの指定には、フラグ引数を使用します。

    -----------------------------------------

    root (string): 作成しているキャラクタのルートとして動作するトランスフォームノードを指定します。このコマンドは、キャラクタノードの他にcharacterOffsetノードを作成します。characterOffsetノードを使えば、キャラクタにオフセットを追加して、階層内にノードを追加しなくてもキャラクタのアニメーションの方向を変更することができます。

    -----------------------------------------

    scheduler (boolean): このキャラクタに関連付けられたスケジューラがあれば返します。スケジューラがあるのは、キャラクタにクリップを作成した場合だけです。
    ...
    subtract (name): 2つのキャラクタ間の操作で、2番目のキャラクタにない最初のキャラクタのメンバーを返します。通常、キャラクタは相互排他的にする必要があります。

    -----------------------------------------

    text (string): キャラクタと共に格納される注釈の文字列を定義します。

    -----------------------------------------

    union (name): リストされたキャラクタすべてのリストを返す操作です。

    -----------------------------------------

    userAlias (name): キャラクタの指定したアトリビュートのユーザ定義エイリアスを返します。定義されていなければ、空の文字列を返します。キャラクタは、キャラクタアニメーションデータを格納するアトリビュートに自動的にエイリアスを作成します。自動的に設定されたエイリアスがaliasAttrコマンドで置き換えられた場合、ユーザが設定したエイリアスが存在します。

    -----------------------------------------

    Return Value:
    None: string作成操作(作成または編集されたキャラクタの名前)string[]照会操作(キャラクタ内の項目名)booleanisMember 操作照会モードでは、戻り値のタイプは照会されたフラグに基づきます。
    """
    pass

    
def characterMap(mapAttr: Tuple[str, str] = tuple("", ""),mapMethod: str = "",mapNode: Tuple[str, str] = tuple("", ""),mapping: str = "",proposedMapping: bool = False,unmapAttr: Tuple[str, str] = tuple("", ""),unmapNode: Tuple[str, str] = tuple("", "")) -> None:
    """
    ...
    mapAttr ([string, string]): 照会モードでこのフラグを使って、指定したマップノードに格納されたマッピング情報を照会でき、文字配列が返されます。照会モード以外では、このフラグを使って指定したキャラクタメンバーのマッピング関係を作成できます。新しく指定したマッピングが優先され、以前のアトリビュートのマッピング情報はすべて除去されます。

    -----------------------------------------

    mapMethod (string): これは作成モードのみで有効で、マッピングの方法を指定します。有効なオプションは、「byNodeName」、「byAttrName」、「byAttrOrder」で、既定は「byAttrOrder」です。これらのフラグの意味は次の通りです。「byAttrOrder」は、キャラクタが内部的にアトリビュートを格納する順番を使用してマッピングします。「byAttrName」は、アトリビュート名を使用して一致を検索します。「byNodeName」はノード名とアトリビュート名を使用して一致を検索します。

    -----------------------------------------

    mapNode ([string, string]): このフラグを使用して、ソースノードのすべてのアトリビュートを対象ノードの一致するアトリビュートにマッピングできます。

    -----------------------------------------

    mapping (string): このフラグは照会モードのみで有効です。文字列引数がある照会フラグの前に使用する必要があります。特定のアトリビュートのマッピングを照会するために使用し、文字配列が返されます。

    -----------------------------------------

    proposedMapping (boolean): このフラグは照会モードのみで有効です。指定したキャラクタと指定したmappingMethod(オプション)でコールした場合、キャラクタマップが提供するマッピングの配列を取得するために使用します。キャラクタにキャラクタマップが存在する場合、マップは照会操作の影響を受けず、文字配列が返されます。

    -----------------------------------------

    unmapAttr ([string, string]): このフラグを使用して、指定したマップノードに格納されたマッピングを解除できます。

    -----------------------------------------

    unmapNode ([string, string]): このフラグを使用して、ソースノードのすべてのアトリビュートと対象ノードの一致するアトリビュートとのマッピングを解除できます。マッピングされたアトリビュートのうち、名前が一致しないものはマッピング解除されません。

    -----------------------------------------

    Return Value:
    None: stringcharacterMap 名照会モードでは、戻り値のタイプは照会されたフラグに基づきます。
    """
    pass
...
def clip(absolute: bool = False,absoluteRotations: bool = False,active: str = "",addTrack: bool = False,allAbsolute: bool = False,allClips: bool = False,allRelative: bool = False,allSourceClips: bool = False,animCurveRange: bool = False,character: bool = False,constraint: bool = False,copy: bool = False,defaultAbsolute: bool = False,duplicate: bool = False,endTime: float = 1.0,expression: bool = False,ignoreSubcharacters: bool = False,isolate: bool = False,leaveOriginal: bool = False,mapMethod: str = "",name: str = "",newName: str = "",paste: bool = False,pasteInstance: bool = False,remove: bool = False,removeTrack: bool = False,rotationOffset: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),rotationsAbsolute: bool = False,scheduleClip: bool = False,sourceClipName: bool = False,split: float = 1.0,startTime: float = 1.0,translationOffset: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),useChannel: str = "") -> None:
    """
    ...
    absolute (boolean): このフラグは非推奨になりました。代わりにaa/allAbsolute、ar/allRelative、ra/rotationsAbsolute、da/defaultAbsoluteを使用してください。このフラグを使用して、クリップがキーフレーム値に追従するか、または滑らかなパスを維持するために一定の値でオフセットするかを制御します。既定はtrueです。

    -----------------------------------------

    absoluteRotations (boolean): このフラグは非推奨になりました。代わりにaa/allAbsolute、ar/allRelative、ra/rotationsAbsolute、da/defaultAbsoluteを使用してください。trueであれば-absoluteフラグをオーバーライドし、回転チャネルが常に絶対オフセットで計算されるようになります。これにより、回転には絶対オフセットを、その他すべてのチャネルには相対オフセットを適用することができます。

    -----------------------------------------

    active (string): アクティブクリップを照会、または編集します。このフラグは作成モードでは無効です。クリップをアクティブにすると、そのアニメーションカーブがクリップライブラリノードにアタッチされ、さらにキャラクタアトリビュートに直接関係づけられます。これにより、クリップに追加のアニメーションカーブを編集、削除、クリップする場合に、より簡単にアニメーションカーブにアクセスできるようになります。

    -----------------------------------------

    addTrack (boolean): このフラグは現在ではサポートしていません。代わりに、clipScheduleコマンドのinsertTrackフラグを使用してください。

    -----------------------------------------

    allAbsolute (boolean): すべてのチャネルを絶対オフセットで計算するように設定します。このフラグはar/allRelative、ra/rotationsAbsolute、da/defaultAbsoluteの各フラグと一緒に使用することはできません。

    -----------------------------------------

    allClips (boolean): このフラグを使用して、シーン内のすべてのクリップを照会します。ポーズを格納する「animClip」タイプのノードは、このコマンドでは返されません。

    -----------------------------------------

    allRelative (boolean): すべてのチャネルを相対オフセットで計算するように設定します。このフラグはaa/allAbsolute、ra/rotationsAbsolute、da/defaultAbsoluteの各フラグと一緒に使用することはできません。

    -----------------------------------------

    allSourceClips (boolean): このフラグを使用して、シーン内のすべてのソースクリップを照会します。ポーズやクリップのインスタンスを格納する「animClip」タイプのノードは、このコマンドでは返されません。

    -----------------------------------------

    animCurveRange (boolean): このフラグはstartTimeフラグやendTimeフラグの代わりに、クリップ作成時に使用することができます。そのクリップの範囲を、クリップに接続されたアニメーションカーブのキー範囲の全長に渡るように指定することができます。

    -----------------------------------------

    character (boolean): 指定したクリップに動作する、照会専用フラグです。クリップに接続されたキャラクタの名前を返します。

    -----------------------------------------

    constraint (boolean): このフラグは、キャラクタ上の任意のコンストレイントからクリップを作成します。コンストレイントはキャラクタからクリップに移動します。コンストレイントはクリップの継続期間のみ有効で、その値はクリップのアトリビュートに従ってスケール、オフセット、循環することができます。

    -----------------------------------------

    copy (boolean): このフラグを使用して、1つまたは複数のクリップをクリップボードにコピーします。nameフラグと一緒に使用し、名前がついたクリップを指定したキャラクタとそのサブキャラクタにコピーします。照会モードでこのフラグを使用すると、クリップボードにコピーされた内容を照会することができます。

    -----------------------------------------

    defaultAbsolute (boolean): クリップ内にある回転以外のすべての最上位チャネルを相対オフセットに、残りのチャネルを絶対オフセットに設定します。offsetフラグが指定されていない場合、クリップ作成時の既定の設定です。このフラグはaa/allAbsolute、ar/allRelative、ra/rotationsAbsoluteの各フラグと一緒に使用することはできません。

    -----------------------------------------

    duplicate (boolean): nameフラグで指定したクリップを複製します。新規クリップの開始時間はstartTimeフラグで指定します。

    -----------------------------------------

    endTime (time): クリップの終了時間を指定

    -----------------------------------------

    expression (boolean): このフラグはキャラクタ上の任意のエクスプレッションからクリップを作成します。エクスプレッションはキャラクタからクリップに移動します。エクスプレッションはクリップの継続期間のみ有効で、その値はクリップのアトリビュートに従ってスケール、オフセット、循環することができます。

    -----------------------------------------

    ignoreSubcharacters (boolean): クリップの作成時、複製時、分離時には、サブキャラクタは既定で含まれます。最上位キャラクタのみにクリップを作成する場合、またはサブキャラクタを含めずに最上位キャラクタのクリップを複製する場合はignoreSubcharactersフラグを使用します。

    -----------------------------------------

    isolate (boolean): このフラグはnameフラグと一緒に使用し、1つまたは複数のクリップを新規のクリップライブラリにコピーするように指定します。このフラグの最も一般的な用途は、キャラクタのすべてのクリップではなく、特定のクリップのみの書き出しです。

    -----------------------------------------

    leaveOriginal (boolean): このフラグはクリップ作成時に使用し、アニメーションカーブをクリップライブラリにコピーし、キャラクタ上に維持するように指定します。

    -----------------------------------------

    mapMethod (string): これはpasteフラグとpasteInstanceフラグのみに有効で、マッピングの方法を指定します。有効なオプションは、「byNodeName」、「byAttrName」、「byCharacterMap」、「byAttrOrder」、「byMapOrAttrName」、「byMapOrNodeName」で、「byAttrName」が既定です。これらのフラグの意味は次の通りです。「byAttrOrder」は、キャラクタが内部的にアトリビュートを格納する順番を使用してマッピングます。「byAttrName」は、アトリビュート名を使用して一致を検索します。「byNodeName」はノード名とアトリビュート名を使用して一致を検索します。「byCharacterMap」は既存のcharacterMapノードを使用してマッピングします。「byMapOrAttrName」は、キャラクタマップが存在すればそれを使用しますが、存在しなければアトリビュート名を使用します。「byMapOrNodeName」は、キャラクタマップが存在すればそれを使用しますが、存在しなければアトリビュート名を使用します。

    -----------------------------------------

    name (string): 作成モードでは、クリップ名を指定します。照会モードでは、すべてのクリップのリストを返します。複製モードでは、複製するクリップを指定します。コピーモードでは、コピーするクリップを指定します。このフラグは多目的ですが、多重使用はコピーフラグのみがサポートしています。作成中の使用とその他のすべてのフラグの場合、nameフラグの最初のインスタンスのみが使用されます。照会モードでは、このフラグは値を受け入れることができます。

    -----------------------------------------

    newName (string): クリップの名前を変更します。このフラグは、名前を変更するクリップを指定するnameフラグと一緒に使用する必要があります。

    -----------------------------------------

    paste (boolean): このフラグを使用して、クリップボードのクリップをキャラクタにペーストします。クリップは、c/copyフラグを使用してクリップボードに追加します。

    -----------------------------------------

    pasteInstance (boolean): このフラグを使用して、クリップボードのクリップのインスタンスをキャラクタにペーストします。オリジナルのソースクリップからアニメーションカーブを複製するp/pasteフラグと異なり、pi/pasteInstanceフラグはソースクリップからのアニメーションカーブを共有します。

    -----------------------------------------

    remove (boolean): nameフラグで指定したクリップを除去します。クリップはライブラリから完全に除去され、またスケジュールされていたすべての時間からも削除されます。

    -----------------------------------------

    removeTrack (boolean): このフラグは現在ではサポートしていません。代わりに、clipScheduleコマンドのremoveTrackフラグを使用してください。

    -----------------------------------------

    rotationOffset ([float, float, float]): クリップの回転の修正に使用されているチャネルオフセットを返します。

    -----------------------------------------

    rotationsAbsolute (boolean): 回転以外のすべてのチャネルを相対オフセットで計算するように設定します。回転チャネルは絶対オフセットで計算されます。このフラグはaa/allAbsolute、ar/allRelative、da/defaultAbsoluteの各フラグと一緒に使用することはできません。

    -----------------------------------------

    scheduleClip (boolean): このフラグを使用して、クリップ作成時にそのクリップを現在のタイムにスケジュールするかどうかを指定します。クリップがスケジュールされていない場合は、将来的な使用のためにライブラリに配置されますが、タイムスライダには配置されません。このフラグは、新規クリップの作成時、または既存クリップの複製時のみに使用します。既定はtrueです。

    -----------------------------------------

    sourceClipName (boolean): このフラグは照会専用で、インスタンス化したクリップを制御するソースクリップの名前を返します。

    -----------------------------------------

    split (time): 既存のクリップを2つに分割します。この分割は、指定した時間の前後に実行されます。

    -----------------------------------------

    startTime (time): クリップの開始時間を指定

    -----------------------------------------

    translationOffset ([float, float, float]): クリップの移動の修正に使用されているチャネルオフセットを返します。

    -----------------------------------------

    useChannel (string): 使用するチャネルを指定します。このフラグは、クリップ作成とisolateフラグと一緒に使用した場合のみに有効です。指定するチャネルはキャラクタのメンバーである必要があります。

    -----------------------------------------

    Return Value:
    None: string[]クリップ名照会モードでは、戻り値のタイプは照会されたフラグに基づきます。
    """
    pass

    
def clipEditor(allTrackHeights: int = 1,autoFit: str = "",autoFitTime: str = "",clipDropCmd: str = "",clipStyle: int = 1,control: bool = False,defineTemplate: str = "",deleteCmd: str = "",deselectAll: bool = False,displayActiveKeyTangents: str = "",displayActiveKeys: str = "",displayInfinities: str = "",displayKeys: str = "",displayTangents: str = "",displayValues: str = "",docTag: str = "",exists: bool = False,filter: str = "",forceMainConnection: str = "",frameAll: bool = False,frameRange: Tuple[float, float] = tuple(1.0, 1.0),highlightConnection: str = "",highlightedBlend: Tuple[str, str] = tuple("", ""),highlightedClip: Tuple[str, str] = tuple("", ""),initialized: bool = False,listAllCharacters: bool = False,listCurrentCharacters: bool = False,lockMainConnection: bool = False,lookAt: str = "",mainListConnection: str = "",manageSequencer: bool = False,menuContext: str = "",panel: str = "",parent: str = "",selectBlend: Tuple[str, str, str] = tuple("", "", ""),selectClip: Tuple[str, str] = tuple("", ""),selectionConnection: str = "",snapTime: str = "",snapValue: str = "",stateString: bool = False,unParent: bool = False,unlockMainConnection: bool = False,updateMainConnection: bool = False,useTemplate: str = "") -> None:
    """
    ...
    allTrackHeights (int): このフラグは現在サポートしていません。代わりにclipStyleを使用します。
    ...
    clipDropCmd (string): クリップノードがTraxエディタにドロップされたときに実行されるコマンドです。

    -----------------------------------------

    clipStyle (int): 指定したエディタにあるクリップトラックのスタイルを設定または返します。既定は2です。有効な値は1から3です。
    ...
    deleteCmd (string): backspaceキーを押すと実行されるコマンド

    -----------------------------------------

    deselectAll (boolean): エディタのすべてのクリップとブレンドを選択解除します。
    ...
    frameAll (boolean): エディタのすべてのクリップのフレームビューです。

    -----------------------------------------

    frameRange ([float, float]): エディタの現在のフレーム範囲。
    ...
    highlightedBlend ([string, string]): スケジューラとインデックスに従いハイライトされたブレンドを返します。

    -----------------------------------------

    highlightedClip ([string, string]): スケジューラとインデックスに従いハイライトされたクリップを返します。

    -----------------------------------------

    initialized (boolean): クリップエディタが完全に初期化されていて、描画するポートがあるかどうかを返します。初期化されていない場合、-frameRangeフラグと-frameAllフラグは失敗します。

    -----------------------------------------

    listAllCharacters (boolean): エディタとアウトライナ(Outliner)にあるすべてのキャラクタのリストです。

    -----------------------------------------

    listCurrentCharacters (boolean): エディタとアウトライナ(Outliner)のみにあるキャラクタのリストです。
    ...
    manageSequencer (boolean): クリップエディタでシーケンサノードを扱うかどうかを設定または返します。扱う場合、アニメーションクリップとキャラクタは表されません。

    -----------------------------------------

    menuContext (string): カーソルの下のデータオブジェクトのタイプを表す文字配列を返します。戻り値:{"timeSlider"}{"nothing"}{"track","trackindex","characternodename","groupname"}{"clip","clipnodename"}
    ...
    selectBlend ([string, string, string]): スケジューラの名前と、ブレンドで使用された2つのクリップのインデックスで指定したブレンドを選択します。照会すると、スケジューラの名前と選択したブレンドのすべての2つのクリップインデックスが返されます。

    -----------------------------------------

    selectClip ([string, string]): スケジューラの名前とクリップインデックスで指定したクリップを選択します。照会すると、選択したすべてのクリップのスケジューラとクリップインデックスが含まれる文字列が返されます。
...
def clipMatching(clipDst: Tuple[str, float] = tuple("", 1.0),clipSrc: Tuple[str, float] = tuple("", 1.0),matchRotation: int = 1,matchTranslation: int = 1) -> None:
    """
    ...
    clipDst ([string, float]): ソースクリップを正しくオフセットするために一致させるクリップです。このフラグは、ソースクリップが目的のクリップの特定の時間と一致するよう、クリップ名と0.0～1.0のパーセンテージ値を取得します。

    -----------------------------------------

    clipSrc ([string, float]): 目的のクリップと位置合わせるためにオフセットするクリップです。このフラグは、クリップの特定の時間と一致するよう、クリップ名と0.0～1.0のパーセンテージ値を取得します。

    -----------------------------------------

    matchRotation (uint): このフラグは回転の一致タイプを設定します。既定では回転と一致しないように設定されています。0-なし1-完全な回転と一致2-地表プレーンに投影された回転と一致

    -----------------------------------------

    matchTranslation (uint): このフラグは移動の一致タイプを設定します。既定では移動と一致しないように設定されています。0-なし1-完全な移動と一致2-地表プレーンに投影された移動と一致
...
def clipSchedule(allAbsolute: bool = False,allRelative: bool = False,blend: Tuple[int, int] = tuple(1, 1),blendNode: Tuple[int, int] = tuple(1, 1),blendUsingNode: str = "",character: bool = False,clipIndex: int = 1,cycle: float = 1.0,defaultAbsolute: bool = False,enable: bool = False,group: bool = False,groupIndex: int = 1,groupName: str = "",hold: float = 1.0,insertTrack: int = 1,instance: str = "",listCurves: bool = False,listPairs: bool = False,lock: bool = False,mute: bool = False,name: str = "",postCycle: float = 1.0,preCycle: float = 1.0,remove: bool = False,removeBlend: Tuple[int, int] = tuple(1, 1),removeEmptyTracks: bool = False,removeTrack: int = 1,rotationsAbsolute: bool = False,scale: float = 1.0,shift: int = 1,shiftIndex: int = 1,solo: bool = False,sourceClipName: bool = False,sourceEnd: float = 1.0,sourceStart: float = 1.0,start: float = 1.0,track: int = 1,weight: float = 1.0,weightStyle: int = 1) -> None:
    """
    ...
    blend ([uint, uint]): このフラグを使用して2つのクリップをブレンドし、そのインデックスはフラグの引数として提供されます。

    -----------------------------------------

    blendNode ([uint, uint]): これは照会専用フラグで、2つのクリップインデックスで定義されたブレンドに接続される、すべてのブレンドノードのリストを作成します。文字配列が返されます。照会モードでは、このフラグは値を受け入れることができます。

    -----------------------------------------

    blendUsingNode (string): このフラグを使用して、既存のブレンドノードでブレンドします。blendフラグと一緒に使用します。blendフラグはブレンドのクリップインデックスを指定します。既存のanimBlendノードの名前を、blendUsingNodeフラグの引数として指定する必要があります。

    -----------------------------------------

    character (boolean): このフラグを使用して、このスケジューラがコントロールするキャラクタを照会します。このフラグは文字配列を返します。

    -----------------------------------------

    clipIndex (uint): スケジュールするクリップインデックスを指定します。照会モードでは、次の形式の文字配列を返します:(clipName,index,start,sourceStart,sourceEnd,scale,preCycle,postCycle)照会モードでは、このフラグは値を受け入れることができます。

    -----------------------------------------

    cycle (float): このフラグは現在ではサポートしていません。代わりにpostCycleフラグを使用してください。
    ...
    enable (boolean): このフラグを使用して、クリップを有効/無効にします。ci/clipIndexフラグと一緒に使用する必要があります。指定したクリップが有効または無効になります。

    -----------------------------------------

    group (boolean): このフラグを使用して、(groupIndexで指定した)クリップのリストをグループに追加(true)またはグループから除去(false)します。

    -----------------------------------------

    groupIndex (uint): このフラグはグループに追加する、またはグループから除去する複数のクリップを指定します。

    -----------------------------------------

    groupName (string): このフラグを使用して、追加するグループを指定します。指定した名前のグループが存在しない場合は、その名前で新しいグループが作成されます。既定では、このフラグを指定しない限り、新しいグループが作成されます。

    -----------------------------------------

    hold (time): クリップの正常な終了後または循環された終了後に、最後の値を保持する時間を指定します。
    ...
    instance (string): 名前をつけたクリップのインスタンス化されたコピーを作成します。インスタンス化されたクリップとは、オリジナルのクリップにリンクされたもので、オリジナルカーブのアニメーションカーブを変更すると、すべてのインスタンス化されたクリップも修正されます。インスタンス化されたクリップの名前は文字列として返されます。

    -----------------------------------------

    listCurves (boolean): このフラグを使用して、クリップに接続されたアニメーションカーブのリストを作成します。目的のクリップを指定するclipIndexフラグと一緒に使用する必要があります。

    -----------------------------------------

    listPairs (boolean): これは照会モード専用のフラグで、クリップが使用するキャラクタのチャネルとそのチャネルをドライブするアニメーションカーブの名前を含む文字配列を返します。文字配列のそれぞれの文字列は、チャネル名、スペース、そのチャネルをアニメートするアニメーションカーブ名という構成になっています。このフラグは、ci/clipIndexフラグと一緒に使用する必要があります。

    -----------------------------------------

    lock (boolean): このフラグは、トラック上のクリップをロックするかどうかを指定します。trackフラグと一緒に使用する必要があります。

    -----------------------------------------

    mute (boolean): このフラグは、トラック上のクリップをミュートするかどうかを指定します。trackフラグと一緒に使用する必要があります。

    -----------------------------------------

    name (string): このフラグを使用して、指定したクリップインデックスに接続されたクリップノードの名前を照会する、またはインスタンス時にインスタンス化されるクリップの名前を指定します。照会モードでは、このフラグは値を受け入れることができます。

    -----------------------------------------

    postCycle (float): クリップが正常に終了した後に繰り返す回数を指定します。

    -----------------------------------------

    preCycle (float): クリップが正常に開始する前に繰り返す回数を指定します。

    -----------------------------------------

    remove (boolean): このフラグを使用して、クリップをタイムスライダから除去します。ci/clipIndexフラグと一緒に使用する必要があります。指定したクリップはタイムスライダから除去されますが、ライブラリには存在し、インスタンス化されたすべてのクリップはタイムスライダに残ります。クリップをシーンから完全に除去するには、clipコマンドを使用します。

    -----------------------------------------

    removeBlend ([uint, uint]): このフラグを使用して、2つのクリップの既存のブレンドを除去します。2つのクリップインデックスは、フラグ引数として提供されます。
    ...
    scale (float): クリップをスケールする量を指定します。0より大きな値を指定する必要があります。

    -----------------------------------------

    shift (int): このフラグはshiftIndexフラグと一緒に使用し、複数のクリップを一定数のトラック分移動することができます。このフラグは接続されたクリップを移動するトラック数を指定します。正の値はクリップを下方に、負の値はクリップを上方に移動します。

    -----------------------------------------

    shiftIndex (uint): このフラグはshiftAmountフラグと一緒に使用し、複数のクリップを一定数のトラック分移動することができます。このフラグは移動するクリップインデックスを指定します。このフラグは、コマンドラインで複数回使用し、移動するクリップの数を指定することができます。

    -----------------------------------------

    solo (boolean): このフラグは、トラック上のクリップを単独操作するかどうかを指定します。trackフラグと一緒に使用する必要があります。

    -----------------------------------------

    sourceClipName (boolean): このフラグを使用して、指定したクリップインデックスに接続されたソースクリップノードの名前を照会します。

    -----------------------------------------

    sourceEnd (time): ソースクリップのアニメーションカーブの終了位置を指定します。

    -----------------------------------------

    sourceStart (time): ソースクリップのアニメーションカーブの開始位置を指定します。

    -----------------------------------------

    start (time): クリップの開始位置を指定します。

    -----------------------------------------

    track (uint): 操作対象のトラックを指定します。たとえば、クリップを配置するトラックや、ミュート、ロック、単独操作を行うトラックを指定します。照会モードでclipIndexフラグと一緒に使用して、クリップのトラック番号を返すことができます。この場合、トラック1がキャラクタの最初のトラックになります。照会モードでは、このフラグは値を受け入れることができます。

    -----------------------------------------

    weight (float): このフラグを使用して、指定したクリップインデックスのウェイトを設定、または照会します。

    -----------------------------------------

    weightStyle (uint): このフラグを使用して、指定したクリップインデックスに接続されたクリップのweightStyleアトリビュートを設定、または照会します。

    -----------------------------------------

    Return Value:
    None: stringクリップ名照会モードでは、戻り値のタイプは照会されたフラグに基づきます。
    """
    pass

    
def clipSchedulerOutliner(annotation: str = "",backgroundColor: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),clipScheduler: str = "",defineTemplate: str = "",docTag: str = "",dragCallback: str = "",dropCallback: str = "",enable: bool = False,enableBackground: bool = False,enableKeyboardFocus: bool = False,exists: bool = False,fullPathName: bool = False,height: int = 1,highlightColor: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),isObscured: bool = False,manage: bool = False,noBackground: bool = False,numberOfPopupMenus: bool = False,parent: str = "",popupMenuArray: bool = False,preventOverride: bool = False,statusBarMessage: str = "",useTemplate: str = "",visible: bool = False,visibleChangeCommand: str = "",width: int = 1) -> None:
    """
    このコマンドは、クリップ スケジューラのアウトライナ(Outliner)コントロールを作成/編集/照会します。
    ...
    clipScheduler (string): 情報を表示するクリップスケジューラの名前を指定します。
...
def deformerEvaluator(chains: bool = False,deformers: bool = False,dumpInfo: bool = False,meshes: bool = False,verbose: bool = False) -> None:
    """
    ...
    deformers (boolean): 現在登録されているすべてのGPUデフォーマのリストを出力します。

    -----------------------------------------

    dumpInfo (boolean): サポートされているすべてのデフォメーションチェーンに関する情報をJSON形式で出力します。
    ...
    verbose (boolean): その他のフラグの詳細を出力します。
...
def evaluationManager(cycleCluster: str = "",disableInfo: str = "",empty: bool = False,enabled: bool = False,fallbackTriggered: bool = False,idleAction: int = 1,idleBuild: bool = False,invalidate: bool = False,manipulation: bool = False,manipulationPrevalidation: bool = False,manipulationReady: bool = False,mode: str = "",downstreamFrom: str = "",nodeTypeGloballySerialize: bool = False,nodeTypeParallel: bool = False,nodeTypeSerialize: bool = False,nodeTypeUntrusted: bool = False,upstreamFrom: str = "",safeMode: bool = False) -> None:
    """
    ...
    disableInfo (string): 評価マネージャが無効になった理由が含まれている文字列のリストを返します(サポートされていないノードタイプまたはアトリビュート値が見つかったなどの理由により、意図的にオフにした場合とは異なります)。リストが空の場合は、評価マネージャは通常どおり動作しています。

    -----------------------------------------

    empty (boolean): 照会モードのみで有効です。評価グラフにノードが含まれているかどうかを確認します。これは、現在のモードに依存しません。
    ...
    fallbackTriggered (boolean): 照会モードのみで有効です。フォールバックシリアル評価がトリガされたかどうかを確認します。並列評価中にエラーが発生して、フォールバックが強制的にシリアルモードになった場合は、trueになります。フォールバックがトリガされた後にモードが再度変更された場合は、falseにリセットされます。

    -----------------------------------------

    idleAction (int): このフラグは、アイドル時にEMが実行するアクションを指定します。使用可能な値は次のとおりです。0-アクションなし1-グラフのリビルド2-EM操作の準備3-グラフのリビルドおよびEM操作の準備この場合:グラフのリビルドを指定すると、アイドルイベント発生時に評価グラフをリビルドできるようになったら、すぐにリビルドします。EM操作の準備を指定すると、評価マネージャは、次のアイドルイベント後にEM操作を使用可能にするために必要なすべての手順を実行します。注:これらのアイドル時のアクションが適用されるのは、標準コンテキストにアタッチされたグラフのみです。その他のすべてのグラフは、それぞれ独自のルールに従って構築されます。アイドル時のアクションを有効にすることの欠点は、グラフを頻繁に変更するワークフローや非常に大きなグラフがある場合に、グラフの構築時間や操作準備時間がワークフローに影響する可能性があることです。ワークフローに影響する場合は、このフラグに0の値を渡して、アイドル時のアクションをオフにすることをお勧めします。

    -----------------------------------------

    idleBuild (boolean): このフラグは現在サポートされていません。アイドル時に評価グラフのリビルドを有効にするには、-idleActionフラグに値1を指定して使用します。
    ...
    manipulationPrevalidation (boolean): このフラグは、評価マネージャ操作の事前検証を有効にするために使用されます。事前検証では、操作内に既知のパターンがないか調べます。事前検証が正常に行われた場合は、操作の最初のフレームでダーティな伝播を使用しなくても、EM操作が安全に使用可能なことを検証できるため、高速操作をすぐに開始できます。

    -----------------------------------------

    manipulationReady (boolean): 照会モードのみで有効です。評価マネージャ操作が実行可能状態または許可状態のどちらであるかを確認します。これは、現在のモードに依存しません。

    -----------------------------------------

    mode (string): 評価マネージャの現在の評価モードを変更します。サポートされる値は、「off」、「serial」、「parallel」です。
...
def filterCurve(cutoffFrequency: float = 1.0,endTime: float = 1.0,filter: str = "",keepKeysOnFrame: bool = False,kernel: str = "",keySync: bool = False,maxTimeStep: float = 1.0,minTimeStep: float = 1.0,period: float = 1.0,precision: float = 1.0,precisionMode: int = 1,preserveKeyTangent: str = "",sampleCount: int = 1,samplingRate: float = 1.0,selectedKeys: bool = False,startTime: float = 1.0,timeTolerance: float = 1.0,tolerance: float = 1.0,useQuaternion: bool = False,width: float = 1.0) -> None:
    """
    filterCurve コマンドはアニム カーブのリストを取得し、指定したフィルタを使用してこれらをフィルタします。次のフィルタがサポートされています。
    ...
    cutoffFrequency (float): Butterworthフィルタのカットオフ周波数(Hz)を定義します。
    ...
    filter (string): 使用するフィルタタイプを指定します。使用可能なフィルタはbutterworth、euler(既定)、gaussian、keyReducer、peakRemover、keySync、resample、simplifyです。

    -----------------------------------------

    keepKeysOnFrame (boolean): 指定した場合、フレーム全体にキーを配置するためのセカンダリフィルタパスが適用されます。このフラグがサポートされるのは、Butterworthフィルタのみです。
    ...
    keySync (boolean): 指定した場合、セカンダリフィルタパスが適用され、検出されたキーごとに、兄弟カーブ(X,Y,Z)にキーが追加されます。このフラグがサポートされるのは、KeyReducerフィルタのみです。
    ...
    precision (float): 精度パラメータを定義します。KeyReducerフィルタの場合、このパラメータはソースカーブと出力カーブの間のエラー制限を指定します。この値を大きくすると、精度は低下します。この値を低くすると、精度は向上します。

    -----------------------------------------

    precisionMode (int): 精度値の処理方法を定義します(0:絶対値。1:パーセンテージ)。

    -----------------------------------------

    preserveKeyTangent (string): 指定した場合、イン接線タイプまたはアウト接線タイプが指定したタイプと一致するキーが保持されます。サポート対象の接線タイプは、fixed、linear、flat、smooth、step、clamped、plateau、stepnext、autoです。このフラグがサポートされるのは、KeyReducerフィルタのみです。

    -----------------------------------------

    sampleCount (int): ガウスフィルタカーネルの寸法を定義します。

    -----------------------------------------

    samplingRate (float): キーがButterworthフィルタに追加されるレートをフレーム/秒(FPS)で定義します。

    -----------------------------------------

    selectedKeys (boolean): 指定した場合、フィルタは選択したキーにのみ適用されます。このフラグは、startTime/endTimeの指定よりも優先します。
    ...
    useQuaternion (boolean): オイラーでなく、クォータニオンを使用する場合に有効にします。

    -----------------------------------------

    width (time): ガウスフィルタの幅を定義します。
...
def flow(divisions: Tuple[int, int, int] = tuple(1, 1, 1),localCompute: bool = False,localDivisions: Tuple[int, int, int] = tuple(1, 1, 1),objectCentered: bool = False) -> None:
    ...
    divisions ([uint, uint, uint]): このフラグはx、y、zにおけるラティススライスの数を指定します。既定値は252です。照会すると、uint32_tuint32_tuint32_tを返します。
    ...
    localDivisions ([uint, uint, uint]): このフラグは影響範囲を指定します。既定値は222です。照会すると、uint32_tuint32_tuint32_tを返します。
...
def freeze(allNodes: bool = False,displayLayers: bool = False,downstream: bool = False,forceDownstream: bool = False,frozen: bool = False,invisible: bool = False,noFreeze: bool = False,unfreeze: bool = False,upstream: bool = False) -> None:
    """
    ...
    allNodes (boolean): 操作に使用するノードを選択します。設定されていない場合は、セレクションリストが使用されます。何も選択されていない場合は、シーン内のすべてのノードが使用されます。このフラグは、照会モードで使用してシーン内でフリーズされているノードを検索できます。作成モードでフィルタ(displayLayers、非表示、またはフリーズ)とともに使用して、シーン内のノードの特定のサブセットをターゲットにすることもできます。

    -----------------------------------------

    displayLayers (boolean): このフラグを有効にすると、リスト内の処理対象の表示レイヤがウォークされます。ディスプレイレイヤの可視性コントロールがオフで、接続されていない場合は、制御対象のすべてのノードが処理リストに追加されます。注:さらに詳しく説明すると、アニメートされた可視状態または有効状態は一致として扱われます。スタティックアニメーションは確認されません。

    -----------------------------------------

    downstream (boolean): このフラグを有効にすると、選択したノードから下流へのフリーズ状態の伝播が試みられます。たとえば、スケルトンリグによって制御されているメッシュシェイプの変形などです。

    -----------------------------------------

    forceDownstream (boolean): このフラグを有効にすると、選択したノードから下流へのフリーズ状態の伝播が試みられます。たとえば、スケルトンリグによって制御されているメッシュシェイプの変形などです。下流のフラグとは異なり、このフラグを使用すると、他のフリーズ解除された入力がある場合にも、下流のノードがフリーズされます。

    -----------------------------------------

    frozen (boolean): このフラグを有効にすると、処理されるノードはフィルタリングで除外され、frozenアトリビュートが既に設定されているノードのみが含まれます。それ以外の場合は、フリーズ状態が伝搬される前に処理されるすべてのノードにfrozenアトリビュートが設定されます。このフラグを使用するのは、ノードが既にフリーズされ、コマンドを使用してグラフ経由でフリーズ状態を伝播するときのみです。

    -----------------------------------------

    invisible (boolean): このフラグを有効にすると、リスト内で処理する非表示ノードがウォークされます。visibilityアトリビュートが接続されていない場合は、DAG階層でその下位のすべてのノードが処理リストに追加されます。注:さらに詳しく説明すると、アニメートされた可視性状態は一致として扱われます。スタティックアニメーションは確認されません。

    -----------------------------------------

    noFreeze (boolean): このフラグは、実際にフリーズせずにフリーズされるノードをプレビューします。

    -----------------------------------------

    unfreeze (boolean): このフラグを有効にすると、処理されるノードのフリーズ状態はオンではなくオフになります。変更するノードを決定する場合は、同じフィルタ処理と伝搬処理がすべて実行されます。照会モードでは、このフラグは既にフリーズされたノードを返す代わりにフリーズ解除されたノードを返します。

    -----------------------------------------

    upstream (boolean): このフラグを有効にすると、選択したノードから上流へのフリーズ状態の伝播が試みられます。たとえば、フリーズされたトランスフォームに流入するパラメータカーブなどがあります。この伝播には、フリーズされたノードが引き続き使用している上流ノードがフリーズ解除された状態で維持されるように、ヒューリスティックが適用されます。

    -----------------------------------------

    Return Value:
    None: string[]照会モードで、現在フリーズまたはフリーズ解除されているノードのリストです。リストは、フリーズ状態が設定(設定解除)されたノード、FrozenAffected 状態が設定(設定解除)されたノード、および選択された残りのノードの 3 つの部分が空の文字列で区切られています。string[]フリーズ状態がコマンドによって設定されたノードのリストです。リストは、フリーズ状態が設定されたノードおよび FrozenAffected 状態が設定されたノードの 2 つの部分が空の文字列で区切られています。照会モードでは、戻り値のタイプは照会されたフラグに基づきます。
    """
    pass
...
def ghosting(action: str = "",allGhostedObjects: bool = False,allInRange: bool = False,customFrames: int = 1,enable: bool = False,farOpacity: float = 1.0,frames: bool = False,geometryFilter: bool = False,ghostedObjects: bool = False,ghostsStep: int = 1,hierarchy: bool = False,jointFilter: bool = False,locatorFilter: bool = False,mode: str = "",nearOpacity: float = 1.0,postColor: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),postFrames: int = 1,preColor: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),preFrames: int = 1,preset: str = "",resetAll: bool = False,useDriver: bool = False) -> None:
    """
    すべてのノードベースのゴースト化パラメータ、およびこのコマンドがゴースト化アクションに使用するグローバル プリファレンスへの集約されたインタフェースを提供します。ゴースト図面が現在有効かどうかを確認するには、「enable」フラグを照会します。 「action」を設定しないで作成モードで実行した場合は、ゴースト化パラメータの現在の値を変更します。「action=ghost」が設定されている場合は、ゴースト化パラメータの現在の値を変更して、選択したオブジェクトに適用します。
    ...
    action (string): コマンドで実行されるアクションを定義します。有効な値は「ghost」、「unghost」、および「unghostAll」です。「ghost」は選択リスト内のすべての_visible_DAGオブジェクトに対してゴースト化を有効にしようとします。中間トランスフォームノードは、軸表示がアクティブな場合のみゴースト化されます。

    -----------------------------------------

    allGhostedObjects (boolean): 編集モードでのみ機能し、選択したオブジェクトだけでなく、すべてのゴースト化オブジェクトに編集が適用されるように指定します。

    -----------------------------------------

    allInRange (boolean): 作成モードで、キーフレームモードで再生範囲のすべてのキーフレームを使用するのか、それとも指定した値を使用するのかを示す既定値を定義します。編集モードで、すべてのゴーストの全範囲内値を変更します。「ghost」アクションが設定された作成モードで、選択したオブジェクトのカスタムゴーストの全範囲内値も設定します。

    -----------------------------------------

    customFrames (int): 作成モードで、カスタムゴーストフレームのリストの既定値を定義します。編集モードで、すべてのゴーストのカスタムゴーストフレームを変更します。特殊なフレーム番号「-9999999」はすべてのカスタムフレームを削除し、コマンドエンジンで空のリストを渡せなくなる問題を回避します。「ghost」アクションが設定された作成モードで、選択したオブジェクトのカスタムゴーストフレームも設定します。

    -----------------------------------------

    enable (boolean): シーン全体でゴーストの可視性を有効または無効にします。これはノードのゴースト化アトリビュートを変更しません。ノードで定義されたゴーストの図面をグローバルに有効または無効にするだけです。これはプリファレンスベースのフラグであるため、異なるゴーストアトリビュート設定で新しいファイルをロードしても、セッション間でその値は保持されます。

    -----------------------------------------

    farOpacity (float): 作成モードで、現在のタイムから最も遠いゴーストの不透明度値の既定値を定義します。編集モードで、すべてのゴーストについて、現在のタイムから最も遠いゴーストの不透明度値を変更します。「ghost」アクションが設定された作成モードで、選択したオブジェクトに対して、現在のタイムから最も遠いゴーストの不透明度値も設定します。

    -----------------------------------------

    frames (boolean): ゴースト化モード、オブジェクトに設定されたパラメータ、現在のタイム(関連する場合)に基づいて、選択したオブジェクトのゴーストフレームの現在のセットを照会します。ゴースト化対応フラグの状態を無視します。

    -----------------------------------------

    geometryFilter (boolean): 作成モードで、既定のゴーストジオメトリフィルタを有効または無効にします。「ghost」アクションが設定された作成モードで、このフラグがfalseの場合にジオメトリノードを省略するように選択のフィルタも行います。

    -----------------------------------------

    ghostedObjects (boolean): 照会モードでのみ機能し、現在ゴースト化されているすべてのDAGノードの名前を検索します。

    -----------------------------------------

    ghostsStep (int): 作成モードで、ゴースト間のステップ数(キーフレームまたはフレームの数)の既定値を定義します。編集モードで、ゴースト間のステップ数(キーフレームまたはフレームの数)の既定値を変更します。「ghost」アクションが設定された作成モードで、選択したオブジェクトのゴースト間の既定のステップ数(キーフレームまたはフレームの数)も設定します。

    -----------------------------------------

    hierarchy (boolean): ゴースト階層の既定値を有効または無効にします。ゴースト化アクションが設定されていない場合、ノードのゴースト化アトリビュートは変更されません。以降のコマンドが影響を受けるノードのリストにフィルタを適用する方法についての基本設定のみを行います。ゴースト化アクションと組み合わせて使用する場合は、まずプリファレンス値を設定してから、その新しい値をゴースト化アクションのフィルタとして使用します。このフラグを指定しないでゴースト化アクションを指定すると、プリファレンスの現在の値がその場所で使用されます。これはプリファレンスベースのフラグであるため、異なるゴーストアトリビュート設定で新しいファイルをロードしても、セッション間でその値は保持されます。

    -----------------------------------------

    jointFilter (boolean): 作成モードで、既定のゴーストジョイントフィルタを有効または無効にします。「ghost」アクションが設定された作成モードで、このフラグがfalseの場合にジョイントノードを省略するように選択のフィルタも行います。

    -----------------------------------------

    locatorFilter (boolean): 作成モードで、既定のゴーストロケータフィルタを有効または無効にします。「ghost」アクションが設定された作成モードで、このフラグがfalseの場合にロケータノードを省略するように選択のフィルタも行います。

    -----------------------------------------

    mode (string): ゴースト化アクションの既定のモードを定義します。適切な値は、「preAndPost」、「pre」、「post」、「custom」、「keyframes」です。

    -----------------------------------------

    nearOpacity (float): 作成モードで、現在のタイムに最も近いゴーストの不透明度値の既定値を定義します。編集モードで、すべてのゴーストについて、現在のタイムに最も近いゴーストの不透明度値を変更します。「ghost」アクションが設定された作成モードで、選択したオブジェクトに対して、現在のタイムに最も近いゴーストの不透明度値も設定します。

    -----------------------------------------

    postColor ([float, float, float]): 作成モードで、現在のタイムの後のゴーストのカラーの既定値を定義します。編集モードで、すべてのゴーストの現在のタイムの後のゴーストのカラーを変更します。「ghost」アクションが設定された作成モードで、選択したオブジェクトに対して、現在のタイムの後のゴーストのカラーも設定します。

    -----------------------------------------

    postFrames (int): 作成モードで、現在のタイムの後のゴースト化されたフレーム数の既定値を定義します。編集モードで、すべてのゴーストの現在のタイムの後のゴースト化されたフレーム数を変更します。「ghost」アクションが設定された作成モードで、選択したオブジェクトに対して、現在のタイムの後の既定のゴースト化されたフレーム数も設定します。

    -----------------------------------------

    preColor ([float, float, float]): 作成モードで、現在のタイムの前のゴーストのカラーの既定値を定義します。編集モードで、すべてのゴーストの現在のタイムの前のゴーストのカラーを変更します。「ghost」アクションが設定された作成モードで、選択したオブジェクトに対して、現在のタイムの前のゴーストのカラーも設定します。

    -----------------------------------------

    preFrames (int): 作成モードで、現在のタイムの前のゴースト化されたフレーム数の既定値を定義します。編集モードで、すべてのゴーストの現在のタイムの前のゴースト化されたフレーム数を変更します。「ghost」アクションが設定された作成モードで、選択したオブジェクトに対して、現在のタイムの前の既定のゴースト化されたフレーム数も設定します。

    -----------------------------------------

    preset (string): ゴースト化プリセットの既定のモードを定義します。有効な値は、「1s」、「2s」、「4s」、「5s」、「10s」、「Custom」です。「Custom」以外を設定した場合は、プリセットのステップ値を使用して、3つ前のフレーム、3つ後のフレームのゴーストが修正されます(たとえば、「2s」を設定すると、2番目のフレームまたはキーフレームごとにゴーストが表示されます)。

    -----------------------------------------

    resetAll (boolean): すべてのゴースト化オプションを既定値にリセットします。使用の際は注意してください。

    -----------------------------------------

    useDriver (boolean): 作成モードで、既定のゴーストのドライバ使用値を有効または無効にします。編集モードで、既存のすべてのゴーストのドライバ使用値を変更します。「ghost」アクションが設定された作成モードで、選択したオブジェクトのドライバ使用値も設定します。

    -----------------------------------------

    Return Value:
    None: booleanゴーストの可視性の以前のグローバル状態(「enable」フラグの設定後)booleanゴーストの可視性のグローバル状態(「enabled」フラグを照会)boolean既定のゴーストの全範囲内値のグローバル状態(「allInRange」フラグを照会)boolean既定のゴースト階層のグローバル状態(「hierarchy」フラグを照会)boolean既定のゴースト ジオメトリ フィルタのグローバル状態(「geometryFilter」フラグを照会)。boolean既定のゴースト ジョイント フィルタのグローバル状態(「jointFilter」フラグを照会)。boolean既定のゴースト ロケータ フィルタのグローバル状態(「locatorFilter」フラグを照会)boolean既定のポスト フレーム数のグローバル状態(「postFrames」フラグを照会)。boolean既定のプリ フレーム数のグローバル状態(「preFrames」フラグを照会)。boolean既定のゴースト フレーム ステップ数のグローバル状態(「ghostsStep」フラグを照会)。string[]ゴースト化が有効になっているすべてのオブジェクトのリスト(作成モードで「action="ghost"」)string[]ゴースト化が無効になっているすべてのオブジェクトのリスト(作成モードで「action="unghost"」または「action="unghostAll"」)string[]ゴースト化が現在有効になっているすべてのオブジェクトのリスト(「ghostedObjects」フラグを照会)。string[]影響を受けるオブジェクトのリスト(編集モードで任意のフラグを指定)float[]選択したすべてのオブジェクトのゴースト化されたフレームの論理和である(frame1、frame2、...)のリスト(「frames」フラグを照会)float現在のタイムから最も遠いゴーストの現在の不透明度値(「farOpacity」フラグを照会)float現在のタイムに最も近いゴーストの現在の不透明度値(「nearOpacity」フラグを照会)float現在のタイムから最も遠いゴーストの以前の不透明度値(「farOpacity」フラグを設定)float現在のタイムに最も近いゴーストの以前の不透明度値(「nearOpacity」フラグを設定)string現在のゴースト化モード(「mode」フラグを照会)string以前のゴースト化モード(「mode」フラグを設定)float[]現在のタイムの後のゴーストに使用される[赤、緑、青]のカラー(「postColor」フラグを照会)float[]現在のタイムの後のゴーストに使用される[赤、緑、青]の前のカラー(「postColor」フラグを設定)float[]現在のタイムの前のゴーストに使用される[赤、緑、青]のカラー(「preColor」フラグを照会)float[]現在のタイムの前のゴーストに使用される[赤、緑、青]の前のカラー(「preColor」フラグを設定)int[]「frames」モードのカスタム フレーム リスト(「customFrames」フラグを照会)int[]前のカスタム フレーム リスト(「customFrames」フラグを設定)照会モードでは、戻り値のタイプは照会されたフラグに基づきます。
    """
    pass
...
def keyingGroup(activator: str = "",addElement: str = "",afterFilters: bool = False,anyMember: str = "",category: str = "",clear: str = "",color: int = 1,copy: str = "",edges: bool = False,editPoints: bool = False,empty: bool = False,excludeDynamic: bool = False,excludeRotate: bool = False,excludeScale: bool = False,excludeTranslate: bool = False,excludeVisibility: bool = False,facets: bool = False,flatten: str = "",forceElement: str = "",include: str = "",intersection: str = "",isIntersecting: str = "",isMember: str = "",layer: bool = False,minimizeRotation: bool = False,name: str = "",noIntermediate: bool = False,noSurfaceShader: bool = False,noWarnings: bool = False,nodesOnly: bool = False,remove: str = "",removeActivator: str = "",renderable: bool = False,setActiveFilter: str = "",size: bool = False,split: str = "",subtract: str = "",text: str = "",union: str = "",vertices: bool = False) -> None:
    """
    ...
    noIntermediate (boolean): セットメンバーを照会するときや、subtract、union、intersection、isIntersectingフラグを使用するときに、中間オブジェクトを除外します。
...
def keyTangent(absolute: bool = False,animation: str = "",attribute: str = "",controlPoints: bool = False,float: Tuple[float, float] = tuple(1.0, 1.0),g: bool = False,hierarchy: str = "",inAngle: float = 1.0,inTangentType: str = "",inWeight: float = 1.0,includeUpperBound: bool = False,index: int = 1,ix: float = 1.0,iy: float = 1.0,lock: bool = False,outAngle: float = 1.0,outTangentType: str = "",outWeight: float = 1.0,ox: float = 1.0,oy: float = 1.0,pluginTangentTypes: str = "",relative: bool = False,shape: bool = False,stepAttributes: bool = False,time: Tuple[float, float] = tuple(1.0, 1.0),unify: bool = False,weightLock: bool = False,weightedTangents: bool = False) -> None:
    ...
    inTangentType (string): イン接線タイプを指定します。有効な値は、「spline」、「linear」、「fast」、「slow」、「flat」、「step」、「stepnext」、「fixed」、「clamped」、「plateau」、「auto」、「autoease」、「automix」、「autocustom」です。照会するとstring[]を返します。
    ...
    outTangentType (string): アウト接線タイプを指定します。有効な値は、「spline」、「linear」、「fast」、「slow」、「flat」、「step」、「stepnext」、「fixed」、「clamped」、「plateau」、「auto」、「autoease」、「automix」、「autocustom」です。照会するとstring[]を返します。
...
def mimicManipulation(manipulations: str = "",prevalidation: bool = False,refresh: bool = False) -> None:
    """
    このコマンドは、評価マネージャの高速操作をサポートするためにさまざまなマニピュレータで行われる動作を模倣します。このコマンドは、テスト、デバッグ、およびベンチマークの目的に使用する必要があります。操作は、JSON オブジェクトを表す文字列を使用して記述されます。このオブジェクトには、配列を含む「session」という名前のメンバーが必要です。この配列の各メンバーは、操作トランザクション(1 つの操作アクションで設定されるプラグ)を表します。これらの各トランザクションは、設定するプラグの配列でもあります。設定するプラグは、操作するプラグを記述する文字列「plug」、およびこのプラグに設定する値「value」をメンバーとして持つオブジェクトです。現在設定できるのは、アトリビュートのタイプが「double」または「double3」のプラグのみです。値には、数値または 3 つの数値からなる配列を指定する必要があります。セッションは、マニピュレータを保持してから放すまでの(その間の移動も含む)操作を表すグローバル アクションとみなすことができます。トランザクションは操作内の 1 つのデルタであるとみなされ、結果を表示するにはこの後に評価を行う必要があります。たとえば、マニピレータを保持したままマウス移動を 1 回行ったら、評価を行ってビューポートをリフレッシュする必要があります。
    ...
    manipulations (string): 実行する操作を表すJSON文字列。

    -----------------------------------------

    prevalidation (boolean): 操作したプラグの事前検証を実行するかどうかをコントロールするフラグ。このフラグが設定されていて、プラグが評価マネージャですでに適切にサポートされている場合は、最初のフレームで評価マネージャの操作が使用されます。初期フレームでダーティな伝播を行うことも、評価マネージャの操作を安全に実行できるか検証するためのDG評価を行うことも必要ありません。

    -----------------------------------------

    refresh (boolean): 各操作の後にリフレッシュがトリガされるかどうかをコントロールするフラグ。評価を開始するには、リフレッシュする必要があることに注意してください。

    -----------------------------------------

    Return Value:
    None: boolean[]指定したトランザクションごとに、評価マネージャでトランザクションを評価できる場合は、true、それ以外の場合は false を返します
    """
    pass
...
def playbackOptions(animationEndTime: float = 1.0,animationStartTime: float = 1.0,blockingAnim: bool = False,by: float = 1.0,framesPerSecond: bool = False,loop: str = "",maxPlaybackSpeed: float = 1.0,maxTime: float = 1.0,minTime: float = 1.0,playbackSpeed: float = 1.0,stepLoop: bool = False,view: str = "") -> None:
    """
    ...
    stepLoop (boolean): 1フレーム進む/戻る場合、および1キー進む/戻る場合に、再生範囲の最初または最後へのループバックのオン/オフを切り替えます。
...
def playblast(activeEditor: bool = False,cameraSetup: Tuple[str, str] = tuple("", ""),clearCache: bool = False,codecOptions: bool = False,combineSound: bool = False,completeFilename: str = "",compression: str = "",editorPanelName: str = "",endTime: float = 1.0,filename: str = "",forceOverwrite: bool = False,format: str = "",frame: float = 1.0,framePadding: int = 1,height: int = 1,indexFromZero: bool = False,offScreen: bool = False,offScreenViewportUpdate: bool = False,options: bool = False,percent: int = 1,quality: int = 1,rawFrameNumbers: bool = False,replaceAudioOnly: bool = False,replaceEndTime: float = 1.0,replaceFilename: str = "",replaceStartTime: float = 1.0,sequenceTime: bool = False,showOrnaments: bool = False,sound: str = "",startTime: float = 1.0,throwOnError: bool = False,useTraxSounds: bool = False,viewer: bool = False,width: int = 1,widthHeight: Tuple[int, int] = tuple(1, 1)) -> None:
    """
    ...
    offScreenViewportUpdate (boolean): 設定されている場合、この切り替えにより、プレイブラストはオフスクリーンバッファを使用してレンダリングしながら、ビューポートを更新できるようになります。
...
def pose(allPoses: bool = False,apply: bool = False,name: str = "") -> None:
    """
    ...
    allPoses (boolean): このフラグを使用して、シーン内の全ポーズを照会します。

    -----------------------------------------

    apply (boolean): このフラグはnameフラグと一緒に使用して、キャラクタに適用するポーズを指定します。

    -----------------------------------------

    name (string): 作成モードでは、ポーズ名を指定します。照会モードでは、キャラクタの全ポーズのリストを返します。適用モードでは、適用するポーズを指定します。
...
def scaleKey(animation: str = "",attribute: str = "",autoSnap: bool = False,controlPoints: bool = False,float: Tuple[float, float] = tuple(1.0, 1.0),floatPivot: float = 1.0,floatScale: float = 1.0,hierarchy: str = "",includeUpperBound: bool = False,index: int = 1,newEndFloat: float = 1.0,newEndTime: float = 1.0,newStartFloat: float = 1.0,newStartTime: float = 1.0,scaleSpecifiedKeys: bool = False,shape: bool = False,time: Tuple[float, float] = tuple(1.0, 1.0),timePivot: float = 1.0,timeScale: float = 1.0,valuePivot: float = 1.0,valueScale: float = 1.0) -> None:
    ...
    autoSnap (boolean): trueの場合は自動スナップのスケールされたキーになります。既定値はscaleKeyAutoSnapオプションに依存します。
...
def sequenceManager(addSequencerAudio: str = "",attachSequencerAudio: str = "",currentShot: str = "",currentTime: float = 1.0,listSequencerAudio: str = "",listShots: bool = False,modelPanel: str = "",node: str = "",writableSequencer: str = "") -> None:
    """
    ...
    addSequencerAudio (string): ファイル名を指定してオーディオクリップをシーケンサに追加します。

    -----------------------------------------

    attachSequencerAudio (string): オーディオノードを指定してオーディオクリップをシーケンサに追加します。

    -----------------------------------------

    currentShot (string): 現在のシーケンス時間で使用されているショットを返します。

    -----------------------------------------

    currentTime (time): 現在のシーケンス時間を設定します。

    -----------------------------------------

    listSequencerAudio (string): シーケンサに追加されたオーディオクリップをリスト表示します。

    -----------------------------------------

    listShots (boolean): すべてのシーンセグメントに渡って現在定義されているショットをすべてリスト表示します。

    -----------------------------------------

    modelPanel (string): シーケンスをコントロールするパネルとして使用される専用のmodelPanelを設定します。

    -----------------------------------------

    node (string): 1つしかないSequenceManagerノードを返します。

    -----------------------------------------

    writableSequencer (string): 書き込み可能なシーケンサノードを取得します。存在しない場合は作成します。
...
def setDrivenKeyframe(attribute: str = "",controlPoints: bool = False,count: bool = False,currentDriver: str = "",driven: bool = False,driver: bool = False,driverValue: float = 1.0,hierarchy: str = "",inTangentType: str = "",insert: bool = False,insertBlend: bool = False,outTangentType: str = "",shape: bool = False,value: float = 1.0) -> None:
    """
    ...
    count (boolean): 選択した項目のドリブン/ドライバアトリビュートの名前ではなく、その数を返します。
...
def setKeyframe(adjustTangent: bool = False,animLayer: str = "",animated: bool = False,attribute: str = "",breakdown: bool = False,clip: str = "",controlPoints: bool = False,dirtyDG: bool = False,float: float = 1.0,hierarchy: str = "",identity: bool = False,inTangentType: str = "",insert: bool = False,insertBlend: bool = False,minimizeRotation: bool = False,noResolve: bool = False,outTangentType: str = "",preserveCurveShape: bool = False,respectKeyable: bool = False,shape: bool = False,time: float = 1.0,useCurrentLockedWeights: bool = False,value: float = 1.0) -> None:
    """
    ...
    adjustTangent (boolean): 挿入キーの場合は接線を調整します。
    ...
    preserveCurveShape (boolean): キーを挿入したときのカーブのシェイプの保持を設定します。既定値はkeySetPreserveCurveShapeオプションに依存します。
...
def shot(audio: str = "",clip: str = "",clipDuration: float = 1.0,clipOpacity: float = 1.0,clipSyncState: bool = False,clipZeroOffset: float = 1.0,copy: bool = False,createCustomAnim: bool = False,currentCamera: str = "",customAnim: bool = False,deleteCustomAnim: bool = False,determineTrack: bool = False,endTime: float = 1.0,favorite: bool = False,flag1: bool = False,flag10: bool = False,flag11: bool = False,flag12: bool = False,flag2: bool = False,flag3: bool = False,flag4: bool = False,flag5: bool = False,flag6: bool = False,flag7: bool = False,flag8: bool = False,flag9: bool = False,hasCameraSet: bool = False,hasStereoCamera: bool = False,imagePlaneVisibility: bool = False,linkAudio: str = "",lock: bool = False,mute: bool = False,paste: bool = False,pasteInstance: bool = False,postHoldTime: float = 1.0,preHoldTime: float = 1.0,scale: float = 1.0,selfmute: bool = False,sequenceDuration: float = 1.0,sequenceEndTime: float = 1.0,sequenceStartTime: float = 1.0,shotName: str = "",sourceDuration: float = 1.0,startTime: float = 1.0,track: int = 1,transitionInLength: float = 1.0,transitionInType: int = 1,transitionOutLength: float = 1.0,transitionOutType: int = 1,unlinkAudio: bool = False) -> None:
    """
    ...
    audio (string): ショットのオーディオクリップを指定します。オーディオはショットにリンクされ、ショットがシーケンサ(Sequencer)に表示されている間、指定のサウンドを再生することができます。ショットやシーケンサ(Sequencer)のオーディオの使用方法の詳細については、ショットノードに関するマニュアルを参照してください。

    -----------------------------------------

    clip (string): このショットに使用されたクリップを示します。このクリップはcurrentCameraのimagePlaneに配置されます。ショットやシーケンサ(Sequencer)のカメラの使用方法の詳細については、ショットノードに関するマニュアルを参照してください。

    -----------------------------------------

    clipDuration (time): クリップの長さを示します。シーケンサ(Sequencer)のクリップインディケータバーの表示に使用されます。

    -----------------------------------------

    clipOpacity (float): ショットのクリップの不透明度を示します。この値はcurrentCameraのimagePlaneに割り当てられます。ショットやシーケンサ(Sequencer)のカメラの使用方法の詳細については、ショットノードに関するマニュアルを参照してください。

    -----------------------------------------

    clipSyncState (boolean): このショットに使用されたクリップのビューポートとの同期状態を示します。戻り値は0=このショットに使用されているクリップはない1=クリップはビューポートと完全に同期され、フレームはシーケンサと1:12=クリップはビューポートと部分的に同期され、ムービーはシーケンサに合わせてスケール可能3=クリップはビューポートと同期されていない(スケール/時間/カメラの相違が考えられる)

    -----------------------------------------

    clipZeroOffset (time): ショットの冒頭に対応させるクリップの時間を指定します。これは、分割したクリップを正確に位置合わせするために使用されます。

    -----------------------------------------

    copy (boolean): このフラグを使用して、1つまたは複数のクリップをクリップボードにコピーします。照会モードでこのフラグを使用すると、クリップボードにコピーされた内容を照会することができます。

    -----------------------------------------

    createCustomAnim (boolean): アニメーションレイヤを作成し、ショットノードのcustomAnimアトリビュートをアニメーションレイヤのweightアトリビュートにリンクします。

    -----------------------------------------

    currentCamera (string): このショットに使用されたカメラを示します。ショットやシーケンサ(Sequencer)のカメラの使用方法の詳細については、ショットノードに関するマニュアルを参照してください。

    -----------------------------------------

    customAnim (boolean): このショットノードのcustomAnimアトリビュートにリンクされたアニメーションレイヤノードの名前を返します。

    -----------------------------------------

    deleteCustomAnim (boolean): このショットのcustomAnimアトリビュートからアニメーションレイヤを接続解除し、アニメーションレイヤノードを削除します。

    -----------------------------------------

    determineTrack (boolean): ショットに使用できるトラックを定義します。新しいトラック番号、または、現在のトラックを使用できる場合は既存のトラック番号を返します。

    -----------------------------------------

    endTime (time): Mayaタイムスライダでのショットの終了時間です。startTimeを変更すると、ショットの継続時間が延長されます。

    -----------------------------------------

    favorite (boolean): ショットをお気に入りに指定します。シーケンサ(Sequencer)のパネルでのナビゲーションを効率化するためのUIインディケータです。

    -----------------------------------------

    flag1 (boolean): このショットについてユーザが指定した状態フラグ1/12

    -----------------------------------------

    flag10 (boolean): このショットについてユーザが指定した状態フラグ10/12

    -----------------------------------------

    flag11 (boolean): このショットについてユーザが指定した状態フラグ11/12

    -----------------------------------------

    flag12 (boolean): このショットについてユーザが指定した状態フラグ12/12

    -----------------------------------------

    flag2 (boolean): このショットについてユーザが指定した状態フラグ2/12

    -----------------------------------------

    flag3 (boolean): このショットについてユーザが指定した状態フラグ3/12

    -----------------------------------------

    flag4 (boolean): このショットについてユーザが指定した状態フラグ4/12

    -----------------------------------------

    flag5 (boolean): このショットについてユーザが指定した状態フラグ5/12

    -----------------------------------------

    flag6 (boolean): このショットについてユーザが指定した状態フラグ6/12

    -----------------------------------------

    flag7 (boolean): このショットについてユーザが指定した状態フラグ7/12

    -----------------------------------------

    flag8 (boolean): このショットについてユーザが指定した状態フラグ8/12

    -----------------------------------------

    flag9 (boolean): このショットについてユーザが指定した状態フラグ9/12

    -----------------------------------------

    hasCameraSet (boolean): このショットに関連付けられたカメラがカメラセットの場合、trueを返します。

    -----------------------------------------

    hasStereoCamera (boolean): このショットに関連付けられたカメラが立体視カメラの場合、trueを返します。

    -----------------------------------------

    imagePlaneVisibility (boolean): ショットイメージプレーンの可視性です。この値はurrentCameraのimagePlaneに割り当てられます。

    -----------------------------------------

    linkAudio (string): このショットにリンクするオーディオクリップを指定します。現在リンクされているオーディオのリンクが解除されることはありません。

    -----------------------------------------

    lock (boolean): 指定のショットをロックします。shotTrackコマンドを使用して実行する、トラック全体のロックとは異なります。

    -----------------------------------------

    mute (boolean): 指定のショットをミュートします。shotTrackコマンドを使用して実行する、トラック全体のミュートとは異なります。このアトリビュートを照会したときに、それ自身がミュートされている、ショットがミュートされている、ショットがソロ化解除されている場合は、trueを返します。ショット自身の状態のみを照会する場合は、「selfmute」フラグを参照してください。

    -----------------------------------------

    paste (boolean): このフラグを使用して、クリップボードのショットをシーケンスタイムスライダにペーストします。ショットは、c/copyフラグを使用してクリップボードに追加します。

    -----------------------------------------

    pasteInstance (boolean): このフラグを使用して、クリップボードのショットのインスタンスをシーケンスタイムスライダにペーストします。オリジナルのソースショットからカメラやイメージプレーンを複製するp/pasteフラグと異なり、pi/pasteInstanceフラグはソースショットのカメラやイメージプレーンを共有します。オーディオノードは複製されます。

    -----------------------------------------

    postHoldTime (time): シーケンスタイムスライダにおいて、ショットの後ろに付加する時間の長さを指定します。シーケンス時間内のショットの最終フレームごとに指定の長さを付加します。

    -----------------------------------------

    preHoldTime (time): シーケンスタイムスライダにおいて、ショットの前に付加する時間の長さを指定します。シーケンス時間内のショットの先頭フレームごとに指定の長さを付加します。

    -----------------------------------------

    scale (float): ショットのMayaフレームレンジをスケールする度合を指定します。これによってsequenceEndFrameは影響を受けますが、sequenceStartFrameはそのまま変更されません。

    -----------------------------------------

    selfmute (boolean): この個別のショット自身のミュート状態が設定されている場合は、照会します。これは「mute」フラグとは異なります。「mute」フラグは、トラックがミュートされているために、または別のトラックがソロ化されているためにこのショットがミュートされている場合は、trueを返します。このフラグを編集すると、このショット自身のミュート状態が設定されます(「mute」フラグと同様の動作)。

    -----------------------------------------

    sequenceDuration (time): ショットのシーケンス継続時間を返し、付加時間やスケールを含みます。このフラグは照会のみです。

    -----------------------------------------

    sequenceEndTime (time): シーケンスタイムスライダでのショットの終了時間です。ショットのendTimeを変更すると、シーケンス時間におけるショットの継続時間が変わります。

    -----------------------------------------

    sequenceStartTime (time): シーケンスタイムスライダでのショットの開始時間です。ショットのstartTimeを変更すると、シーケンス時間におけるショットの開始時間が変わります。

    -----------------------------------------

    shotName (string): ショットのユーザ定義名を指定します。Mayaのノード名としては無効な名前を割り当てることができます。shotNameアトリビュートが定義されると、必ずその値がUIで使用されます。

    -----------------------------------------

    sourceDuration (time): ショットにおけるソースフレーム数を返します。このフラグは照会のみです。

    -----------------------------------------

    startTime (time): Mayaタイムスライダでのショットの開始時間です。startTimeを変更すると、ショットの継続時間が延長されます。

    -----------------------------------------

    track (int): このショットが属するトラックを指定します。

    -----------------------------------------

    transitionInLength (time): ショットへのトランジションの長さ。

    -----------------------------------------

    transitionInType (int): ショットへのトランジションについてトランジションのタイプを指定します。0=フェード1=ディゾルブ

    -----------------------------------------

    transitionOutLength (time): ショットからのトランジションの長さ。

    -----------------------------------------

    transitionOutType (int): ショットからのトランジションについてトランジションのタイプを指定します。0=フェード1=ディゾルブ

    -----------------------------------------

    unlinkAudio (boolean): コメント現在リンクしているオーディオのリンクがすべて解除されます。

    -----------------------------------------

    Return Value:
    None: stringショットの名前照会モードでは、戻り値のタイプは照会されたフラグに基づきます。
    """
    pass

    
def shotRipple(deleted: bool = False,endDelta: float = 1.0,endTime: float = 1.0,startDelta: float = 1.0,startTime: float = 1.0) -> None:
    """
    リップル編集モードを有効にすると、操作したショットに隣接するショットがシーケンス時間において移動され、そのノードの編集に応じて前に進めたりギャップを詰めたりできます。ショット編集にパラメータを指定すると、移動するその他のショットが選択されて適切なサイズで移動されます。ショット名を指定しない場合、最初に選択したショットが使用されます。
    ...
    deleted (boolean): このリップル編集によってショットを削除するかどうかを指定します。

    -----------------------------------------

    endDelta (time): フレームの終了時間の変更を指定します。

    -----------------------------------------

    endTime (time): シーケンスタイムスライダでの先頭ショットの終了時間を指定します。

    -----------------------------------------

    startDelta (time): フレームの開始時間の変更を指定します。

    -----------------------------------------

    startTime (time): シーケンスタイムスライダでの先頭ショットの開始時間を指定します。
...
def snapKey(animation: str = "",attribute: str = "",controlPoints: bool = False,float: Tuple[float, float] = tuple(1.0, 1.0),hierarchy: str = "",includeUpperBound: bool = False,index: int = 1,mergeDuplicate: bool = False,shape: bool = False,time: Tuple[float, float] = tuple(1.0, 1.0),timeMultiple: float = 1.0,valueMultiple: float = 1.0) -> None:
    """
    このコマンドは、キーセット上に作用します。キーセットは、1 つまたは複数のアニメーション カーブ上の指定したタイム レンジ内のキー グループとして定義されます。
    ...
    mergeDuplicate (boolean): このフラグが存在する場合、複製されたフレームを持つキーは1にマージされます。既定はfalseです。
...
def timeEditorClip(absolute: bool = False,addAttribute: str = "",allowShrinking: bool = False,animSource: str = "",audio: str = "",blendMode: int = 1,children: int = 1,clipAfter: bool = False,clipBefore: bool = False,clipDataType: bool = False,clipId: int = 1,clipIdFromNodeName: int = 1,clipIdFromPath: bool = False,clipNode: bool = False,clipPath: bool = False,copyClip: bool = False,crossfadeMode: int = 1,crossfadePlug: bool = False,curveTime: float = 1.0,defaultGhostRoot: bool = False,drivenAttributes: bool = False,drivenClipsBySource: str = "",drivenObjects: bool = False,drivenRootObjects: bool = False,drivingSources: str = "",duplicateClip: bool = False,duration: float = 1.0,emptySource: bool = False,endTime: float = 1.0,existingOnly: bool = False,exists: bool = False,explode: int = 1,exportAllClips: bool = False,exportFbx: str = "",extend: bool = False,extendParent: bool = False,ghost: bool = False,ghostRootAdd: str = "",ghostRootRemove: str = "",group: bool = False,holdEnd: float = 1.0,holdStart: float = 1.0,importTakeDestination: int = 1,isContainer: bool = False,listUserGhostRoot: bool = False,loopEnd: float = 1.0,loopStart: float = 1.0,minClipDuration: bool = False,modifyAnimSource: bool = False,moveClip: float = 1.0,mute: bool = False,name: str = "",parent: int = 1,parentClipId: int = 1,parentGroupId: bool = False,pasteClip: float = 1.0,path: str = "",preserveAnimationTiming: bool = False,razorClip: float = 1.0,remap: Tuple[str, str] = tuple("", ""),remapNamespace: Tuple[str, str] = tuple("", ""),remapSource: Tuple[str, str] = tuple("", ""),remappedSourceAttrs: bool = False,remappedTargetAttrs: bool = False,removeAttribute: str = "",removeClip: bool = False,removeCrossfade: bool = False,removeWeightCurve: bool = False,resetTiming: bool = False,resetTransition: bool = False,ripple: bool = False,rootClipId: int = 1,rootPath: str = "",scaleEnd: float = 1.0,scalePivot: float = 1.0,scaleStart: float = 1.0,setKeyframe: str = "",speedRamping: int = 1,startTime: float = 1.0,timeWarp: bool = False,timeWarpCurve: bool = False,timeWarpType: int = 1,track: str = "",tracksNode: bool = False,transition: bool = False,trimEnd: float = 1.0,trimStart: float = 1.0,truncated: bool = False,uniqueAnimSource: bool = False,userGhostRoot: bool = False,weightCurve: bool = False,zeroKeying: bool = False,addObjects: str = "",addRelatedKG: bool = False,addSelectedObjects: bool = False,attribute: str = "",exclusive: bool = False,importAllFbxTakes: bool = False,importFbx: str = "",importFbxTakes: str = "",importMayaFile: str = "",importOption: str = "",importPopulateOption: str = "",importedContainerNames: str = "",includeRoot: bool = False,populateImportedAnimSources: str = "",poseClip: bool = False,recursively: bool = False,removeSceneAnimation: bool = False,showAnimSourceRemapping: bool = False,takeList: str = "",takesToImport: str = "",type: str = "") -> None:
    ...
    blendMode (int): -clipIdフラグで指定したクリップのブレンドモードを設定します。0:標準-絶対ブレンド1:加算-相対ブレンド
    ...
    remapNamespace ([string, string]): ネームスペースをリマップします。作成モードの場合のみ、-importFbx/fbx、-importMayaFile/mf、または-attribute/atフラグと組み合わせて使用できます。このフラグは、指定したネームスペースをすべて、代替として指定したネームスペースに置き換えます。このフラグは2つの文字列引数を取ります。最初の引数は置き換えるネームスペースを指定します。2番目の引数は、代替ネームスペースを指定します。このフラグを-sar/showAnimSourceRemappingフラグと一緒に使用することはできません。-remapNamespaceフラグを指定してtimeEditorClipコマンドを呼び出す前に、既存のトラックを指定する必要があることに注意してくださ。
...
def timeWarp(deleteFrame: int = 1,frame: float = 1.0,g: bool = False,interpType: Tuple[int, str] = tuple(1, ""),moveFrame: Tuple[int, float] = tuple(1, 1.0)) -> None:
    """
    ...
    deleteFrame (int): フラグの値は、削除するワープフレームの0ベースのインデックスを示します。このフラグは編集モードでのみ使用できます。

    -----------------------------------------

    frame (float): 作成モードと編集モードでは、このフラグを使用してワープ操作に追加するワープフレームを指定できます。照会モードでは、このフラグはワープが実行されるフレームの値のリストを返します。moveFrameフラグコマンドを使用して、関連付けられたワープ値を照会できます。

    -----------------------------------------

    g (boolean): 作成モードでは、シーン内でアニメートされるオブジェクトすべてに影響するグローバルタイムワープノードを作成します。照会モードでは、グローバルタイムワープノードを返します。注:シーン内に存在できるグローバルタイムワープは1つだけです。

    -----------------------------------------

    interpType ([int, string]): このフラグを使用して、指定したスパンの補間タイプを設定できます。有効な補間タイプは、linear、easeIn、およびeaseOutです。照会された場合、指定したタイムワープの補間タイプの文字列配列を返します。

    -----------------------------------------

    moveFrame ([int, float]): このフラグを使用して、単独のワープフレームを移動できます。最初に指定する値は、移動するワープフレームの0ベースのインデックスを示します。2番目の値は、新しいワープフレーム値を示します。このフラグは、編集モードと照会モードでのみ使用できます。照会すると、ワープフレームの値の配列が返されます。

    -----------------------------------------

    Return Value:
    None: stringtimeWarp 名照会モードでは、戻り値のタイプは照会されたフラグに基づきます。
    """
    pass
...
def ubercam() -> None:
    """
    シーケンサのショットで使用されるすべてのカメラと同等の動作をする"ubercam" を作成します。
    ...
    Return Value:
    None: stringUbercam name
    """
    pass

    
def bakeDeformer(colorizeSkeleton: bool = False,customRangeOfMotion: Tuple[float, float] = tuple(1.0, 1.0),dstMeshName: str = "",dstSkeletonName: str = "",hierarchy: bool = False,influences: Tuple[str] = tuple(""),maxInfluences: int = 1,pruneWeights: float = 1.0,smoothWeights: int = 1,srcMeshName: str = "",srcSkeletonName: str = "") -> None:
    """
    ...
    colorizeSkeleton (boolean): 作成された新しいスキンクラスタは、そのスケルトンが色付けされます。-srcSkeletonNameおよび-dstSkeletonNameフラグと併用する必要があります。

    -----------------------------------------

    customRangeOfMotion (timerange): 使用するモーション範囲のフレームでこのフラグが指定されている場合、ツールは各フレームを独立したポーズとしてステップ実行します。それ以外の場合は、ツールで既存のモーション範囲が使用されて、各インフルエンスが45度回転します。使用例:-rom"10:20"は、10～20の範囲(現在の時間単位、両方の値を含む)のすべてのフレームを意味します。範囲の一端を省略すると、次の例のように、アニメーション範囲のいずれかの端(または両端)が使用されます。-rom"10:"は、時間10(現在の時間単位)から(タイムライン上の)アニメーション範囲内の最大時間までのすべてのフレームを意味します。-rom":10"は、(タイムライン上の)アニメーション範囲の最大時間から、時間10(現在の時間単位、両方の値を含む)までのすべてのフレームを意味します。-rom":"は、現在のアニメーション範囲の、最小時間から最大時間までのすべてのフレームを指定する短縮形です。このフラグを使用した場合、指定したモーション範囲内のジョイントの一部が十分に変更されていない可能性があります。bakeDeformerの結果を改善するため、またはアルゴリズムエラーを回避するために、このコマンドは指定したモーション範囲内で十分に移動していないインフルエンスのリストを返します。これらのジョイントを検出するために、以降のフレーム間で各ジョイントのローカルトランスフォームが比較されます。次の基準のいずれかが満たされている場合は、ジョイントが十分に変更されたとみなされます。5度以上の回転がある(回転角はトランスフォーム間の最短の回転によって決まる)。各フレームのすべてのジョイントを含む最大のバウンディングボックスのサイズが1%以上変化している。1%以上のスケーリング変更がある。このパーセンテージは、最小スケーリング値を最大スケーリング値で割った値を表します(絶対値)。いずれかの基準を満たさないジョイントがある場合は、正常に移動されなかったジョイントに関する警告にこのジョイントが追加されます。カスタムのモーション範囲は実験段階のものです。
    ...
    hierarchy (boolean): 渡されたジョイントの子のうち、インフルエンスフラグで使用されているものがすべて使用されます。

    -----------------------------------------

    influences (string[]): ジョイントのリストは、新しいウェイトを決定するためのインフルエンスとして使用されます。
    ...
    pruneWeights (float): 新規に作成されたスキンクラスタで、指定した値を下回るすべてのウェイトにゼロを設定します(ポストプロセス)。これにより、次のようにskinPercentコマンドが呼び出されます。「skinPercent-pruneWeights[value][skinClusterName][dstMeshName]」。[value]はこのフラグに渡される値、[skinClusterName]は、このツールの実行後に作成される新しいskinClusterノードの名前、[dstMeshName]は-dstMeshNameフラグで指定されたメッシュです。

    -----------------------------------------

    smoothWeights (int): スムージングウェイトのスムージングの反復回数です(ポストプロセス)。これにより、残りのウェイトも再正規化されます。
...
def cluster(after: bool = False,afterReference: bool = False,before: bool = False,bindState: bool = False,deformerTools: bool = False,envelope: float = 1.0,exclusive: str = "",frontOfChain: bool = False,geometry: str = "",geometryIndices: bool = False,ignoreSelected: bool = False,includeHiddenSelections: bool = False,name: str = "",parallel: bool = False,prune: bool = False,relative: bool = False,remove: bool = False,resetGeometry: bool = False,split: bool = False,useComponentTags: bool = False,weightedNode: Tuple[str, str] = tuple("", "")) -> None:
    """
...
def deformableShape(chain: bool = False,createOriginalGeometry: bool = False,createTweakNode: bool = False,createUpstreamTagInjectionNode: bool = False,frontOfChain: bool = False,localShapeInAttr: bool = False,localShapeOutAttr: bool = False,nodeChain: bool = False,originalGeometry: bool = False,outputPlugChain: bool = False,plugChain: bool = False,tagInjectionList: bool = False,tagInjectionNode: bool = False,tweakNode: bool = False,upstreamTagInjectionNode: bool = False,worldShapeOutAttr: bool = False) -> None:
    """
    ...
    chain (boolean): このフラグは、指定したシェイプを変形するデフォーマのリストを返します。

    -----------------------------------------

    createOriginalGeometry (boolean): このシェイプの元のジオメトリがまだ存在しない場合は、作成します。

    -----------------------------------------

    createTweakNode (boolean): これにより、従来の微調整ノードがまだ存在しない場合は、作成されます。

    -----------------------------------------

    createUpstreamTagInjectionNode (boolean): これにより、編集可能なノードがまだ存在しない場合は、上流コンポーネントタグの射出ノードが作成されます。

    -----------------------------------------

    frontOfChain (boolean): このフラグは、デフォメーションチェーンのフロントエンドにあるシェイプノードのプラグ名前を返します。何も存在しない場合は、空のプラグを返します。

    -----------------------------------------

    localShapeInAttr (boolean): ローカルシェイプの入力アトリビュートの名前を返します。

    -----------------------------------------

    localShapeOutAttr (boolean): ローカルシェイプの出力アトリビュートの名前を返します。

    -----------------------------------------

    nodeChain (boolean): このフラグは、ジオメトリがこのシェイプに到達するために通過するノードのリストを返します。

    -----------------------------------------

    originalGeometry (boolean): このフラグは、originalGeometryとして使用するのに最適なデフォメーションチェーン内のノード(通常はフロントエンドのノード)のプラグ名を返します。何も存在しない場合は、空のプラグを返します。

    -----------------------------------------

    outputPlugChain (boolean): このフラグは、シェイプに接続されている出力プラグのリストを返します。

    -----------------------------------------

    plugChain (boolean): このフラグは、シェイプ(入力と出力の両方のプラグ)に接続されているプラグのリストを返します。

    -----------------------------------------

    tagInjectionList (boolean): このフラグは、非手続き型のcomponentTag射出ノードのリストを返します。

    -----------------------------------------

    tagInjectionNode (boolean): このフラグは、デフォメーションチェーンのできるだけ上位にある、参照されていないコンポーネントタグの射出ノード名を返します。入力シェイプと同じにするか、何も存在しない場合は空の文字列にすることもできます。

    -----------------------------------------

    tweakNode (boolean): このフラグは、デフォメーションチェーン内の微調整ノードの名前を返します。何も存在しない場合は、空の文字列を返します。

    -----------------------------------------

    upstreamTagInjectionNode (boolean): このフラグは、入力シェイプの上流(ただし、最上流は含まない)にある、参照されていないコンポーネントタグの射出ノード名を返します。何も存在しない場合は、空の文字列を返します。何も存在しない場合は、cti/createUpstreamTagInjectionNodeフラグを使用して作成できます。

    -----------------------------------------

    worldShapeOutAttr (boolean): ワールドシェイプの出力アトリビュートの名前を返します。
...
def deformer(after: bool = False,afterReference: bool = False,before: bool = False,deformerTools: bool = False,exclusive: str = "",frontOfChain: bool = False,geometry: str = "",geometryIndices: bool = False,ignoreSelected: bool = False,includeHiddenSelections: bool = False,name: str = "",parallel: bool = False,prune: bool = False,remove: bool = False,split: bool = False,type: str = "",useComponentTags: bool = False) -> None:
    """
...
def deformerWeights(attribute: str = "",defaultValue: float = 1.0,deformer: str = "",export: bool = False,format: str = "",ignoreName: bool = False,im: bool = False,method: str = "",path: str = "",positionTolerance: float = 1.0,remap: str = "",shape: str = "",skip: str = "",vertexConnections: bool = False,weightPrecision: int = 1,weightTolerance: float = 1.0,worldSpace: bool = False) -> None:
    """
    XML ファイルとのデフォーマ ウェイトの読み込みと書き出しを行うコマンド。ウェイトのデータは、ジオメトリ入力からデフォーマへの頂点に該当する「ポイント クラウド」と一緒に、頂点単位で格納されます。たとえば、クラスタ デフォーマは次の情報を保有します。
    ...
    attribute (string): deformerWeightsと一緒に読み込み/書き出しする必要のあるデフォーマアトリビュートのロングネームを指定します。つまり、-at"envelope"-at"skinningMethod"のようになります。指定したアトリビュートが特定のデフォーマに存在しない場合は、警告またはエラーが表示されません。そのため、複数のデフォーマでこのコマンドを使用しても、読み込み/書き出しプロセスが中止されたり、速度が低下することがありません。現在は数値アトリビュートおよび行列アトリビュートをサポートしています。

    -----------------------------------------

    defaultValue (float): 手動で既定値を設定します。既定値はファイルに書き込まれません。たとえば、blendShapesの既定値は自動的に1.0に設定され、ディスクに書き込まれません。skinClustersの値は0.0です。すべてのウェイトをディスクに書き込むように強制する必要がある場合は、defaultValue=-1.0に設定します。

    -----------------------------------------

    deformer (string): ウェイトを書き出し/読み込みするデフォーマを指定します。デフォーマ名のパターン(つまりcluster*)を指定した場合は、-skipオプションと組み合わせて使用した場合を除き、パターンと一致する最初のデフォーマの読み込み/書き出しのみが行われます。

    -----------------------------------------

    export (boolean): 指定デフォーマの書き出し

    -----------------------------------------

    format (string): 名前を付けて保存する場合のファイル拡張子として「XML」または「JSON」を指定します。

    -----------------------------------------

    ignoreName (boolean): 読み込む時にレイヤ名を無視し、代わりにレイヤの順序を使用します。これは、ジョイント名を変更した場合にも使用できます。読み込む時に一致した名前のみをそのままデフォーマに書き込みます。

    -----------------------------------------

    im (boolean): 指定したデフォーマにウェイトを読み込みます。対象のデフォーマにウェイトがどのようにマッピングされるかについては、methodフラグを参照してください。

    -----------------------------------------

    method (string): 読み込み時のウェイトのマッピングに使用する手法を指定します。有効な値は、「index」、「nearest」、「barycentric」、「bilinear」、「over」です。「index」法は、頂点インデックスを使用してウェイトをオブジェクトにマッピングします。マッピング先のオブジェクトと書き出し後のデータのトポロジが同じ場合、これが最も便利な手法です。「nearest」法は、読み込んだデータのニアレスト頂点を検索し、ウェイト値をその値に設定します。これは、高解像度メッシュを低解像度メッシュにマッピングする場合に最適です。「barycentric」および「bilinear」法は、-vc/vertexConnectionsフラグを使用して書き出されたポリゴンメッシュでのみサポートされています。「barycentric」法は、入力したジオメトリのニアレスト三角形を検索し、ニアレスト三角形の各頂点の重心ウェイトに応じてウェイトを三角形の頂点に再スケールします。「bilinear」法は、入力したジオメトリのニアレスト凸型四角を検索し、ニアレスト凸型四角の各頂点の重心ウェイトに応じてウェイトを四角の頂点に再スケールします。四角以外のポリゴンでは、「bilinear」メソッドは「barycentric」法にフォールバックします。「over」法は「index」法に似ていますが、マッピング前に対象メッシュのウェイトがクリアされないため、一致していないインデックスのウェイトがそのまま維持されます。

    -----------------------------------------

    path (string): 指定ファイルへのパス。既定は現在のプロジェクトです。

    -----------------------------------------

    positionTolerance (float): nearestのメソッドの検索では、位置の許容値を使用して検索の半径を決定します。このフラグは読み込みのみに使用できます。既定は大きな数値です。

    -----------------------------------------

    remap (string): リマップは、ソースの正規表現を宛先の形式にマップします。これにより、正規表現(セミコロンの前)に一致する名前が、この式の形式(セミコロンの後)にマップされます。たとえば、-remap"test:(.*);$1"の場合、test名前空間内のすべての項目がグローバルネームスペースに名前変更されます。この式の形式では、パターンホルダとして$1、$2、..、$9を使用できます。remapフラグは読み込みまたは書き出しとともに使用する必要があります。読み込みとともに使用すると、正規表現と一致するxlmファイル内のオブジェクトの名前が、シーン内のオブジェクトにリマップされます。書き出しとともに使用すると、正規表現と一致するシーン内のオブジェクトの名前が、xlmファイル内のオブジェクトにリマップされます。

    -----------------------------------------

    shape (string): ソースのシェイプを指定します。書き出すと、シェイプノードのすべてのデフォーマが単一のファイルに書き出されます。シェイプ名のパターン(pCylinder*など)を指定した場合は、-skipオプションと組み合わせて使用した場合を除き、パターンと一致する最初のシェイプの読み込み/書き出しのみが行われます。

    -----------------------------------------

    skip (string): 指定した正規表現の文字列と名前が一致するすべてのデフォーマ、シェイプ、またはレイヤをスキップします。

    -----------------------------------------

    vertexConnections (boolean): -m/-method"barycentric"および"bilinear"で必要な頂点接続情報を書き出します。このフラグを一緒に使用する必要があるのは-ex/-exportフラグのみです。xmlファイルで有効になっている場合は、読み込み中に頂点接続情報が自動的にロードされます。

    -----------------------------------------

    weightPrecision (uint): 書き出されたウェイトの出力の小数精度を設定します。既定値は3です。

    -----------------------------------------

    weightTolerance (float): ウェイトの許容値は、指定したウェイト値が既定値に近いため含める必要がないかどうかを判断するのに使用します。このフラグは書き出しのみに使用できます。既定値は.001です。

    -----------------------------------------

    worldSpace (boolean): 空間ベースの関連メソッド(nearest)の場合、ローカルオブジェクト空間ではなく、ワールド空間の位置をベースにする必要があります。

    -----------------------------------------

    Return Value:
    None: STRINGファイルの読み込み/書き出しに成功した場合は、そのファイルのパスです。照会モードでは、戻り値のタイプは照会されたフラグに基づきます。
    """
    pass

    
def deltaMush(after: bool = False,afterReference: bool = False,before: bool = False,deformerTools: bool = False,envelope: float = 1.0,exclusive: str = "",frontOfChain: bool = False,geometry: str = "",geometryIndices: bool = False,ignoreSelected: bool = False,includeHiddenSelections: bool = False,inwardConstraint: float = 1.0,name: str = "",outwardConstraint: float = 1.0,parallel: bool = False,pinBorderVertices: bool = False,prune: bool = False,remove: bool = False,smoothingIterations: int = 1,smoothingStep: float = 1.0,split: bool = False,useComponentTags: bool = False) -> None:
    """
    このコマンドを使用して、deltaMush ノードを作成、編集、照会します。
    ...
    envelope (float): デルタmushノードのエンベロープ。エンベロープにより、デフォメーションのパーセントが決まります。値は[0,1]の範囲にクランプされます。既定値は1です。
    ...
    inwardConstraint (float): 入力変形シェイプから内側に入らないように頂点の移動を制限して輪郭を維持します。値は[0,1]の範囲です。既定は0.0です。
    ...
    outwardConstraint (float): 入力変形シェイプから外側に出ないように頂点の移動を制限して輪郭を維持します。値は[0,1]の範囲です。既定は0.0です。
    ...
    pinBorderVertices (boolean): 有効にすると、メッシュの境界線上の頂点がスムージング中に現在の位置に固定されます。既定はtrueです。
    ...
    smoothingIterations (uint): デルタmushノードによって実行されるスムージングの反復回数。既定値は10です。

    -----------------------------------------

    smoothingStep (float): スムージングの反復ステップごとに使用されるステップ量です。値は[0,1]の範囲にクランプされます。値を大きくすると不安定になる場合がありますが、値を小さくした場合と比較して収束が速くなります。既定は0.5です。
    ...
    Return Value:
    None: stringデルタ mush デフォーマ ノードの名前照会モードでは、戻り値のタイプは照会されたフラグに基づきます。
    """
    pass
...
def posePanel(control: bool = False,copy: str = "",createString: bool = False,defineTemplate: str = "",docTag: str = "",editString: bool = False,exists: bool = False,init: bool = False,isUnique: bool = False,label: str = "",menuBarRepeatLast: bool = False,menuBarVisible: bool = False,needsInit: bool = False,parent: str = "",popupMenuProcedure: str = "",poseEditor: bool = False,replacePanel: str = "",tearOff: bool = False,tearOffCopy: str = "",tearOffRestore: bool = False,unParent: bool = False,useTemplate: str = "") -> None:
    """
    このコマンドは、poseEditor を格納するベースのパネル クラスから派生するパネルを作成します。
    ...
    poseEditor (boolean): パネルに関連するエディタ名を返す照会モード専用フラグ。
...
def tension(after: bool = False,afterReference: bool = False,before: bool = False,deformerTools: bool = False,envelope: float = 1.0,exclusive: str = "",frontOfChain: bool = False,geometry: str = "",geometryIndices: bool = False,ignoreSelected: bool = False,includeHiddenSelections: bool = False,inwardConstraint: float = 1.0,name: str = "",outwardConstraint: float = 1.0,parallel: bool = False,pinBorderVertices: bool = False,prune: bool = False,remove: bool = False,smoothingIterations: int = 1,smoothingStep: float = 1.0,split: bool = False,useComponentTags: bool = False) -> None:
    """
    このコマンドを使用して、テンション ノードを作成、編集、および照会します。
    ...
    envelope (float): テンションノードのエンベロープ。エンベロープにより、デフォメーションのパーセントが決まります。値は[0,1]の範囲にクランプされます。既定値は1です。
    ...
    outwardConstraint (float): 入力変形シェイプから外側に出ないように頂点の移動を制限して輪郭を維持します。値は[0,1]の範囲です。
    ...
    smoothingIterations (uint): テンションノードによって実行されるスムージングの反復回数。既定値は10です。
    ...
    Return Value:
    None: stringテンション デフォーマ ノードの名前照会モードでは、戻り値のタイプは照会されたフラグに基づきます。
    """
    pass

    
def textureDeformer(after: bool = False,afterReference: bool = False,before: bool = False,deformerTools: bool = False,direction: str = "",envelope: float = 1.0,exclusive: str = "",frontOfChain: bool = False,geometry: str = "",geometryIndices: bool = False,ignoreSelected: bool = False,includeHiddenSelections: bool = False,name: str = "",offset: float = 1.0,parallel: bool = False,pointSpace: str = "",prune: bool = False,remove: bool = False,split: bool = False,strength: float = 1.0,useComponentTags: bool = False,vectorOffset: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),vectorSpace: str = "",vectorStrength: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0)) -> None:
    """
...
def boneLattice(after: bool = False,afterReference: bool = False,before: bool = False,bicep: float = 1.0,deformerTools: bool = False,exclusive: str = "",frontOfChain: bool = False,geometry: str = "",geometryIndices: bool = False,ignoreSelected: bool = False,includeHiddenSelections: bool = False,joint: str = "",lengthIn: float = 1.0,lengthOut: float = 1.0,name: str = "",parallel: bool = False,prune: bool = False,remove: bool = False,split: bool = False,transform: str = "",tricep: float = 1.0,useComponentTags: bool = False,widthLeft: float = 1.0,widthRight: float = 1.0) -> None:
    """
...
def copyFlexor() -> None:
    """
    このコマンドを使うと、既存のボーン/ジョイント フレクサが、あるボーン(ジョイント)から別のボーン(ジョイント)にコピーされます。latticeFfd の微調整と同様、フレクサのアトリビュートと接続が元のフレクサから新規フレクサにコピーされます。選択したボーン(ジョイント)が既存のフレクサのボーン(ジョイント)のミラーであるような場合、ffd ラティス グループのトランスフォームは新規ボーン(ジョイント)に反映されます。コマンドの引数は、ffd ラティスと対象ジョイントの名前です。コマンド ラインに引数が指定されていない場合は、現在のセレクション リストから選択されます。
...
def flexor(atBones: bool = False,atJoints: bool = False,deformerCommand: str = "",list: bool = False,name: str = "",noScale: bool = False,toSkeleton: bool = False,type: str = "") -> None:
    """
    ...
    atBones (boolean): ボーンにフレクサを追加します。-tsフラグも指定されている場合は、フレクサは選択した各ボーンまたは、選択したスケルトンのすべてのボーンに追加されます。

    -----------------------------------------

    atJoints (boolean): ジョイントでフレクサを追加します。-tsフラグが指定されている場合、フレクサは選択した各ジョイントまたは選択したスケルトンのすべてのジョイントに追加されます。

    -----------------------------------------

    deformerCommand (string): 下にあるデフォーマのコマンド文字列を表す文字列です。

    -----------------------------------------

    list (boolean): 考えられるフレクサのあらゆるタイプをリストします。照会モード専用です。
    ...
    noScale (boolean): 近くのジオメトリのサイズにフレクサを自動スケールしないようにします。

    -----------------------------------------

    toSkeleton (boolean): フレクサが、選択したジョイントやボーンだけでなく、スケルトン全体に追加されるように指定します。このフラグは-abと-ajのフラグと一緒に使用されます。

    -----------------------------------------

    type (string): フレクサのタイプを指定します。有効なタイプのリストを確認するには、「flexor-query-list」コマンドを使用します。

    -----------------------------------------

    Return Value:
    None: string[](新規フレクサ ノード名)照会モードでは、戻り値のタイプは照会されたフラグに基づきます。
    """
    pass
...
def geomBind(bindMethod: int = 1,falloff: float = 1.0,geodesicVoxelParams: Tuple[int, bool] = tuple(1, False),maxInfluences: int = 1) -> None:
    ...
    bindMethod (uint): 使用するバインドアルゴリズムを指定します。既定では、測地線ボクセルが使用されます。使用可能なアルゴリズム:3-測地線ボクセル
    ...
    geodesicVoxelParams ([uint, boolean]): パラメータをバインドする測地線ボクセルを指定します。このフラグは0～最大ボクセルグリッド解像度(2の累乗の必要あり)の3つのパラメータで構成されます。たとえば、64,128,256など)1-有効にすると、ポストボクセルの状態の検証を実行します。既定値は256trueです。
...
def skinCluster(addInfluence: str = "",addToSelection: bool = False,after: bool = False,afterReference: bool = False,baseShape: str = "",before: bool = False,bindMethod: int = 1,deformerTools: bool = False,dropoffRate: float = 1.0,exclusive: str = "",forceNormalizeWeights: bool = False,frontOfChain: bool = False,geometry: str = "",geometryIndices: bool = False,heatmapFalloff: float = 1.0,ignoreBindPose: bool = False,ignoreHierarchy: bool = False,ignoreSelected: bool = False,includeHiddenSelections: bool = False,influence: str = "",lockWeights: bool = False,maximumInfluences: int = 1,moveJointsMode: bool = False,name: str = "",normalizeWeights: int = 1,nurbsSamples: int = 1,obeyMaxInfluences: bool = False,parallel: bool = False,polySmoothness: float = 1.0,prune: bool = False,recacheBindMatrices: bool = False,remove: bool = False,removeFromSelection: bool = False,removeInfluence: str = "",removeUnusedInfluence: bool = False,selectInfluenceVerts: str = "",skinMethod: int = 1,smoothWeights: float = 1.0,smoothWeightsMaxIterations: int = 1,split: bool = False,toSelectedBones: bool = False,toSkeletonAndTransforms: bool = False,unbind: bool = False,unbindKeepHistory: bool = False,useComponentTags: bool = False,useGeometry: bool = False,volumeBind: float = 1.0,volumeType: int = 1,weight: float = 1.0,weightDistribution: int = 1,weightedInfluence: bool = False) -> None:
    ...
    volumeBind (float): ボリュームバインドノードを作成し、新しいスキンクラスタノードにアタッチします。このノードは、ボリュームベースのウェイト付けシステムのハルジオメトリのパラメータを保持します。ウェイト付けシステムはインタラクティブスキニングに使用されます。渡される値によって、ボリュームを初期化する際のウェイトの最小値が決まります。ボリュームが増加すると、この値より大きな値の頂点は囲まれます。

    -----------------------------------------

    volumeType (int): バインドするボリューム(volumeBindを参照)の初期シェイプを定義します。0-既定(現在はカプセルに設定)1-カプセル、2-円柱。
...
def geometryConstraint(layer: str = "",name: str = "",remove: bool = False,targetList: bool = False,weight: float = 1.0,weightAliasList: bool = False) -> None:
    """
...
def normalConstraint(aimVector: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),layer: str = "",name: str = "",remove: bool = False,targetList: bool = False,upVector: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),weight: float = 1.0,weightAliasList: bool = False,worldUpObject: str = "",worldUpType: str = "",worldUpVector: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0)) -> None:
    """
...
def pointOnPolyConstraint(layer: str = "",maintainOffset: bool = False,name: str = "",offset: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),remove: bool = False,skip: str = "",targetList: bool = False,weight: float = 1.0,weightAliasList: bool = False) -> None:
    """
    ターゲット オブジェクトの位置、または多くのターゲットの平均位置に、オブジェクトの位置を制限します。
...
def scaleConstraint(layer: str = "",maintainOffset: bool = False,name: str = "",offset: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),remove: bool = False,scaleCompensate: bool = False,skip: str = "",targetList: bool = False,weight: float = 1.0,weightAliasList: bool = False) -> None:
    """
    ...
    maintainOffset (boolean): コンストレイントされたオブジェクトの最初のスケールを保持するために必要なオフセットが計算され、オフセットとして使用されます。
    ...
    offset ([float, float, float]): オフセットの値を設定または照会します。既定は1,1,1です。
    ...
    scaleCompensate (boolean): ジョイントを拘束する場合にのみ使用できます。これは、コンストレイントされたジョイントにscaleCompensateを適用するかどうかを指定します。trueの場合、Tjoint.aCompensateForParentScaleをscaleContraint.aConstraintScaleCompensateに接続します。
...
def tangentConstraint(aimVector: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),layer: str = "",name: str = "",remove: bool = False,targetList: bool = False,upVector: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0),weight: float = 1.0,weightAliasList: bool = False,worldUpObject: str = "",worldUpType: str = "",worldUpVector: Tuple[float, float, float] = tuple(1.0, 1.0, 1.0)) -> None:
    """
...
def ikHandleDisplayScale() -> None:
    ...
    Return Value:
    None: floatikHandle の現在の表示サイズを返します。照会モードでは、戻り値のタイプは照会されたフラグに基づきます。
    """
    pass
...
def jointDisplayScale(absolute: bool = False,ikfk: float = 1.0) -> None:
    """
    ...
    absolute (boolean): スケール係数とは異なり、float引数を表示サイズとして解釈します。

    -----------------------------------------

    ikfk (float): ik/fkスケルトンジョイントの表示サイズを設定します。

    -----------------------------------------

    Return Value:
    None: floatスケルトン ジョイントの現在の表示サイズを返します。照会モードでは、戻り値のタイプは照会されたフラグに基づきます。
    """
    pass
...
def reroot() -> None:
    """
    スケルトンのルートが再設定されます。選択したジョイント、またはコマンド ラインで指定したジョイントが、スケルトンの新しいルートになります。選択したジョイントかその上を通過するすべての IK ハンドルは削除されます。
...
def applyTake(channel: str = "",device: str = "",filter: str = "",preview: bool = False,recurseChannel: bool = False,reset: bool = False,specifyChannel: bool = False,startTime: float = 1.0) -> None:
    """
    ...
    channel (string): このフラグは、セットチャネルの有効値をオーバーライドします。チャネルが指定されると、チャネルは有効になります。C:既定は、デバイス用にapplyTakeが有効になっているチャネルすべてです。

    -----------------------------------------

    device (string): どのデバイスにテイクを含めるかを指定します。C:既定は、applyTakeが有効になっているすべてのデバイスです。

    -----------------------------------------

    filter (string): このフラグは、applyTake使用時に使うフィルタを指定します。このフラグが複数回使用される場合、フィルタの順序は左から右になります。C:既定は、フィルタなしです。

    -----------------------------------------

    preview (boolean): デバイスアタッチメントに接続されたターゲットアトリビュートにアタッチされているblendDeviceノードにテイクを適用します。アトリビュートにアタッチされているアニメーションカーブは、プレビューデータが定義される場合以外は変更されません。プレビューデータは再生時に使用されるデータです。C:既定は、プレビューしないです。

    -----------------------------------------

    recurseChannel (boolean): このフラグを使用すると、-c/channelで指定されるチャネルの子にも適用されます。C:既定は有効になったすべてのチャネルです。

    -----------------------------------------

    reset (boolean): -previewによって影響を受けるblendDeviceノードをリセットします。プレビューデータが除去され、アニメーションカーブが存在する場合は、それらのカーブが再生時に使用されます。

    -----------------------------------------

    specifyChannel (boolean): このフラグは、-c/channelフラグと一緒に使用されます。使用される場合、applyTakeは、-c/channelフラグでリストされるチャネルでだけ動作します。C:既定は有効になったすべてのチャネルです。

    -----------------------------------------

    startTime (time): テイク用の既定開始時間は記録時に決定されます。startTimeオプションは、現在のアニメーション単位でテイクの開始時間を設定します。C:既定は、テイクの最初のタイムスタンプです。テイクのタイムスタンプが存在しない場合は、0が使用されます。
...
def defineDataServer(device: str = "",server: str = "",undefine: bool = False) -> None:
    """
    ...
    device (string): サーバ接続に必要なデバイス名を指定します。デバイス名は固有でなければなりません。固有でない場合は、コマンドが失敗します。

    -----------------------------------------

    server (string): 定義したデバイスが接続するサーバの名前を指定します。2とおりの方法で指定できます。name--サーバソケットの名前name形式のサーバ名は、nameに対応するローカルホストでサーバソケットに接続します。nameが「/」で始まらない場合は、/tmp/nameが使用されます。これは、ほとんどのサーバの既定の動作です。nameが「/」で始まる場合、nameはソケットのフルパスを示します。host:service-指定ホスト上のudpサービスです。serviceは、「udpサービス名」、「ポート番号」、または「tcpmux」の名前付きサービスのいずれかで、この順序で見つかります。hostを省略すると、ローカルホストが使用されます。どんな場合でも、サーバがみつからない場合、デバイスは定義(作成)されず、このコマンドはエラーになります。

    -----------------------------------------

    undefine (boolean): dataServerデバイスの定義を取り消し(破棄)して、サーバとの接続を閉じます。
...
def defineVirtualDevice(axis: int = 1,channel: str = "",clear: bool = False,create: bool = False,device: str = "",parent: str = "",undefine: bool = False,usage: str = "") -> None:
    """
    ...
    axis (int): チャネルの軸番号を指定します。子はすべて、親の軸番号と親のチャネルの幅で定義された軸番号を持ちます。このフラグが使用されない場合、チャネルの順序で軸番号が定義されます。

    -----------------------------------------

    channel (string): -createの開始後、チャネルがデバイス定義に追加されることがあります。チャネル文字列は、デバイスに追加されているチャネルの名前です。-channelフラグも、-usageフラグと場合により-axisフラグを付ける必要があります。

    -----------------------------------------

    clear (boolean): -clearオプションデバイスの定義を終了して、定義されたチャネルを破棄します。

    -----------------------------------------

    create (boolean): 仮想デバイスの定義を開始します。デバイスの定義中であれば、-createフラグはエラーを生成します。

    -----------------------------------------

    device (string): -deviceフラグはデバイスの定義を終了します。-createフラグと-deviceフラグ間のチャネルすべてが指定したデバイスに追加されます。デバイスがすでに存在すれば、このコマンドはエラーになり、デバイスを別のデバイス名で再定義する必要があります。現在定義されているデバイスを確認するには、listInputDevicesコマンドを使用します。-deviceフラグも-undefineと一緒に使用して仮想デバイスの定義を取り消します。

    -----------------------------------------

    parent (string): 定義されるチャネルの親チャネルを指定します。チャネルが存在しない場合、または互換性のないタイプの場合、このコマンドはエラーになります。

    -----------------------------------------

    undefine (boolean): -deviceフラグで指定したデバイスの定義を取り消します。

    -----------------------------------------

    usage (string): -usageオプションは、すべての-channelフラグに必要です。定義されたチャネルの使用タイプを記述します。使用タイプは次のとおりです。unknownscalarposrotposRotquaternionposQuaternionrotXYZrotYZXrotZXYrotXZYrotYXZrotZYXposRotXYZposRotYZXposRotZXYposRotXZYposRotXZYposRotZYXposXposYposZrotXrotYrotZ
...
def deviceManager(attachment: bool = False,axisCoordChanges: bool = False,axisIndex: int = 1,axisName: bool = False,axisOffset: bool = False,axisScale: bool = False,deviceIndex: int = 1,deviceNameFromIndex: int = 1,numAxis: bool = False,numDevices: bool = False) -> None:
    """
    このコマンドは、接続されたデバイスに関する情報について、内部デバイス マネージャを照会します。
    ...
    attachment (boolean): デバイスと軸が接続されているプラグを返します。-deviceIndexと-axisIndexフラグ一緒に使用することを要求します。

    -----------------------------------------

    axisCoordChanges (boolean): 軸の座標が変化するかどうかを返します。-deviceIndexと-axisIndexフラグ一緒に使用することを要求します。

    -----------------------------------------

    axisIndex (int): 通常はその他のフラグと一緒に使用され、軸のインデックスを示します。

    -----------------------------------------

    axisName (boolean): 軸の名前を返します。-deviceIndexと-axisIndexフラグ一緒に使用することを要求します。

    -----------------------------------------

    axisOffset (boolean): 軸のオフセットを返します。-deviceIndexと-axisIndexフラグ一緒に使用することを要求します。

    -----------------------------------------

    axisScale (boolean): 軸のスケールを返します。-deviceIndexと-axisIndexフラグ一緒に使用することを要求します。

    -----------------------------------------

    deviceIndex (int): 通常はその他のフラグと一緒に使用され、デバイスのインデックスを示します。

    -----------------------------------------

    deviceNameFromIndex (int): 指定したインデックスを持つデバイスの名前を返します。

    -----------------------------------------

    numAxis (boolean): デバイスが持つ軸の数を返します。-deviceIndexフラグの使用を要求します。

    -----------------------------------------

    numDevices (boolean): 現在接続されているデバイスの数を返します。
...
def enableDevice(apply: bool = False,device: str = "",enable: bool = False,monitor: bool = False,record: bool = False) -> None:
    """
    ...
    apply (boolean): 指定したチャネルの「applyTake」を有効/無効にします。

    -----------------------------------------

    device (string): 変更するデバイスを指定します。

    -----------------------------------------

    enable (boolean): モニタ、記録、適用を有効(または無効)にします。

    -----------------------------------------

    monitor (boolean): デバイスの更新の表示を有効/無効にします(既定)。

    -----------------------------------------

    record (boolean): 「recordDevice」デバイスの記録を有効/無効にします。
...
def filter(name: str = "",type: str = "") -> None:
    """
    ...
    name (string): 作成されるフィルタの名前

    -----------------------------------------

    type (string): 作成するフィルタのタイプ:filterEulerオイラー角の「demangler」filterResampleいくつかのフィルタリングオプションで入力データを一定の出力率でリサンプルします。filterSimplifyほぼ直線のデータポイントのグループをラインセグメントに統合します。filterClosestSample最も近いサンプルポイントを使って、入力データを一定の出力率でリサンプルします。

    -----------------------------------------

    Return Value:
    None: stringフィルタ名照会モードでは、戻り値のタイプは照会されたフラグに基づきます。
    """
    pass

    
def movIn(file: str = "",startTime: float = 1.0) -> None:
    """
    リストしたアトリビュートに接続されるアニメーション カーブに .mov ファイルが読み込まれます。アニメーション カーブが作成されてアトリビュートに接続されるので、アトリビュートは書込み可能である必要があります。アニメーション カーブがアトリビュートにすでに接続されている場合、読み込まれるデータはそのカーブにペーストされます。
    ...
    file (string): .movファイルの名前。拡張子を使用しない場合は、.movが追加されます。

    -----------------------------------------

    startTime (time): .movファイルを読み込む場合の既定の開始時間が現在のタイムになります。startTimeオプションは、現在のタイム単位で.movデータの開始時間を設定します。
...
def movOut(comment: bool = False,file: str = "",precision: int = 1,time: Tuple[float, float] = tuple(1.0, 1.0)) -> None:
    """
    リストしたアトリビュートから .mov ファイルが書き出されます。有効なアトリビュート タイプは、数値アトリビュート、時間アトリビュート、リニア アトリビュート、角度アトリビュート、以前にリストされたタイプから構成される複合アトリビュート、および以前にリストされたタイプから構成されるマルチ アトリビュートです。
    ...
    comment (boolean): trueにすると、.movファイルに書き込まれたアトリビュートは、.movファイル冒頭のコメントセクションにリストされます。既定はfalseです。
    ...
    precision (uint): .movファイルの小数点の右側の桁数を設定します。C:既定は6です。

    -----------------------------------------

    time (timerange): .movファイルとして保存する時間範囲。既定は現在の時間範囲です。
...
def readTake(angle: str = "",device: str = "",frequency: float = 1.0,linear: str = "",noTime: bool = False,take: str = "") -> None:
    """
    このアクションでは、定義されたデバイスにテイク ファイル(.mov)を読み込みます。
    ...
    angle (string): テイクで使用する角度単位を設定します。有効な文字列は「deg」、「degree」、「rad」、「radian」です。C:既定は、現在のユーザ角度単位です。

    -----------------------------------------

    device (string): テイクデータを読み込むデバイスを指定します。この引数は必須です。

    -----------------------------------------

    frequency (float): タイムスタンプは無視され、指定した周波数が使用されます。timeStampデータが.movファイルにない場合、-noTimestampフラグも使用されます。データではなく、このフラグのリサンプルが、指定した周波数であることが仮定されます。C:テイクファイルがタイムスタンプを使用しない場合、既定の周波数は60Hzです。

    -----------------------------------------

    linear (string): テイクで使用するリニア単位を設定します。有効な文字列は「mm」、「millimeter」、「cm」、「centimeter」、「m」、「meter」、「km」、「kilometer」、「in」、「inch」、「ft」、「foot」、「yd」、「yard」、「mi」、「mile」です。C:既定は、現在のユーザ長さ単位です。

    -----------------------------------------

    noTime (boolean): テイクファイル(.movファイル)がタイムスタンプを含むかどうかを指定します。C:既定では、タイムスタンプがテイクファイルの一部であることが仮定されます。

    -----------------------------------------

    take (string): 指定したテイクファイルを読み込みます。フラグにフルパスを渡すのが最も安全です。
...
def recordDevice(cleanup: bool = False,data: bool = False,device: str = "",duration: int = 1,playback: bool = False,state: bool = False,wait: bool = False) -> None:
    """
    ...
    cleanup (boolean): 記録されたデータをデバイスから除去します。

    -----------------------------------------

    data (boolean): デバイスに記録されたデータがあるかどうかを指定します。照会されたときにデバイスが記録中であれば、フラグはfalseを返します。Q:照会するとintを返します。

    -----------------------------------------

    device (string): 記録を開始するデバイスを指定します。リストされたデバイスでは、記録可能状態に関係なく記録が始まります。C:既定では、記録が有効なすべてのデバイスで記録が始まります。

    -----------------------------------------

    duration (int): 秒単位の記録期間です。記録期間が過ぎてもデバイスは記録状態にある場合、記録を停止するよう通知しなければなりません。C:既定は60です。Q:照会するとintを返します。

    -----------------------------------------

    playback (boolean): アトリビュートがアニメーションカーブに接続されている場合、アニメーションカーブはデバイスの記録中に再生され、アトリビュートにアタッチされたすべてのアニメーションカーブが記録されます。C:既定はfalseです。Q:照会するとintを返します。

    -----------------------------------------

    state (boolean): デバイスの記録を開始または停止します。C:既定はtrueです。Q:照会するとintを返します。

    -----------------------------------------

    wait (boolean): -p/playbackが指定されている場合、再生が完了するまで待機してからコントロールをユーザに返します。-pを使用しない場合、このフラグは無視されます。
...
def writeTake(angle: str = "",device: str = "",linear: str = "",noTime: bool = False,precision: int = 1,take: str = "",virtualDevice: str = "") -> None:
    """
    ...
    angle (string): テイクで使用する角度単位を設定します。[deg|degree|rad|radian]の文字列が有効です。C:既定は、現在のユーザ角度単位です。

    -----------------------------------------

    device (string): テイクを含むデバイスを指定します。この引数は必須です。デバイスにテイクが入っていない場合、このアクションは失敗します。

    -----------------------------------------

    linear (string): テイクで使用するリニア単位を設定します。有効文字列は、[mm|millimeter|cm|centimeter|m|meter|km|kilometer|in|inch|ft|foot|yd|yard|mi|mile]です。C:既定は、現在のユーザ長さ単位です。

    -----------------------------------------

    noTime (boolean): テイクファイル(.mov)ファイルにタイムスタンプを含めないようにします。C:既定では、テイクファイルにはタイムスタンプが入ります。

    -----------------------------------------

    precision (int): テイクファイルの小数部分の数字の桁数を設定します。C:既定は6です。

    -----------------------------------------

    take (string): 指定した名前でテイクを書き出します。

    -----------------------------------------

    virtualDevice (string): 仮想デバイス設定を、指定した名前でMELスクリプトに書き出します。
